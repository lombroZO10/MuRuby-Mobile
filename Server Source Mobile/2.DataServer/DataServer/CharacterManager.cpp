...
#if(CB_AutoBanking)
bool CCharacterManager::GetAccountInfo(CHARACTER_INFO* lpCharacterInfo, char* Account) // OK
{
	this->m_critical.lock();

	std::string nme(Account);

	std::transform(nme.begin(), nme.end(), nme.begin(), tolower);

	for (std::map<std::string, CHARACTER_INFO>::iterator it = this->m_CharacterInfo.begin(); it != this->m_CharacterInfo.end(); it++)
	{
		if (strcmp(it->second.Account, Account) == 0)
		{
			(*lpCharacterInfo) = it->second;
			this->m_critical.unlock();
			return 1;
...
bool CCharacterManager::CheckCharacterOnline(char* name) // OK
{
	this->m_critical.lock();
