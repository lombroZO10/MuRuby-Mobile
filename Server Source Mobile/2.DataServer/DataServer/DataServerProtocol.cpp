...
void DataServerProtocolCore(int index,BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0x39:
			...
			break;
		case 0x41:
			GDMonterPointSaveRecv((SDHP_MONTERPOINT_SAVE_SEND*)lpMsg);
			break;
		case 0x3A:
		...
		{
#if(CUSTOM_BCHOTROI)
			g_ChoTroi.Protocol(((lpMsg[0] == 0xC1) ? lpMsg[3] : lpMsg[4]), lpMsg, index);
#endif
		}
		break;
		case 0xD3:
			...
			case 0x7A:
			{
				gCBAutoNapGame.SendConfigToGS(index);
			}
			break;
			case 0x7B:
			{
				gCBAutoNapGame.UserGetLichSuNap(lpMsg, index);
			}
			break;
			case 0x7C:
			{
				gCBAutoNapGame.RecvNapTienBank(lpMsg, index);
			}
			break;
#endif
			}
...
#if(DANH_HIEU_NEW)
		pMsg.rDanhHieu = gQueryManager.GetAsInteger("DanhHieu"); // only add in is void
#endif
		#if(DATASERVER_UPDATE>=602)
...
#if (CB_VIP_CHAR)
		//==========Select Custom CB
		gQueryManager.ExecQuery("SELECT mLvVIPChar,mPointVIPChar from Character where AccountID='%s' AND Name='%s'", lpMsg->account, lpMsg->name);

		gQueryManager.Fetch();

		pMsg.LvVIPChar = (DWORD)gQueryManager.GetAsInteger("mLvVIPChar");
		pMsg.PointVIPChar = (DWORD)gQueryManager.GetAsInteger("mPointVIPChar");

		gQueryManager.Close();
#endif
#if (CB_NEWQUESTFF)
		//==========Select Custom CB
		gQueryManager.ExecQuery("SELECT NQuest_Status,NQuest_LV,NQuest_C1,NQuest_C2,NQuest_C3,NQuest_C4,NQuest_C5 from Character where AccountID='%s' AND Name='%s'", lpMsg->account, lpMsg->name);

		gQueryManager.Fetch();

		pMsg.NQuest_LV = (DWORD)gQueryManager.GetAsInteger("NQuest_LV");
		pMsg.NQuest_Status = (DWORD)gQueryManager.GetAsInteger("NQuest_Status");
		pMsg.NQuest_C[0] = (DWORD)gQueryManager.GetAsInteger("NQuest_C1");
		pMsg.NQuest_C[1] = (DWORD)gQueryManager.GetAsInteger("NQuest_C2");
		pMsg.NQuest_C[2] = (DWORD)gQueryManager.GetAsInteger("NQuest_C3");
		pMsg.NQuest_C[3] = (DWORD)gQueryManager.GetAsInteger("NQuest_C4");
		pMsg.NQuest_C[4] = (DWORD)gQueryManager.GetAsInteger("NQuest_C5");
...
void GDCharacterInfoSaveRecv(SDHP_CHARACTER_INFO_SAVE_RECV* lpMsg) // OK
{
	...
	gQueryManager.Close();
#if(DANH_HIEU_NEW)
	gQueryManager.ExecQuery("UPDATE Character SET DanhHieu=%d WHERE Name='%s'", lpMsg->rDanhHieu, lpMsg->name);
	gQueryManager.Close();

#endif
	//=== Custom Save
#if (CB_VIP_CHAR)
	gQueryManager.ExecQuery("UPDATE Character SET mLvVIPChar='%d',mPointVIPChar='%d' WHERE AccountID='%s' AND Name='%s'", lpMsg->LvVIPChar, lpMsg->PointVIPChar, lpMsg->account, lpMsg->name);
	gQueryManager.Close();
#endif
#if (CB_NEWQUESTFF)
	gQueryManager.ExecQuery("UPDATE Character SET NQuest_Status='%d', NQuest_LV='%d',NQuest_C1='%d',NQuest_C2='%d',NQuest_C3='%d',NQuest_C4='%d',NQuest_C5='%d' WHERE AccountID='%s' AND Name='%s'", lpMsg->NQuest_Status,lpMsg->NQuest_LV, lpMsg->NQuest_C[0], lpMsg->NQuest_C[1], lpMsg->NQuest_C[2], lpMsg->NQuest_C[3], lpMsg->NQuest_C[4], lpMsg->account, lpMsg->name);
	gQueryManager.Close();
#endif
...
//====================
void GDMonterPointSaveRecv(SDHP_MONTERPOINT_SAVE_SEND* lpMsg) // OK
{
	gQueryManager.ExecQuery("EXEC WZ_SetDiemMonter '%s','%s','%d'", lpMsg->account, lpMsg->name, lpMsg->PointAdd);
	gQueryManager.Fetch();
	gQueryManager.Close();
}
