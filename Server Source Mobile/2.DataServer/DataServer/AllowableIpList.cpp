...
{
	if(this->m_AllowableIpInfo.find("*") != this->m_AllowableIpInfo.end() || this->m_AllowableIpInfo.find("all") != this->m_AllowableIpInfo.end() || this->m_AllowableIpInfo.find("0.0.0.0") != this->m_AllowableIpInfo.end())
	{
		return 1;
	}

	for(std::map<std::string,ALLOWABLE_IP_INFO>::iterator it = this->m_AllowableIpInfo.begin(); it != this->m_AllowableIpInfo.end(); it++)
	{
		const std::string& allowableIp = it->first;

		if(allowableIp.empty() == 0 && allowableIp[allowableIp.size()-1] == '.' && strncmp(ip,allowableIp.c_str(),allowableIp.size()) == 0)
		{
			return 1;
		}
	}
