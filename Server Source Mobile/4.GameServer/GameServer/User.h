...
#define MAX_CHAT_MESSAGE_SIZE 59	//chinh dai ki tu chat
#define MAX_ROAD_PATH_TABLE 16
#if LAGBOSS
#define MAX_MONSTER_SEND_MSG 20 //200
#define MAX_MONSTER_SEND_ATTACK_MSG 30 //300
#else
...
/*+300*/	char	ObtainPickItemList[12][16];
	...
	DWORD DHGST;
#endif
	DWORD BankJewelry[15]; //tam
	bool CacheSendOnlogin;
#if(JEWELBANKVER2)
	...
	DWORD CacheAttackTimmeRun;
#endif
#if(CB_ATTACK_HIDEN_WING)
	DWORD CacheAttackTimmeRun2;
#endif
#if(CB_AUTORESETINFO)
	int AutoResetType;
#endif
	...
	DWORD SecurityPacketWindowTick;
	DWORD SecurityPacketWindowCount;
	DWORD SecurityLastRateLogTick;
	DWORD SecurityLastStateLogTick;
	DWORD SecurityLastUnknownLogTick;
	DWORD SecurityStateViolationCount;
	DWORD SecurityUnknownCount;
	DWORD SecurityRepeatWindowTick;
	DWORD SecurityRepeatCount;
	BYTE SecurityRepeatHead;
	BYTE SecurityRepeatSub;
	//=================================================
//FakeOnline_EMU
...
int GetItemCountInInventory(int aIndex, int ItemID, int ItemLevel, int ExlOpt = -1, int Skill = -1, int Luck = -1, int Option = -1);
int DeleteItemsCount(int aIndex, short Type, short Level, int dCount, int ExlOpt = -1, int Skill = -1, int Luck = -1, int Option = -1);
bool CBCustomAddMob(int aIndex, int Class, int Map, int PosX, int PosY, int Dir, DWORD RemainTime = 0);

LPOBJ gObjFindByAcc(char* Account); // OK	//FakeOnline_EMU
