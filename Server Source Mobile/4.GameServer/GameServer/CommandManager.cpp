...
bool CCommandManager::ManagementCore(LPOBJ lpObj,char* message, int Npc) // OK
{
	...
	switch(code)
	{
		...
		case COMMAND_STARTARENA:
			...
			break;
		case COMMAND_SETPASSPARTY:
			Result = CommandSetPassParty(lpObj, argument);
			break;
...
bool CCommandManager::CommandSetPassParty(LPOBJ lpObj, char* arg) // OK
{
	lpObj->Option |= 1;
	lpObj->Option |= 2;
	lpObj->Option |= 4;
...
bool CCommandManager::CommandPost(LPOBJ lpObj,char* arg) // OK
{
	...
	//	GDGlobalPostSend(gMapServerManager.GetMapServerGroup(), 1, lpObj->Name, arg); //Gốc
	}
...
// Update Giết người mất zen tính theo mạng đạt thêm 24/12/2024
	int Count = 0;
	Count = lpObj->PKCount*2000000;
	if(lpObj->PKCount >=1)
	{
...
void CCommandManager::CommandResetAuto(LPOBJ lpObj,char* arg,int Npc) // OK
{
	...
	SendStatusAutoReset(lpObj->Index);
}
...
void CCommandManager::CommandResetAutoProc(LPOBJ lpObj) // OK
{
	...
	if (lpObj->AutoResetType == 2)
	{
		...
		if (!gResetTableVip.KiemTraDieuKienReset(lpObj))
		{
			//LogAdd(LOG_RED, "[RS] Check 10");
			return;
		}
	}
	else
	{
		...
		if (!gResetTable.KiemTraDieuKienReset(lpObj))
		{
			//LogAdd(LOG_RED, "[RS] Check 10");
			return;
		}
...
void CCommandManager::DGCommandResetRecv(SDHP_COMMAND_RESET_RECV* lpMsg) // OK
{
	...
	if(gServerInfo.m_CommandResetType == 1)
	{
		...
		if (lpObj->AutoResetType == 2) //Rs VIP
		{
			point = gResetTableVip.GetResetPoint(lpObj,true);
			gResetTableVip.KiemTraDieuKienReset(lpObj, true);
		}
		else
		{
			point = gResetTable.GetResetPoint(lpObj, true);
			gResetTable.KiemTraDieuKienReset(lpObj, true);
		}
#else
	...
	if (gServerInfo.m_CommandResetType == 2)
	{
		...
		if (lpObj->AutoResetType == 2) //Rs VIP
		{
			if (gResetTableVip.GetResetPointKeep(lpObj) > 0)
			{
		...
		{
			if (gResetTable.GetResetPointKeep(lpObj) > 0)
			{
				point = gResetTable.GetResetPointKeep(lpObj);
			}
			gResetTable.KiemTraDieuKienReset(lpObj, true);
		...
		lpObj->LevelUpPoint += point;
#else
		int point = gResetTable.GetResetPointTable(lpObj);
...
bool CCommandManager::CommandChangeClass(LPOBJ lpObj,char* arg,int Npc) // OK
{
	...
	if (gChangeClass.Enable) return 0;
...
bool CCommandManager::CommandRename(LPOBJ lpObj,char* arg) // OK
{
	...
	for (int n = 0; n < 16; n++)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1603));
			return 0;
		}
	}
#endif
...
bool CCommandManager::CommandReAdd(LPOBJ lpObj, char* arg, int Npc)
{
	...
	lpObj->CloseCount = gServerInfo.m_ThoiGianThoatGame; //Log Out
