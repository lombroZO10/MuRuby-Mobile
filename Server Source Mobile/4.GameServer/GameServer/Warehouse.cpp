	...
	for (int n = 0; n < WAREHOUSE_SIZE; n++)
	{
		if (lpObj->Warehouse[n].IsItem() != 0)
		{
			...
			gLog.Output(LOG_ITEM_CHARACTER, "[Baú Compartilhado] - [%s +%d] - DB[%.0f] TC[%d] Sorte[%d] Op[%d] Exc[%d] Conjunto[%d] oEx[%d] Har[%d] Socket[%03d,%03d,%03d,%03d,%03d][%d] Série[%08X]",
				...
				lpObj->Warehouse[n].m_Option1,
				lpObj->Warehouse[n].m_Option2,
				lpObj->Warehouse[n].m_Option3,
				lpObj->Warehouse[n].m_NewOption,
				lpObj->Warehouse[n].m_SetOption,
				lpObj->Warehouse[n].m_ItemOptionEx,
				lpObj->Warehouse[n].m_JewelOfHarmonyOption,
				lpObj->Warehouse[n].m_SocketOption[0],
				lpObj->Warehouse[n].m_SocketOption[1],
				lpObj->Warehouse[n].m_SocketOption[2],
				lpObj->Warehouse[n].m_SocketOption[3],
				lpObj->Warehouse[n].m_SocketOption[4],
				lpObj->Warehouse[n].m_SocketOptionBonus,
				lpObj->Warehouse[n].m_Serial);
		}
...
void CWarehouse::GDWarehouseItemSaveSend(int aIndex) // OK
{
	...
	for (int n = 0; n < WAREHOUSE_SIZE; n++)
	{
		if (lpObj->Warehouse[n].IsItem() != 0)
		{
			...
			gLog.Output(LOG_ITEM_HomDo, "[Item Warehouse] - [%s +%d] - [%.0f,%d,%d,%d] ex[%d] set[%d] oEx[%d] ho[%d] sk[%03d,%03d,%03d,%03d,%03d][%d] seri[%08X]",
				gItemManager.GetItemName(lpObj->Warehouse[n].m_Index),
				lpObj->Warehouse[n].m_Level,
				lpObj->Warehouse[n].m_Durability,
				lpObj->Warehouse[n].m_Option1,
