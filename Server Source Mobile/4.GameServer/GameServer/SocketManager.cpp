...
bool CSocketManager::CreateWorkerThread() // OK
{
	...
	this->m_ServerWorkerThreadCount = MAX_SERVER_WORKER_THREAD;
...
bool CSocketManager::DataRecv(int index,IO_MAIN_BUFFER* lpIoBuffer) // OK
{
	...
	while(true)
	{
		...
		{
			gLog.Output(LOG_CONNECT,"[SECURITY][GameServer] Protocol header error (Index: %d, Ip: %s, Account: %s, Header: %x)",index,gObj[index].IpAddr,gObj[index].Account,lpMsg[count]);
			return 0;
		}

		if(size < 3 || size > MAX_MAIN_PACKET_SIZE)
		{
			gLog.Output(LOG_CONNECT,"[SECURITY][GameServer] Protocol size error (Index: %d, Ip: %s, Account: %s, Header: %x, Size: %d, Head: %x)",index,gObj[index].IpAddr,gObj[index].Account,header,size,head);
			return 0;
		}
