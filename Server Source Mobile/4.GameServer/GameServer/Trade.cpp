	...
	for(int n=0;n < TRADE_SIZE;n++)
	{
		if(lpObj->Trade[n].IsItem() != 0)
		{
			gLog.Output(LOG_TRADE, "Comércio de %s | %s para %s | %s, Índice: %04d:, Item %s + %d, Habilidade: %d, Sorte: %d, Opção: %d, Excellent: %d, Ancient: %d, Harmony: %03d, Item 380: %03d, Socket: %03d, %03d, %03d, %03d, %03d, Série: %08X)"
				...
				, lpObj->Name
				, lpTarget->Account
				, lpTarget->Name, lpObj->Trade[n].m_Index
...
#if(CB_BOTTRADE)
	if (gCB_BotTrader.TradeOpen(aIndex, bIndex) == 1)
	{
		return;
	}
#endif	
...
#endif
#if(CB_BOTTRADE)
		if (gCB_BotTrader.IsInTrade(aIndex) == 1 && lpMsg->flag == 1)
		{
			gCB_BotTrader.TradeOk(aIndex);
			return;
		}
#endif	
	int bIndex = lpObj->TargetNumber;
...
#endif	
#if(CB_BOTTRADE)
		if (gCB_BotTrader.IsInTrade(aIndex) == 1)
		{
			gCB_BotTrader.TradeCancel(aIndex);
			return;
		}
#endif	
	int bIndex = lpObj->TargetNumber;
