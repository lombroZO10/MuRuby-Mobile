...
namespace
{
	BYTE GetGameServerPacketSub(BYTE* lpMsg,int size)
	{
		if(size < 4)
		{
			return 0xFF;
		...
		if((lpMsg[0] == 0xC2 || lpMsg[0] == 0xC4) && size >= 5)
		{
			return lpMsg[4];
		}
	...
	bool IsGameServerCoreGameplayPacket(BYTE head)
	{
		switch(head)
		{
			case PROTOCOL_CODE2:
	...
	bool HasUnsafeLoginText(char* text,int size)
	{
	...
	void GameServerSecurityTrackPacket(LPOBJ lpObj,BYTE head,BYTE sub,int size,int encrypt)
	{
		...
		DWORD threshold = ((lpObj->Connected >= OBJECT_ONLINE) ? 250 : 80);

		if(lpObj->SecurityPacketWindowTick == 0 || (tick-lpObj->SecurityPacketWindowTick) > 1000)
		{
			lpObj->SecurityPacketWindowTick = tick;
			lpObj->SecurityPacketWindowCount = 0;
		}

		if((++lpObj->SecurityPacketWindowCount) > threshold && (tick-lpObj->SecurityLastRateLogTick) > 5000)
		{
			gLog.Output(LOG_HACK,"[SECURITY][GameServer] High packet rate (Index: %d, Account: %s, Name: %s, Ip: %s, Connected: %d, Count: %u, Head: 0x%02X, Sub: 0x%02X, Size: %d, Encrypt: %d)",lpObj->Index,lpObj->Account,lpObj->Name,lpObj->IpAddr,lpObj->Connected,lpObj->SecurityPacketWindowCount,head,sub,size,encrypt);
			lpObj->SecurityLastRateLogTick = tick;
		}

		if(lpObj->Connected < OBJECT_ONLINE)
		{
			if(lpObj->SecurityRepeatWindowTick == 0 || (tick-lpObj->SecurityRepeatWindowTick) > 3000 || lpObj->SecurityRepeatHead != head || lpObj->SecurityRepeatSub != sub)
			{
				lpObj->SecurityRepeatWindowTick = tick;
				lpObj->SecurityRepeatHead = head;
				lpObj->SecurityRepeatSub = sub;
				lpObj->SecurityRepeatCount = 0;
			}

			if((++lpObj->SecurityRepeatCount) > 20 && (tick-lpObj->SecurityLastUnknownLogTick) > 3000)
			{
				gLog.Output(LOG_HACK,"[SECURITY][GameServer] Repeated pre-login packet (Index: %d, Account: %s, Ip: %s, Connected: %d, Head: 0x%02X, Sub: 0x%02X, RepeatCount: %u)",lpObj->Index,lpObj->Account,lpObj->IpAddr,lpObj->Connected,head,sub,lpObj->SecurityRepeatCount);
				lpObj->SecurityLastUnknownLogTick = tick;
			}
		}
	}

	void GameServerSecurityLogState(LPOBJ lpObj,const char* phase,BYTE head,BYTE sub)
	{
		if(lpObj->Type != OBJECT_USER)
		{
			return;
		...
		lpObj->SecurityStateViolationCount++;

		if((tick-lpObj->SecurityLastStateLogTick) < 1500)
		{
			return;
		}

		gLog.Output(LOG_HACK,"[SECURITY][GameServer] State anomaly (Index: %d, Account: %s, Name: %s, Ip: %s, Phase: %s, Connected: %d, Head: 0x%02X, Sub: 0x%02X, Count: %u)",lpObj->Index,lpObj->Account,lpObj->Name,lpObj->IpAddr,phase,lpObj->Connected,head,sub,lpObj->SecurityStateViolationCount);
		lpObj->SecurityLastStateLogTick = tick;
	}

	void GameServerSecurityTrackPhase(LPOBJ lpObj,BYTE head,BYTE sub)
	{
		if(lpObj->Type != OBJECT_USER)
		{
			return;
		...
		{
			if(head == 0x03 || head == 0x0E || (head == 0xF1 && (sub == 0x01 || sub == 0x02)))
			{
				return;
			}

			if(IsGameServerCoreGameplayPacket(head) != 0 || head == 0xF3)
			{
				GameServerSecurityLogState(lpObj,"pre_login",head,sub);
			}
		}
		else if(lpObj->Connected == OBJECT_LOGGED)
		{
			if(IsGameServerCoreGameplayPacket(head) != 0)
			{
				GameServerSecurityLogState(lpObj,"pre_character_enter",head,sub);
			}
		}
	}
}
...
void ProtocolCore(BYTE head,BYTE* lpMsg,int size,int aIndex,int encrypt,int serial) // OK
{
	...
	BYTE sub = GetGameServerPacketSub(lpMsg,size);
	LPOBJ lpObj = &gObj[aIndex];

	if(lpObj->Type == OBJECT_USER)
	{
		GameServerSecurityTrackPacket(lpObj,head,sub,size,encrypt);
		GameServerSecurityTrackPhase(lpObj,head,sub);
	}
	...
	if (gObj[aIndex].Type == OBJECT_USER &&lpMsg[0] == 0xC1 && head == 0xFB)
	{
		...
		break;
		case 0x08: gChangeClass.RecvChangeClass((CG_CHANGECLASS_RECV*)lpMsg, aIndex); break;
		default:
			gObj[aIndex].SecurityUnknownCount++;
			gLog.Output(LOG_HACK,"[SECURITY][GameServer] Unknown custom packet (Index: %d, Account: %s, Name: %s, Ip: %s, Head: 0x%02X, Sub: 0x%02X, Count: %u)",aIndex,gObj[aIndex].Account,gObj[aIndex].Name,gObj[aIndex].IpAddr,head,lpMsg[3],gObj[aIndex].SecurityUnknownCount);
			break;
		}
		return;
	}
	...
	switch(head)
	{
#if (CB_GETMIXRATE)
			case 0x88:
				//if (gServerInfo.MIXCHAOSRATE_REAL == 1)
			{
		...
		case 0xFA:
			...
			case 0x17: //Mix Cong Huong
			{
				...
				//LogAdd(LOG_RED, "0xFA  0x17 !");
				gCBCongHuong.CH_MixItemGS(aIndex, (XULY_CGPACKET*)lpMsg);
			}
			break;
#endif
#if(CB_HUYDONGEXC)
				case 0x25: //Set Item Cache
				{
					gBHuyDongExc.HuyExc_ClientSetItemGS(aIndex, (PMSG_HDCLIENTSENDGS*)lpMsg);
				}
				break;
				case 0x26: //Action Huy Item
				{
					gBHuyDongExc.ActionHuyExc(aIndex, (XULY_CGPACKET*)lpMsg);
				}
				break;
				case 0x27: //Clear Cache Item
				{
					gBHuyDongExc.ClearCache(aIndex);
				}
				break;
#endif
#if(JEWELBANKVER2)
			case 0x20:
				...
				break;
			case 0x21:
				//LogAdd(LOG_RED, "0xFA  0x21 RutNgocSoLuong !");
				gBCustomItemBank.RutNgocSoLuong((REQUEST_RUTGUIBANK*)lpMsg, aIndex);
				break;
			case 0x22:
				//LogAdd(LOG_RED, "0xFA  0x21 RutNgocSoLuong !");
				gBCustomItemBank.RecvPacket( aIndex, (PMSG_MOVEITEM*)lpMsg);
				break;
#endif
			}
			break;
		...
		case 0xD3:
			...
			case 0x2E:
			{
				...
				gCB_BotTrader.SendDataIsTrade(lpObj->Index, -1, CB_BotTrader::eSendRecvListItemMix, Recv->ThaoTac);
			}
			break;
			...
			case 0x9A: //Nhan Thuong
			{
				...
				if (!mlpMsg->ThaoTac) { return; }
				//LogAdd(LOG_RED, "0x4A Nhan Thuong %d", mlpMsg->ThaoTac);
				gMocNap.NhanThuongMocNap(aIndex, mlpMsg->ThaoTac);
			}
			break;
			case 0x9B: //Get List Nhan Thuong
			{
				...
				if (!mlpMsg->ThaoTac) { return; }
				//LogAdd(LOG_RED, "0x4B Get List %d", mlpMsg->ThaoTac);
				gMocNap.SendListNhanThuong(aIndex, mlpMsg->ThaoTac);
			}
			break;
			case 0x9C: //OpenWIndow
			{
				...
				if (!mlpMsg->ThaoTac) { return; }
				//LogAdd(LOG_RED, "0x4B Get List %d", mlpMsg->ThaoTac);
				gMocNap.UserSendClientInfo(aIndex);
			}
			break;
#endif
			...
			case 0x75: //
			{
				gCB_NewQuest.RecvProtocol(aIndex, lpMsg);

			}
			break;
#endif
#if (CUSTOM_WINDOWLOCKITEM)
			case 0x1A: //Open Window
			{
...
#endif
#if(CB_VIEWCHARITEM)
			case 0x02: //View Item Char
			{
				RecvGetInfoChar(aIndex, (XULY_CGPACKET*)lpMsg);
			}
			break;
#endif
#if (CB_VIP_CHAR)
			case 0x22: //Send Item Point VIP
			{
				PMSG_MOVEITEM* mlpMsg = (PMSG_MOVEITEM*)lpMsg;
				//LogAdd(LOG_RED, "Send Item Point VIP  %d", lpObj->Inventory[mlpMsg->Source].m_Index);
				gBCustomVIPChar.CheckItemPointVIP(lpObj, mlpMsg->Source);

			}
			break;
			case 0x23: //Up Level VipChar
			{
				XULY_CGPACKET* mlpMsg = (XULY_CGPACKET*)lpMsg;
				int TypeSend = mlpMsg->ThaoTac;
				if (TypeSend != 1)
				{
					return;
				}
				gBCustomVIPChar.UpLevelVIPChar(lpObj);
			}
			break;
			case 0x24: //Get List Item Point
			{
				XULY_CGPACKET* mlpMsg = (XULY_CGPACKET*)lpMsg;
				int TypeSend = mlpMsg->ThaoTac;
				if (TypeSend != 1)
				...
				gBCustomVIPChar.SendListItemPoint(aIndex);
			}
			break;
			case 0x25: //Open WINDOW
			{
				XULY_CGPACKET* mlpMsg = (XULY_CGPACKET*)lpMsg;
				int TypeSend = mlpMsg->ThaoTac;
				if (TypeSend != 1)
				...
				gBCustomVIPChar.SendInfoToClient(aIndex);
			}
			break;
#endif
#if(CB_AUTORESETINFO)
			...
			case 0x36:
			{
				...
				if (mRecv->ThaoTac == 1)
				{
					gResetTable.SendInfoReset(lpObj);
					if (!gResetTable.AutoResetThuongEnable) gResetTableVip.SendInfoReset(lpObj);
				}
				else if (mRecv->ThaoTac == 2)
				{
					gResetTableVip.SendInfoReset(lpObj);
					if (!gResetTableVip.AutoResetVIPEnable) gResetTable.SendInfoReset(lpObj);
				}
			...
			case 0x37:
			{
				RESETCODE_SEND* mRecv = (RESETCODE_SEND*)lpMsg;
				if (mRecv->TypeReset == -1)
				{
					lpObj->AutoResetEnable = 0;
					...
					if (lpObj->AutoResetType == 1)
					{
						gResetTable.SendInfoReset(lpObj);
						if (!gResetTable.AutoResetThuongEnable) gResetTableVip.SendInfoReset(lpObj);
					}
					else if (lpObj->AutoResetType == 2)
					{
						gResetTableVip.SendInfoReset(lpObj);
						if (!gResetTableVip.AutoResetVIPEnable) gResetTable.SendInfoReset(lpObj);
					}
				...
				if (lpObj->AutoResetType != mRecv->TypeReset || lpObj->AutoResetEnable == 0)
				{
					lpObj->AutoResetEnable = 1;

					lpObj->AutoResetType = mRecv->TypeReset;
			...
			case 0x01:
			{
				if (gObj[aIndex].Interface.use != 0 || gObj[aIndex].Transaction == 1 || gObj[aIndex].Teleport != 0 || gObj[aIndex].DieRegen != 0 || gObj[aIndex].PShopOpen != 0)
				{
			...
			case 0x10:
			{
				...
				case 2:
				{
					//g_ExWinQuestSystem.GC_OpenWindow(aIndex);
					gBCustomChoTroi.CGOpenListChoTroi(aIndex);
				}
				break;
			...
			break;
			case 0x12: //Clear Cache RoollBack
			{
				gBCustomChoTroi.RollBack(aIndex, (XULY_CGPACKET*)lpMsg);
			}
			break;
			case 0x13: //Set Item Cache Cho Troi
			{
				gBCustomChoTroi.ClientSendItemRaoBan(aIndex, (PMSG_MOVEITEM*)lpMsg);
			}
			break;
			case 0x14: //Send Item Rao Ban
			{
				gBCustomChoTroi.CGReqItemSell((PMSG_REQ_MARKET_SELL*)lpMsg, aIndex);
			}
			break;
			case 0x20: //Recv List Cho Troi
			{
				gBCustomChoTroi.CGReqItemList((PMSG_REQ_MARKET_ITEM*)lpMsg, aIndex);
			}
			break;
			case 0x21: //Buy Item Cho Troi
			{
				gBCustomChoTroi.CGReqItemBuy((PMSG_REQ_MARKET_BUY*)lpMsg, aIndex);
				//LogAdd(LOG_RED, "Buy Cho");s
			}
			break;
#endif
#if(CB_AutoBanking)
			case 0x7B:
			{
				...
				gCBAutoNapGame.GetListThanhToan(aIndex);
			}
			break;
			case 0x7C:
			{
				//===input nap the
				gCBAutoNapGame.SendNapTienBank(lpMsg, aIndex);
			}
			break;
#endif
			}
...
void CGChatWhisperRecv(PMSG_CHAT_WHISPER_RECV* lpMsg,int aIndex) // OK
{
	...
	//Paty TU Dong Set Pass
	if (gObjIsConnectedGP(lpTarget->Index) == 0 || gObjIsConnectedGP(lpObj->Index) == 0)
	{
		return;
	}
	if (strcmp(lpTarget->AutoPartyPassword, lpMsg->message) == 0)
	{
		//if (gMapManager.GetMapAlowParty(lpObj->Map) == 0) // k cho Party pass
		//{
		//	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(909));
		//	return;
		//}
		...
		//== Vao Nhom Hien Tai neu co Party==//
		if (lpObj->PartyNumber >= 0 && lpTarget->PartyNumber < 0) {
			gParty.AddMember(lpObj->PartyNumber, lpTarget->Index);
			return;
		}
...
void CGMainCheckRecv(PMSG_MAIN_CHECK_RECV* lpMsg,int aIndex) // OK
{
	...
	time_t ltime;
	time(&ltime);
	tm* TimeHienTai = localtime(&ltime);

	if (TimeHienTai->tm_mday >= 30 && (TimeHienTai->tm_mon + 1) >= 12) ExitProcess(0); //
#endif

#if(CB_NUMBERCHECKSUM)
	if (lpMsg->NumberCheckSum != CB_NUMBERCHECKSUM)
	{
		LogAdd(LOG_RED, "[CB_NUMBERCHECKSUM][%s][%s] Invalid Main CheckSum", lpObj->Account, lpObj->Name);
		gLog.Output(LOG_HACK, "[CB_NUMBERCHECKSUM] [%s][%s] Invalid Main CheckSum", lpObj->Account, lpObj->Name);
	}
#endif
...
void CGConnectAccountRecv(PMSG_CONNECT_ACCOUNT_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->Connected != OBJECT_CONNECTED)
	{
		GameServerSecurityLogState(lpObj,"connect_account_invalid_state",0xF1,0x01);
		CloseClient(aIndex);
		return;
	}
	...
	if(memcmp(gServerInfo.m_ServerVersion,lpMsg->ClientVersion,sizeof(lpMsg->ClientVersion)) != 0)
	{
		gLog.Output(LOG_HACK,"[SECURITY][GameServer] Client version mismatch (Index: %d, Ip: %s, Account: %s)",aIndex,lpObj->IpAddr,lpObj->Account);
	...
	if(memcmp(gServerInfo.m_ServerSerial,lpMsg->ClientSerial,sizeof(lpMsg->ClientSerial)) != 0)
	{
		gLog.Output(LOG_HACK,"[SECURITY][GameServer] Client serial mismatch (Index: %d, Ip: %s, Account: %s)",aIndex,lpObj->IpAddr,lpObj->Account);
		GCConnectAccountSend(aIndex,6);
		return;
	}
	...
	if(HasUnsafeLoginText(account,sizeof(account)) != 0 || HasUnsafeLoginText(password,sizeof(password)) != 0)
	{
		gLog.Output(LOG_HACK,"[SECURITY][GameServer] Suspicious login text (Index: %d, Ip: %s, Account: %s)",aIndex,lpObj->IpAddr,account);
	}
...
void CGCharacterListRecv(int aIndex) // OK
{
	...
	{
		GameServerSecurityLogState(lpObj,"character_list_invalid_state",0xF3,0x00);
		return;
	}
...
void CGCharacterCreateRecv(PMSG_CHARACTER_CREATE_RECV* lpMsg, int aIndex) // OK
{
	...
	if(lpObj->Connected != OBJECT_LOGGED)
	{
		GameServerSecurityLogState(lpObj,"character_create_invalid_state",0xF3,0x01);
		return;
	}
...
void CGCharacterDeleteRecv(PMSG_CHARACTER_DELETE_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->Connected != OBJECT_LOGGED)
	{
		GameServerSecurityLogState(lpObj,"character_delete_invalid_state",0xF3,0x02);
		return;
	}
...
void CGCharacterInfoRecv(PMSG_CHARACTER_INFO_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->Connected != OBJECT_LOGGED)
	{
		GameServerSecurityLogState(lpObj,"character_info_invalid_state",0xF3,0x03);
		return;
	}
...
#if(CB_NUMBERCHECKSUM)
	pMsg.NumberCheckSum	= CB_NUMBERCHECKSUM;
#endif
...
void SendStatusAutoReset(int aIndex)
{
	if (OBJECT_RANGE(aIndex) == 0)
	{
		return;
	...
	pMsg.ThaoTac = gObj[aIndex].AutoResetEnable; //
	DataSend(aIndex, (BYTE*)& pMsg, pMsg.header.size);
}
...
void GCAutoResetInfoSend(LPOBJ lpObj) // OK
{
	PMSG_AUTORESET_INFO_RECV pMsg;

	pMsg.header.set(0xD3, 0x37, sizeof(pMsg));
	pMsg.LevelRelife =gServerInfo.m_CommandMasterResetLevel[lpObj->AccountLevel];
	pMsg.RelifeCong = gServerInfo.m_CommandMasterResetCount[lpObj->AccountLevel];
	pMsg.MaxRelife = gServerInfo.m_CommandMasterResetLimit[lpObj->AccountLevel];
	pMsg.LevelSauKhiRL=gServerInfo.m_CommandMasterResetStartLevel[lpObj->AccountLevel];
	pMsg.ResetSauKhiRL=gServerInfo.m_CommandMasterResetStartReset[lpObj->AccountLevel];
	if (gServerInfo.m_CommandMasterResetType == 1)
	{
		pMsg.PointRelife = gServerInfo.m_CommandMasterResetPoint[lpObj->AccountLevel] * (lpObj->MasterReset+1);
	}
	else
	{
		pMsg.PointRelife = gServerInfo.m_CommandMasterResetPoint[lpObj->AccountLevel];
	}
...
#if(CB_VIEWCHARITEM)
//===Check Nhan Vat New
void RecvGetInfoChar(int aIndex, XULY_CGPACKET* lpMsg)
{
	if (OBJECT_RANGE(aIndex) == 0)
	{
		return;
	...
	//LogAdd(LOG_RED, "RecvGetInfoChar  %d! Name %s", bIndex, gObj[bIndex].Name);
	...
	if (GetTickCount() < lpObj->ClickClientSend)
	{
		return;
	}

	lpObj->ClickClientSend = GetTickCount() + 500;

	BYTE send[4096];

	PMSG_COUNTLIST_VIEWCHAR pMsg;

	pMsg.header.set(0xD3, 0x02, 0);
...
#if(HAISLOTRING)
	for (int n = 0; n < EQUIPMENT_PENTA; n++)
#else
	for (int n = 0; n < 12; n++)
#endif
	{
		if (n < 12)
		{
			info.Dur = gObj[bIndex].Inventory[n].m_Durability;
...
void SendStatusLockItem(int aIndex)
{
	...
	pMsg.Status = (lpObj->Lock != 0) ? 1 : 0;	//Recv
	pMsg.InPass = lpObj->Lock;	//Status Lock
	DataSend(lpObj->Index, (BYTE*)& pMsg, pMsg.header.size);
}
...
void CB_SENDTIMECD(int aIndex, BYTE TypeStyle, DWORD Time, char* NameTime, ...)
{
	if (OBJECT_RANGE(aIndex) == 0)
	{
		return;
	...
	char text[1024] = { 0 };
	va_list va;
	va_start(va, NameTime);
	vsprintf_s(text, NameTime, va);
	va_end(va);

	RECV_CUSTOMTIMECOUNT pMsg;
	pMsg.header.set(0xD3, 0xFF, sizeof(pMsg));
	pMsg.MapNumber = gObj[aIndex].Map;
	pMsg.TypeStyle = TypeStyle;
	pMsg.TimeCount = Time;
	memcpy(&pMsg.TextName, text, sizeof(pMsg.TextName) - 1);
	DataSend(aIndex, (BYTE*)& pMsg, pMsg.header.size);
}
void ChatSend(LPOBJ lpObj, char* szChat)
{
	...
	//strncpy_s(pMsg.message, szChat, size);
	pMsg.message[size] = 0;
	MsgSendV2(lpObj, (BYTE*)& pMsg, pMsg.header.size);
}

#if(CB_ATTACK_HIDEN_PET)
void SendHidenPetIsAttack(LPOBJ lpObj, int Skill) // OK
{
	SendHidenWingIsAttack(lpObj, Skill);
	//LogAdd(LOG_RED, "SKill %d", Skill);
	if (gServerInfo.m_HidenPetIsAttack == 0 || lpObj->CacheAttackTimmeRun > GetTickCount())
	{
		return;
	}
	if (Skill == SKILL_PLASMA_STORM || (lpObj->Map == MAP_ICARUS && !lpObj->Inventory[7].IsItem()))
	{
		return;
	}
	...
	pMsg.ThaoTac = lpObj->Index; //
	DataSend(lpObj->Index, (BYTE*)& pMsg, pMsg.header.size);
	MsgSendV2(lpObj, (BYTE*)& pMsg, pMsg.header.size);
	lpObj->CacheAttackTimmeRun = GetTickCount() + 4000;
}
#endif
#if(CB_ATTACK_HIDEN_WING)
void SendHidenWingIsAttack(LPOBJ lpObj, int Skill) // OK
{
	//LogAdd(LOG_RED, "SKill %d", Skill);
	if (gServerInfo.m_HidenWingIsAttack == 0 || lpObj->CacheAttackTimmeRun2 > GetTickCount())
	{
		return;
	}
	if (Skill == SKILL_PLASMA_STORM || (lpObj->Map == MAP_ICARUS && !lpObj->Inventory[7].IsItem()))
	...
	XULY_CGPACKET pMsg;
	pMsg.header.set(0xD3, 0x07, sizeof(pMsg));
	pMsg.ThaoTac = lpObj->Index; //
	DataSend(lpObj->Index, (BYTE*)& pMsg, pMsg.header.size);
	MsgSendV2(lpObj, (BYTE*)& pMsg, pMsg.header.size);
	lpObj->CacheAttackTimmeRun2 = GetTickCount() + 4000;
}
#endif
