...
void DataServerProtocolCore(BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0xD9:
			...
			{
#if (JEWELBANKVER2)
			case 0x18://=== Custom Item Bank Nhan KQ TU DS
		...
		case 0xFE:
		{
			...
			case 0x00:
				gBCustomChoTroi.DGAnsItemList((CBCUSTOM_LOAD_COUNT*)lpMsg); //Nhan KQ TU DS
				break;
			case 0x02:
				gBCustomChoTroi.DGAnsItemBuy((SDHP_ANS_MARKET_BUY*)lpMsg);
				break;
			case 0x03: //Cong Coin
				gBCustomChoTroi.DGAnsItemStatus((CBCUSTOM_LOAD_COUNT*)lpMsg);
				break;
...
#if(CB_AutoBanking)
			case 0x7A:
				gCBAutoNapGame.RecvConfigNapThe(lpMsg);
				break;
			case 0x7B:
				gCBAutoNapGame.RecvListThanhToan(lpMsg);
				break;
#endif
			}
			break;
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
 ...
 //DATCheckMap 	Check nếu thoát ra vào lại thì đá về Fate yêu cầu!
	LPOBJ lpObj = &gObj[lpMsg->index];

	if ((lpObj->Map == 88 || lpObj->Map == 98 || lpObj->Map == 105 ||lpObj->Map == 36))
	{
...
#if(MOCNAP)
	gMocNap.UserSendClientInfo(lpObj->Index);
#endif
	...
	gLog.Output(LOG_ITEM_CHARACTER, "[CHECKITEM] ON Game: Account:[%s]- Name[%s] ", lpObj->Account, lpObj->Name);
	for (int n = 0; n < INVENTORY_SIZE; n++)
	{
		if (lpObj->Inventory[n].IsItem() != 0)
		{

			gLog.Output(LOG_ITEM_CHARACTER, "[Item] - [%s +%d] - DB[%.0f] TC[%d] Luck[%d] Op[%d] Exc[%d] Set[%d] oEx[%d] Har[%d] Socket[%03d,%03d,%03d,%03d,%03d][%d] Seri[%08X]",
				gItemManager.GetItemName(lpObj->Inventory[n].m_Index),
				lpObj->Inventory[n].m_Level,
				lpObj->Inventory[n].m_Durability,
				lpObj->Inventory[n].m_Option1,
				lpObj->Inventory[n].m_Option2,
				lpObj->Inventory[n].m_Option3,
				lpObj->Inventory[n].m_NewOption,
				lpObj->Inventory[n].m_SetOption,
				lpObj->Inventory[n].m_ItemOptionEx,
				lpObj->Inventory[n].m_JewelOfHarmonyOption,
				lpObj->Inventory[n].m_SocketOption[0],
				lpObj->Inventory[n].m_SocketOption[1],
				lpObj->Inventory[n].m_SocketOption[2],
				lpObj->Inventory[n].m_SocketOption[3],
				lpObj->Inventory[n].m_SocketOption[4],
				lpObj->Inventory[n].m_SocketOptionBonus,
				lpObj->Inventory[n].m_Serial);
		}

	}
	gLog.Output(LOG_ITEM_CHARACTER, "[CHECKITEM]ON Game [%s][%s]", lpObj->Account, lpObj->Name);

	LogAddConnect(LOG_BLUE, "[%d] LogInCharacter (%s)", lpObj->Index, lpObj->Name);
	//======================================================================================
//FakeOnline_EMU
...
void SendThongTinSauKhiVaoGame(int aIndex)
{
#if(CB_ActiveInvasionsD)
	gCB_ActiveInvasions.send_list_to_client();
#endif
#if(CB_NEWQUESTFF)
	if (gObj[aIndex].NQuest_Status != CB_NewQuest::eNone) 
	{
		gCB_NewQuest.SendInfoNQuest(aIndex, gObj[aIndex].NQuest_LV);
		gCB_NewQuest.SendInfoMonterProc(aIndex);
	}
#endif
#if (JEWELBANKVER2)
	//===Get KQ Bank
	gBCustomItemBank.GSSendDSGetData(aIndex);
#endif
#if (CUSTOM_WINDOWLOCKITEM)
	SendStatusLockItem(aIndex);
#endif
	//==Status AutoReset
	if (gObj[aIndex].AutoResetEnable)
	{
		SendStatusAutoReset(aIndex);
	}
	if (gServerInfo.m_FlyingDragonsSwitch == 1)
	{
		//Dragones Meter al loguear con personaje
		gDragonMaps.FlyingDragonsCheck(gObj[aIndex].Map, gObj[aIndex].Index);
	}
#if	BsvEvent
	if (gObj[aIndex].Map == gBsVEvent.EventMap)
	{
...
void DGCreateItemRecv(SDHP_CREATE_ITEM_RECV* lpMsg) // OK
{
	...
	{
		if (GetRandomItemDropLocation(lpMsg->Map, &px, &py, 2, 2, 10) == 0)
		{
			px = lpObj->X;
			py = lpObj->Y;
		}
	...
	lpMsg->Y = py;
...
#if LAGBOSS
			gLog.Output(LOG_ITEMBAGDROP, "[OK][ItemDrop] NV[%s] Map ([%d](%d/%d) )Drop Item: [%s], Serial: %08X, level: %d, Skill: %d, Luck %d, Option3: %d, NewOption: %d, SetOption: %d, JOH: %d, OptionEx: %d, SocketOption:%d,%d,%d,%d,%d \n===============================", lpObj->Name, lpMsg->Map, lpMsg->X, lpMsg->Y, gItemManager.GetItemName(lpMsg->ItemIndex), lpMsg->Serial, lpMsg->Level, lpMsg->Option1, lpMsg->Option2,
		...
		{
			gLog.Output(LOG_ITEMBAGDROP, "[Error]\n===============================[ItemDrop] NV[%s] Map ([%d](%d/%d))Khong Drop Duoc Item Ten [%s], Serial: %08X, level: %d, Skill: %d, Luck %d, Option3: %d, NewOption: %d, SetOption: %d, JOH: %d, OptionEx: %d, SocketOption:%d,%d,%d,%d,%d \n===============================", lpObj->Name, lpMsg->Map, lpMsg->X, lpMsg->Y, gItemManager.GetItemName(lpMsg->ItemIndex), lpMsg->Serial, lpMsg->Level, lpMsg->Option1, lpMsg->Option2,
				lpMsg->Option3, lpMsg->NewOption, lpMsg->SetOption, lpMsg->JewelOfHarmonyOption, lpMsg->ItemOptionEx, lpMsg->SocketOption[1], lpMsg->SocketOption[2], lpMsg->SocketOption[3], lpMsg->SocketOption[4], lpMsg->SocketOption[5]);
...
#if(JEWELBANKVER2)
	gBCustomItemBank.GSSendDSSaveDataUser(aIndex);
#endif
...
#endif
#if(CB_VIP_CHAR)
	pMsg.LvVIPChar = lpObj->LvVIPChar;
	pMsg.PointVIPChar = lpObj->PointVIPChar;
#endif
#if(CB_NEWQUESTFF)
	pMsg.NQuest_Status = lpObj->NQuest_Status;
	pMsg.NQuest_LV = lpObj->NQuest_LV;
	pMsg.NQuest_C[0] = lpObj->NQuest_C[0];
	pMsg.NQuest_C[1] = lpObj->NQuest_C[1];
	pMsg.NQuest_C[2] = lpObj->NQuest_C[2];
	pMsg.NQuest_C[3] = lpObj->NQuest_C[3];
	pMsg.NQuest_C[4] = lpObj->NQuest_C[4];
#endif
#if(MOCNAP)
...
//==Ham Mo Rong set cong tru jewel item cac loai
void GDSetCustomCoin(int aIndex, int Type, int value1)
{
	LPOBJ lpObj = &gObj[aIndex];
	switch (Type)
	{
	...
	case JEWELRY_LIFE:
		break;
	case JEWELRY_CREATION:
		break;
	case JEWELRY_STONE:
		break;
	case JEWELRY_GUARDYAN:
		break;
	case JEWELRY_HARMONY:
		break;
	case JEWELRY_LOWER:
		break;
	case JEWELRY_HIGHER:
...
//
//
//	if (CheckCoinBBT(lpObj->Coin1 + value1) || CheckCoinBBT(lpObj->Coin2 + value2) || CheckCoinBBT(lpObj->Coin3 + value3))
//	{
//		LogAdd(LOG_RED, "[MaxCoin] GDSetCoinSend -> %s %d,%d,%d", LogName, value1, value2, value3);
//		gLog.Output(LOG_DEBUG, "[MaxCoin] GDSetCoinSend -> %s %d,%d,%d", LogName, value1, value2, value3);
//		return;
//	}
//
...
//		}
//#if (FIXBUGCOIN)
//		lpObj->Coin1 = CheckCoinBB(lpObj->Coin1);
//		lpObj->Coin2 = CheckCoinBB(lpObj->Coin2);
//		lpObj->Coin3 = CheckCoinBB(lpObj->Coin3);
//#endif
//		#if(GAMESERVER_UPDATE<=402) 
//				gPcPoint.GDPcPointPointSend(lpObj->Index);
//		#endif
...
// Hàm định dạng số có dấu '.'
std::string FormatNumber(int num) {
	std::ostringstream oss;
	oss.imbue(std::locale("")); // Định dạng số có dấu '.'
	oss << std::fixed << num;
	return oss.str();
}

// Hàm ghi log thay đổi WCoin trước và sau khi thay đổi
void LogWCoinChange(LPOBJ lpObj, const char* logName, int value, int oldCoin, int newCoin, const char* coinType, int minLogValue) {
	if (value >= minLogValue || value <= -minLogValue) {
...
void GDSetCoinSend(int aIndex, int value1, int value2, int value3, char* LogName) {
	...
	// Lưu giá trị WCoin cũ trước khi thay đổi
	int oldCoin1 = lpObj->Coin1;
	int oldCoin2 = lpObj->Coin2;
	int oldCoin3 = lpObj->Coin3;

	// Cập nhật số lượng WCoin
	lpObj->Coin1 += value1;
	lpObj->Coin2 += value2;
	lpObj->Coin3 += value3;
	...
	// Gửi thông báo thay đổi WCoin đến người chơi
	if (value1 != 0) gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(value1 > 0 ? 730 : 760), value1);
	if (value2 != 0) gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(value2 > 0 ? 731 : 761), value2);
	if (value3 != 0) gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(value3 > 0 ? 732 : 762), value3);

	// Gửi dữ liệu cập nhật đến DataServer
	SDHP_SETCOIN_SEND pMsg;
	pMsg.header.set(0xF3, sizeof(pMsg));
	pMsg.index = aIndex;
	...
	// Ghi log cảnh báo nếu có giao dịch lớn hoặc trừ WCoin
	if (value1 >= 100 || value1 < 0) {
		LogAdd(LOG_ALERT, "[%s][%s] WZ_SetCoin(%s) WC: %d, WP: %d, WG: %d",
			lpObj->Account, lpObj->Name, LogName, value1, value2, value3);
	}

	// Ghi log thay đổi WCoin
	LogWCoinChange(lpObj, LogName, value1, oldCoin1, lpObj->Coin1, "WC", 100);
	LogWCoinChange(lpObj, LogName, value2, oldCoin2, lpObj->Coin2, "WP", 1);
	LogWCoinChange(lpObj, LogName, value3, oldCoin3, lpObj->Coin3, "WG", 1);
