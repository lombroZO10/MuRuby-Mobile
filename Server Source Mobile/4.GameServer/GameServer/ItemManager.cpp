...
void CItemManager::CGItemGetRecv(PMSG_ITEM_GET_RECV* lpMsg,int aIndex) // OK
{
	...
	if (gServerInfo.m_PickUpSwitch == 2)
	{
		DWORD PK_ColorMessPick = 0x0;
		sscanf_s(gServerInfo.PK_ColorMessPick, "%x", &PK_ColorMessPick);
		gNotice.GCNoticeSendToAll(eMessagePK, 0, 0, gServerInfo.PK_DelayMess, PK_ColorMessPick, 0, gMessage.GetMessage(1112), lpObj->Name, gItemManager.GetItemName(lpItem->m_Index), gMapManager.GetMapName(lpObj->Map)); //
		//gNotice.GCNoticeSendToAll(eMessagePK, 0, 0, gServerInfo.PK_DelayMess, PK_ColorMessPick, 0, gMessage.GetMessage(516), lpObj->Name, gItemManager.GetItemName(lpItem->m_Index), gMapManager.GetMapName(lpObj->Map)); //
	}
	...
	{
		if (gBCustomItemBank.CheckItemBankAutoPick(aIndex, gBCustomItemBank.mListItemBank[GetSlotItemBank].ItemIndex, gBCustomItemBank.mListItemBank[GetSlotItemBank].ItemLevel))
		{
			int SL = gBCustomItemBank.GetCountItemSlot(GetSlotItemBank, lpItem->m_Index, lpItem->m_Level);
			if (gBCustomItemBank.CongTruBank(aIndex, gBCustomItemBank.mListItemBank[GetSlotItemBank].ItemIndex, gBCustomItemBank.mListItemBank[GetSlotItemBank].ItemLevel, +(SL), FALSE))
			{
				lpItem->Clear();
				pMsg.result = -1;
	...
	gCustomNoticeDrop.NoticeItem(lpObj, lpItem);
...
void CItemManager::CGItemMoveRecv(PMSG_ITEM_MOVE_RECV* lpMsg,int aIndex) // OK
{
	...
	//LogAdd(LOG_RED, "CGItemMoveRecv SourceFlag %d Slot %d -> TargetFlag %d Slot %d", lpMsg->SourceFlag, lpMsg->SourceSlot, lpMsg->TargetFlag, lpMsg->TargetSlot);
	...
	else if (lpMsg->SourceFlag == 0 && lpMsg->TargetFlag == 40) // Inventory -> Inventory Equip
	{
		...
		//LogAdd(LOG_RED, "pMsg.result %d", pMsg.result);
	}
	...
	//===BCustomMix Check Item Inv -> Chaos Box
	if (lpObj->Interface.use != 0 && lpObj->Interface.type == INTERFACE_CHAOS_BOX && pMsg.result != 0xFF)
	{
		gCustomMix.GetSendInfoMix(lpObj);
	}
...
void CItemManager::CGMoveItemProc(PMSG_MOVEITEM* aRecv, short aIndex)
{
	...
	if ((aRecv->tFlag == 3 || aRecv->tFlag == 5|| aRecv->tFlag == 6|| aRecv->tFlag == 7|| aRecv->tFlag == 8 || aRecv->tFlag == 11 || aRecv->tFlag == 13|| aRecv->tFlag == 20) && aRecv->sFlag == 0) //move Inv -> ChaosBox
	{
		if (!lpObj->Inventory[aRecv->Source].IsItem()  || lpObj->IsChaosMixCompleted == 1)
		{
			LogAdd(LOG_RED, "CGMoveItemProc Error 1");
			return; //no item
		}
	...
	else if (aRecv->tFlag == 0 && (aRecv->sFlag == 3 || aRecv->sFlag == 5|| aRecv->sFlag == 6|| aRecv->sFlag == 7|| aRecv->sFlag == 8 || aRecv->sFlag == 11 || aRecv->sFlag == 13)) //move ChaosBox -> Inv
	{
	...
	else if (((aRecv->tFlag == 3|| aRecv->tFlag == 5|| aRecv->tFlag == 6|| aRecv->tFlag == 7|| aRecv->tFlag == 8|| aRecv->tFlag == 11 || aRecv->tFlag == 13) && aRecv->sFlag == 0) || (aRecv->tFlag == 0 && aRecv->sFlag == 0))
	{
		if (!lpObj->Inventory[aRecv->Source].IsItem())
		{
...
int CItemManager::RepairItemEquip(LPOBJ lpObj, CItem* lpItem, int slot, int type)
{
	...
	// fix slot pet 
	if (lpItem->m_Slot == 8 && lpItem->m_Index != GET_ITEM(13, 4) || lpItem->m_Slot == 12)
	{
		return 0;
	}
...
BYTE CItemManager::MoveItemToEquipFromInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) //ZEUS UP20
{
	if (INVENTORY_FULL_RANGE(SourceSlot) == 0)
	{
		//LogAdd(LOG_RED, "MoveItemToEquipFromInventory 1");
		return 0xFF;
	}

	if (INVENTORY_EXT1_RANGE(SourceSlot) != 0 && lpObj->ExtInventory < 1)
	{
		//LogAdd(LOG_RED, "MoveItemToEquipFromInventory 2");
		return 0xFF;
	}

	if (INVENTORY_EXT2_RANGE(SourceSlot) != 0 && lpObj->ExtInventory < 2)
	{
		//LogAdd(LOG_RED, "MoveItemToEquipFromInventory 3");
		return 0xFF;
	}

	if (INVENTORY_EXT3_RANGE(SourceSlot) != 0 && lpObj->ExtInventory < 3)
	{
		//LogAdd(LOG_RED, "MoveItemToEquipFromInventory 4");
		return 0xFF;
	}

	if (INVENTORY_EXT4_RANGE(SourceSlot) != 0 && lpObj->ExtInventory < 4)
	{
		//LogAdd(LOG_RED, "MoveItemToEquipFromInventory 5");
		return 0xFF;
	}

	if (gObjInventorySearchSerialNumber(lpObj, lpObj->Inventory[SourceSlot].m_Serial) == 0)
	{
		//LogAdd(LOG_RED, "MoveItemToEquipFromInventory 6");
		return 0xFF;
	}

	if (gObjWarehouseSearchSerialNumber(lpObj, lpObj->Inventory[SourceSlot].m_Serial) == 0)
	{
		//LogAdd(LOG_RED, "MoveItemToEquipFromInventory 7");
		return 0xFF;
	}

	if (this->CheckItemMoveToEquip(lpObj, &lpObj->Inventory[SourceSlot], TargetSlot) == 0)
	{
		//LogAdd(LOG_RED, "MoveItemToEquipFromInventory 8");
		return 0xFF;
	}

	if (g_pNewMyInventory.PetInventoryAddItem(lpObj->Index, lpObj->Inventory[SourceSlot], TargetSlot) == 0xFF)
	{
		//LogAdd(LOG_RED, "MoveItemToEquipFromInventory 9");
		return 0xFF;
	}
