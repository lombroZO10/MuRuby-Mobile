...
{
	gLog.Output(LOG_CONNECT,"[JoinServer] connecting to %s:%d",gServerInfo.m_JoinServerAddress,gServerInfo.m_JoinServerPort);

	if(gJoinServerConnection.Connect(gServerInfo.m_JoinServerAddress,(WORD)gServerInfo.m_JoinServerPort,wMsg) == 0)
	{
		gLog.Output(LOG_CONNECT,"[JoinServer] connection failed to %s:%d",gServerInfo.m_JoinServerAddress,gServerInfo.m_JoinServerPort);
		return 0;
	}

	gLog.Output(LOG_CONNECT,"[JoinServer] connected to %s:%d",gServerInfo.m_JoinServerAddress,gServerInfo.m_JoinServerPort);
...
{
	gLog.Output(LOG_CONNECT,"[DataServer] connecting to %s:%d",gServerInfo.m_DataServerAddress,gServerInfo.m_DataServerPort);

	if(gDataServerConnection.Connect(gServerInfo.m_DataServerAddress,(WORD)gServerInfo.m_DataServerPort,wMsg) == 0)
	{
		gLog.Output(LOG_CONNECT,"[DataServer] connection failed to %s:%d",gServerInfo.m_DataServerAddress,gServerInfo.m_DataServerPort);
		return 0;
	}

	gLog.Output(LOG_CONNECT,"[DataServer] connected to %s:%d",gServerInfo.m_DataServerAddress,gServerInfo.m_DataServerPort);
