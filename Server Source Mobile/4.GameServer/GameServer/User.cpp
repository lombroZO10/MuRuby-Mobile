...
//======================================================
	...
	//== Custom Set
#if (CUSTOM_BCHOTROI || CongHuongV2)
...
#endif
	ZeroMemory(lpObj->BankJewelry, sizeof(lpObj->BankJewelry));
	lpObj->CacheSendOnlogin = 0;
	lpObj->IsFakeOnline = 0;
#if(CB_AUTORESETINFO)
	lpObj->AutoResetType = -1;
#endif
#if (CB_VIP_CHAR)
	lpObj->LvVIPChar = 0;
	lpObj->PointVIPChar = 0;
#endif
#if(CB_NEWQUESTFF)
	lpObj->NQuest_Status = 0;
	lpObj->NQuest_LV = 0;
	lpObj->NQuest_C[0] = 0;
	lpObj->NQuest_C[1] = 0;
	lpObj->NQuest_C[2] = 0;
...
#endif
	lpObj->ScoreKill = 0;
	lpObj->ScoreKillDevil = 0;
	lpObj->TongNap = 0;
	lpObj->NhanMocNap = 0;
	lpObj->IsAutoMoveRunning = 0;
	lpObj->SecurityPacketWindowTick = 0;
	lpObj->SecurityPacketWindowCount = 0;
	lpObj->SecurityLastRateLogTick = 0;
	lpObj->SecurityLastStateLogTick = 0;
	lpObj->SecurityLastUnknownLogTick = 0;
	lpObj->SecurityStateViolationCount = 0;
	lpObj->SecurityUnknownCount = 0;
	lpObj->SecurityRepeatWindowTick = 0;
	lpObj->SecurityRepeatCount = 0;
	lpObj->SecurityRepeatHead = 0xFF;
	lpObj->SecurityRepeatSub = 0xFF;
#if(CB_BXHDMG)
	lpObj->HitDmgIndexTop = -1;
	lpObj->HitDmgIndexTarget = -1;
#endif
}
...
short gObjAdd(SOCKET socket,char* IpAddress,int aIndex) // OK
{
	...
	LogAddConnect(LOG_BLUE, "AddClient - IP: %s",lpObj->IpAddr);
...
short gObjDel(int aIndex) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		...
		if(lpObj->Account[0] != 0){LogAddConnect(LOG_ALERT,"DelAccount [%s] LogOut!",lpObj->Account);}

		LogAddConnect(LOG_ALERT,"DelClient IP: %s",lpObj->IpAddr);
...
//FakeOnline_EMU
	...
	//{
	//	PostMessagePK(lpTarget->Name,gMessage.GetMessage(515),lpObj->Name, gMapManager.GetMapName(lpObj->Map));
	//}
	////Die System Monster
	//if(gServerInfo.m_DieMonsterSwitch == 1 && lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_MONSTER)
	//{
	//	PostMessagePK(lpTarget->Name,gMessage.GetMessage(516),lpObj->Name, gMapManager.GetMapName(lpObj->Map));
	//}
	...
	if (gServerInfo.m_DieUserSwitch != 0 && lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
	{
		if (gServerInfo.m_DieUserSwitch == 2)
		{
			...
			gNotice.GCNoticeSendToAll(eMessagePK, 0, 0, gServerInfo.PK_DelayMess, PVPColor, 0, gMessage.GetMessage(515), lpTarget->Name, lpObj->Name, gMapManager.GetMapName(lpObj->Map)); //
		}
		else
		{
			PostMessagePK(lpTarget->Name, gMessage.GetMessage(515), lpObj->Name, gMapManager.GetMapName(lpObj->Map));
	...
	if (gServerInfo.m_DieMonsterSwitch != 0 && lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_MONSTER)
	{
		...
		if (gServerInfo.m_DieMonsterSwitch == 2)
		{
			...
			gNotice.GCNoticeSendToAll(eMessagePK, 0, 0, gServerInfo.PK_DelayMess, PVMColor, 0, gMessage.GetMessage(516), lpTarget->Name, lpObj->Name, gMapManager.GetMapName(lpObj->Map)); //
		}
		else
		{
			PostMessagePK(lpTarget->Name, gMessage.GetMessage(516), lpObj->Name, gMapManager.GetMapName(lpObj->Map));
...
BOOL gObjMoveGate(int aIndex,int gate) // OK
{
	...
	if (lpObj->IsFakeOnline == false && lpObj->m_OfflineMode == false)
	{
		lpObj->State = OBJECT_DELCMD;
	}
...
void gObjSecondProc()
{
	...
	for(n = 0; n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Connected > OBJECT_LOGGED &&	lpObj->Type == OBJECT_USER)
		{
			...
			if(lpObj->CheckSumTime > 0 && GetTickCount() - lpObj->CheckSumTime > 5000 && lpObj->IsFakeOnline != true) //Thêm FakeOnline_EMU
			{
			...
			if(lpObj->Type == OBJECT_USER)
			{
				if(lpObj->Connected >= OBJECT_LOGGED)
				{
					if(GetTickCount() - lpObj->ConnectTickCount > 60000)
					{
						...
						if (lpObj->m_OfflineMode == 1 || lpObj->IsFakeOnline == 1) //Fake Online
						{
							lpObj->ConnectTickCount = GetTickCount();
							continue;
						}
				...
				{
					if(GetTickCount() - lpObj->ConnectTickCount > (60000*15))
					{
						CloseClient(n);
...
int GetItemCountInInventory(int aIndex, int ItemID, int ItemLevel, int ExlOpt, int Skill, int Luck, int Option)
{
	...
	{
		if (lpObj->Inventory[i].m_Index != ItemID)
		{
			continue;
		}
		if (lpObj->Inventory[i].m_Level != ItemLevel)
		{

			continue;
		}
		if (lpObj->Inventory[i].m_NewOption != ExlOpt && ExlOpt != -1)
		{

			continue;
		}
		if (lpObj->Inventory[i].m_Option1 != Skill && Skill != -1)
		{

			continue;
		}
		if (lpObj->Inventory[i].m_Option2 != Luck && Luck != -1)
		{

			continue;
		}
		if (lpObj->Inventory[i].m_Option3 != Option && Option != -1)
		{
...
//#define ITEMGET(x,y) ( (x)*MAX_SUBTYPE_ITEMS2 + (y))

int DeleteItemsCount(int aIndex, short Type, short Level, int dCount, int ExlOpt, int Skill, int Luck, int Option)
{
	if (dCount <= 0)
	{
		return 0;
	...
	for (int i = INVENTORY_WEAR_SIZE; i < INVENTORY_SIZE; i++)
	{
		if (lpObj->Inventory[i].m_Index != Type)
		{
			continue;
		}
		if (lpObj->Inventory[i].m_Level != Level)
		{
			continue;
		}
		if (lpObj->Inventory[i].m_NewOption != ExlOpt && ExlOpt != -1)
...
bool CBCustomAddMob(int aIndex, int Class, int Map, int PosX, int PosY, int Dir, DWORD RemainTime)
{
	...
	if (RemainTime)
	{
		lpMonster->MonsterDeleteTime = GetTickCount() + (RemainTime * 1000);
	}
