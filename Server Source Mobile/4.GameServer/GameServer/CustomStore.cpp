...
void CCustomStore::OnPShopBuyItemCallbackRecv(LPOBJ lpObj,LPOBJ lpTarget,DWORD slot,DWORD WCoinC,DWORD WCoinP,DWORD GoblinPoint) // OK
{
	...
	gLog.Output(LOG_WC, "[Loja Pessoal] %s/%s | Anterior: %d | Novo: %d | Recebido de: %s/%s | Preço: %d WC, WCP: %d, WCG: %d | Sim: WC %d |Também: WC %d", lpTarget->Account, lpTarget->Name, lpTarget->Coin1, lpTarget->Coin1 + PShopWCCValue, lpObj->Account, lpObj->Name, PShopWCCValue, PShopWCPValue, PShopWGPValue, lpObj->Coin1, lpObj->Coin1 - PShopWCCValue);
	gNotice.NewNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, "Você comprou %s de %s por %d WCoinC", gItemManager.GetItemName(lpTarget->Inventory[slot].m_Index), lpTarget->Name, PShopWCCValue);
	#if(GAMESERVER_UPDATE>=501)
