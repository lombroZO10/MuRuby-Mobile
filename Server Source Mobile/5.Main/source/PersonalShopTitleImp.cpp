...
{
	float Width = 640;//GetScreenWidth();
	float Height = 480;
...
void CPersonalShopTitleImp::CShopTitleDrawObj::Draw(int iPkLevel)
{
	...
	g_pRenderText->RenderText(RenderPos.x, RenderPos.y- iLineHeight, GlobalText[1104], RenderBoxSize.cx, iLineHeight,3);

	//g_pRenderText->SetBgColor(iNameBkColor);
	g_pRenderText->SetTextColor(iNameTextColor);
	g_pRenderText->RenderText(RenderPos.x, RenderPos.y, m_fullname.c_str(),
		RenderBoxSize.cx, iLineHeight,3);
	...
	if( !m_bottomTitle.empty() )
	{
		...
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, m_topTitle.c_str(), RenderBoxSize.cx, iLineHeight,3);
		RenderPos.y += iLineHeight;
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, m_bottomTitle.c_str(), RenderBoxSize.cx, iLineHeight,3);
	}
