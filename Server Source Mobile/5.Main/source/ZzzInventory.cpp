...
void RenderTipTextList(const int sx, const int sy, int TextNum, int Tab, int iSort, int iRenderPoint, BOOL bUseBG, BOOL Render3DItem)
{
	...
	//===
	if (MouseWheel < 0 && (int)(CacheY + fHeight) > 420) CacheY -= 5.f;
	else if (MouseWheel > 0 && (int)CacheY < 0) CacheY += 5.f;
	if (MouseWheel != 0) MouseWheel = 0;
	

	BlockMouseWheel = GetTickCount() + 1000;
	...
	if (bUseBG == TRUE && TextNum > 0)
	{
		...
		RenderColor ((float)iPos_x-1, (fsy - FixHRender3DItem) - 1, (float)1, (float)fHeight + 1);
		RenderColor ((float)iPos_x-1 + fWidth + 1, (float)(fsy - FixHRender3DItem) - 1, (float)1, (float)fHeight + 1);
		RenderColor ((float)iPos_x-1, (fsy - FixHRender3DItem) - 1 + fHeight + 1, (float)fWidth + 2, (float)1);

		glColor4f(0.0f, 0.0f, 0.0f, 0.8f);
		RenderColor ((float)iPos_x, (fsy - FixHRender3DItem), (float)fWidth, (float)fHeight);
...
bool IsRequireClassRenderItem(const short sType)
{
	if(
		...
		|| gCustomPet2.GetInfoPetType(sType) == 1
		|| sType == ITEM_WING+15
		|| sType == ITEM_WING+32
		|| sType == ITEM_WING+33
		|| sType == ITEM_WING+34
	...
	for(int i=0; i<MAX_CLASS; ++i)
	{
		...
		case CLASS_DARK:
			{	
				...
				else if(byRequireClass == 3|| byRequireClass == 2)
				{
	...
	for(int i = 0; i < (int)(g_pMyInventory->GetInventoryCtrl()->GetNumberOfItems()); ++i)
	{
		...
		if(pItem)
		{
			...
			if( (pItem->Type >= ITEM_HELPER && pItem->Type <= ITEM_HELPER+5) || pItem->Type == ITEM_HELPER+10  || pItem->Type == ITEM_HELPER+31)
				continue;
			if( pItem->Type == ITEM_BOW+7 || pItem->Type == ITEM_BOW+15 || pItem->Type >= ITEM_POTION ) //CustomArrow
				continue;
			...
			if(pItem->Type == ITEM_HELPER+37 || gCustomPet2.GetInfoPetType(pItem->Type) == 5)
				continue;
...
WORD calcMaxDurability(const ITEM* ip, ITEM_ATTRIBUTE* p, int Level)
{
	...
	int index = ip->Type;
	if (index == GET_ITEM(12, 144) || index == GET_ITEM(12, 146) || index >= GET_ITEM(12, 200) && index <= GET_ITEM(12, 220)) // Mithril Fragment,Elixir Fragment
	{
		return p->Durability;
	}
	...
	if (index != GET_ITEM(0, 19) && index != GET_ITEM(2, 13) && index != GET_ITEM(4, 18) && index != GET_ITEM(5, 10) && index != GET_ITEM(5, 36) && p->m_byItemSlot != 7) // Sword of Archangel,Scepter of Archangel,Crossbow of Archangel,Staff of Archangel,Stick of Archangel
	{
		if ((ip->ExtOption % 0x04) == EXT_A_SET_OPTION || (ip->ExtOption % 0x04) == EXT_B_SET_OPTION)
		{
			dur += 20;
	...
	//== Fix Dur Custom Ring Pen
	if ((p->m_byItemSlot == 10 || p->m_byItemSlot == 9) && index >= GET_ITEM(13, 115))
	{
		dur = p->Durability;
		if ((ip->ExtOption % 0x04) == EXT_A_SET_OPTION || (ip->ExtOption % 0x04) == EXT_B_SET_OPTION)
		{
...
//=== Fix Get Item Name Pick Up
void GetItemName(int iType, int iLevel, char* Text)
{
	int ItemType = iType;
	auto tooltip = m_ItemToolTipData.find(ItemType);

	if (tooltip == m_ItemToolTipData.end()) return;

	
	if (tooltip->second.ItemLevelToolTipIndex != -1) //Get Name Item Level
	{
		
		int tooltip_lvl_index = tooltip->second.ItemLevelToolTipIndex + iLevel;
		std::map<int, _ITEM_LEVEL_TOOLTIP>::iterator tooltip_level = m_ItemLevelToolTipData.find(tooltip_lvl_index);
		if (tooltip_level == m_ItemLevelToolTipData.end()) return;
		::wsprintf(Text, "%s", tooltip_level->second.name);
	}
	else
	{
		if (iLevel == 0)
		{
			::wsprintf(Text, "%s", tooltip->second.name);
		}
		else
		{
			::wsprintf(Text, "%s +%d", tooltip->second.name, iLevel);
		}
	}
}
//void GetItemName ( int iType, int iLevel, char* Text )
...
//		|| gCustomWing.CheckCustomWingByItem(iType)
//#ifdef PBG_ADD_NEWCHAR_MONK_ITEM
//		|| (iType == ITEM_WING+50)
//#endif //PBG_ADD_NEWCHAR_MONK_ITEM
//		)
...
#endif //PBG_ADD_NEWCHAR_MONK_SKILL
{
	...
	case MASTER_SKILL_ADD_UPPER_BEAST_ENHANCED:
		...
		break;
	case AT_SKILL_MUITENPHUONGHOANG:
		gSkillManager.GetSkillInformation(Option, 1, NULL, &iMana, NULL);
...
#if(USE_ITEMTOOLTIP_SS6)
int field_11BB64 = -1;
//==render_item_tooltip
BOOL pIsAbsoluteWeapon(__int16 a2)
{
	return a2 == 19 || a2 == 2066 || a2 == 2570 || a2 == 1037 || a2 == 2596;
}
BOOL pIsEnableOptionWing( __int16 a2)
{
	return a2 == 6147
		...
		|| a2 == 6194
		|| a2 == 6193
		|| (COMGEM::NOGEM != COMGEM::Check_Jewel_Com(a2, false))
		|| (COMGEM::NOGEM != COMGEM::Check_Jewel_Unit(a2, false))
		|| gCustomWing.CheckCustomWingByItem(a2)
		|| gCustomJewel.CheckCustomJewelByItem(a2)
		|| a2 == 6686;
}
...
int FixLucChongDoWing(int OptionType ,ITEM* ip)
{
	for (int i = 0; i < ip->SpecialNum; i++)
	{
		...
		//===Fix opt wing 2 DL
		if (ip->Special[i] == OptionType)
		{
			return ip->SpecialValue[i];
		}
	}
	return 0;
}
bool pSetTooltipValueText(std::string* new_text, std::string Format, int* line, int* color, int type, ITEM* item, ITEM_ATTRIBUTE* item_attr, int level)
{
	char Buffer[1024] = { 0 };
	switch (type)
	{
	case 0:
	{
		if (item->DamageMin)
		{
			...
			{
				StrengthenCapability SC;
				g_pUIJewelHarmonyinfo->GetStrengthenCapability(&SC, item, 1);
			...
			int DamageMin = item->DamageMin;
			int DamageMax = item->DamageMax;
			if (minindex + DamageMin < maxindex + DamageMax)
				wsprintf(Buffer, Format.c_str(), DamageMin + minindex, maxindex + DamageMax);
			else
				wsprintf(Buffer, Format.c_str(), maxindex + DamageMax, maxindex + DamageMax);
			...
			if (DamageMin > 0)
			{
				...
				else if ((item->Option1 & 0x3F) != 0)
				{
					*color = 1;
				}
			}
			wsprintf(Buffer, Format.c_str(), DamageMin, DamageMax);
		}
	}
	break;
	case 1:
		wsprintf(Buffer, Format.c_str(), item->Durability);
		break;
	case 2:
	{
		int maxdefense = 0;
		...
		{
			StrengthenCapability SC;
			g_pUIJewelHarmonyinfo->GetStrengthenCapability(&SC, item, 2);
		...
		if((item->Type == ITEM_WING + 49 || item->Type == ITEM_HELPER + 30))
		{
			wsprintf(Buffer, Format.c_str(), FixLucChongDoWing(AT_SET_OPTION_IMPROVE_DEFENCE,item));
		}
		else
		{
			wsprintf(Buffer, Format.c_str(), item->Defense + maxdefense);
		}
		...
		{
			if (item->Type >= ITEM_HELM && item->Type < ITEM_BOOTS + MAX_ITEM_INDEX && (item->Option1 & 63) > 0)
	...
	break;
	case 3:
		wsprintf(Buffer, Format.c_str(), item->MagicDefense);
		break;
	case 4:
		{
			int maxDurability = calcMaxDurability(item, item_attr, level);
			wsprintf(Buffer, Format.c_str(), item->Durability, maxDurability);
		}
		break;
	case 5:
		{
	...
	case 11:
		...
		wsprintf(TextList[TextNum], Format.c_str(), item->MagicPower);
	...
	case 12:
		{
			int DamageMin = item->DamageMin;
			int DamageMax = item->DamageMax;
			if (item->Type >= ITEM_ETC && item->Type < ITEM_ETC + MAX_ITEM_INDEX)
			{
				int SkillIndex = getSkillIndexByBook(item->Type);
			...
			wsprintf(Buffer, Format.c_str(), DamageMin, DamageMax);
			if (DamageMin > 0 && (item->Option1 & 0x3F) != 0)
			{
				*color = 1;
			}
		...
		break;
	case 14:
		wsprintf(Buffer, Format.c_str(), level + 1);
		break;
	case 15:
		wsprintf(Buffer, Format.c_str(), item->SuccessfulBlocking);
		if ((item->Option1 & 0x3F) != 0)
			*color = 1;
		break;
	case 16:
		wsprintf(Buffer, Format.c_str(), 10 * item->Durability);
		break;
	case 101:
		wsprintf(Buffer, Format.c_str(), item_attr->WeaponSpeed);
		break;
	case 102:
		wsprintf(Buffer, Format.c_str(), item_attr->WeaponSpeed);
		break;
	...
	break;
	case 207:
		wsprintf(Buffer, Format.c_str(), level + 32);
		break;
	case 208:
		{
			//==WingSetIncDamage
			if (gCustomWing.CheckCustomWingByItem(item->Type))
			{
				wsprintf(Buffer, Format.c_str(), gCustomWing.GetCustomWingIncDamage(item->Type, level));
			}
			else
			{
				wsprintf(Buffer, Format.c_str(), 2 * level + 39);
			}
		}
		
		break;
	case 209:
		wsprintf(Buffer, Format.c_str(), 2 * level + 20);
		break;
	case 210:
		wsprintf(Buffer, Format.c_str(), 2 * level + 12);
		break;
	case 211:
		wsprintf(Buffer, Format.c_str(), 2 * level + 25);
		break;
	case 212:
		wsprintf(Buffer, Format.c_str(), 2 * level + 24);
		break;
	case 213:
		wsprintf(Buffer, Format.c_str(), 2 * level + 10);
		break;
	case 214:
		wsprintf(Buffer, Format.c_str(), level + 10);
		break;
	case 216: //thuoc tinh
		{


			//SocketSeedID = CNewUIItemTooltip::GetSocketSeedID(this, *a7, a9);
			//sub_7E79D0((char*)& dword_986C1B8, v26, 0, SocketSeedID);
			//LOBYTE(v68) = 2;
			//v10 = sub_402880(v26);
		...
		break;
	case 218:
		wsprintf(Buffer, Format.c_str(), CharacterAttribute->Level);
		break;
	case 219:
		{
			ITEM_ADD_OPTION ItemAddOtioninfo = g_pItemAddOptioninfo->GetItemAddOtioninfo(item->Type);
			sprintf(Buffer, Format.c_str(), ItemAddOtioninfo.m_byValue1);
		}
		break;
	...
	case 220:
		{
			ITEM_ADD_OPTION ItemAddOtioninfo = g_pItemAddOptioninfo->GetItemAddOtioninfo(item->Type);
			sprintf(Buffer, Format.c_str(), ItemAddOtioninfo.m_byValue2);
		}
		break;
	case 223:
		wsprintf(Buffer, Format.c_str(), 5-item->Durability);
		break;
	case 225:
		wsprintf(Buffer, Format.c_str(), CharacterAttribute->Level /2, CharacterAttribute->Level /2);
		break;
	case 226:
		wsprintf(Buffer, Format.c_str(), CharacterAttribute->Level / 12, CharacterAttribute->Level / 25);
		break;
	}
...
void RenderItemInfo(int sx, int sy, ITEM* ip, bool Sell, int Inventype, bool bItemTextListBoxUse, bool Render3DItem)
{
	...
	//g_ConsoleDebug->Write(1, "Item Level %d %d", (int)item_level, item->Level >> 3);
	for (int i = 0; i < 60; i++)
	{
		TextList[i][0] = NULL;
	}
	auto tooltip = m_ItemToolTipData.find(ip->Type);

	if (tooltip == m_ItemToolTipData.end())
		return;
	std::map<int, _ITEM_LEVEL_TOOLTIP>::iterator tooltip_level;

	if (tooltip->second.ItemLevelToolTipIndex != -1)
	{
		if (ip->Type == GET_ITEM(13, 37))
		{
			switch (ip->Option1)
			{
			case 1:
			...
			int tooltip_lvl_index = tooltip->second.ItemLevelToolTipIndex + field_11BB64;
			tooltip_level = m_ItemLevelToolTipData.find(tooltip_lvl_index);
		}
		else
		{
			int tooltip_lvl_index = tooltip->second.ItemLevelToolTipIndex + item_level;
			tooltip_level = m_ItemLevelToolTipData.find(tooltip_lvl_index);
		}

		if (tooltip_level == m_ItemLevelToolTipData.end())
			return;
	...
	{
		if (tooltip->second.ItemLevelToolTipIndex == -1)
			color = tooltip->second.color;
		else
			color = tooltip_level->second.color;
	}
	if (!pIsEnableOptionWing(ip->Type))
	{
		if ((ip->ExtOption % 0x04) == 1 || (ip->ExtOption % 0x04) == 2)
			...
			color = TEXT_COLOR_YELLOW;
		else if (item_level < 7 && field_11BB64 == -1)
			color = tooltip->second.color;
		else
		{
			if (tooltip->second.ItemLevelToolTipIndex == -1)
	...
	if (ip->Type == ITEM_HELPER + 4 || ip->Type == ITEM_HELPER + 5)
	{
		...
		if (giPetManager::RenderPetItemInfo(sx, sy, ip, Inventype)) return;
		...
		&& !IsPersonalShopBan(ip))
	{
		{
			int price = 0;
			int indexInv = (MAX_MY_INVENTORY_EX_INDEX)+(ip->y * COL_PERSONALSHOP_INVEN) + ip->x;
			char Text[100];
			...
			if (GetPersonalItemPrice(indexInv, price, g_IsPurchaseShop))
			{
...
#if(CB_CUSTOMOFFTRADE)
				if (gCB_OffTrade) gCB_OffTrade->ShowPrice(TextNum, Text);
#else
				sprintf(TextList[TextNum], GlobalText[63], Text); //gia ban : %s
#endif
				...
				if ((int)gold < price && g_IsPurchaseShop == PSHOPWNDTYPE_PURCHASE)
				{
					TextListColor[TextNum] = TEXT_COLOR_RED;
					TextBold[TextNum] = true;
					sprintf(TextList[TextNum], GlobalText[423]); //khong du zen
					TextNum++;
					sprintf(TextList[TextNum], "\n"); TextNum++; SkipNum++;
				}
#endif
	...
	char TextName[128];
	if (!g_csItemOption.GetSetItemName(TextName, ip->Type, ip->ExtOption) || pIsAbsoluteWeapon(ip->Type))
		strcpy(TextName, tooltip->second.name);
	else
		strcat(TextName, tooltip->second.name);

	if ((ip->Option1 & 0x3F) == 0 || pIsAbsoluteWeapon(ip->Type) || pIsEnableOptionWing(ip->Type))
	{
		if (tooltip->second.ItemLevelToolTipIndex == -1)
		{
			if (item_level && tooltip->second.ShowItemLevel)
				sprintf(TextList[TextNum], "%s +%d", TextName, item_level);
			else
				sprintf(TextList[TextNum], "%s", TextName);
		}
		else
		{
			sprintf(TextList[TextNum], "%s", tooltip_level->second.name);
		}
	}
	else if (field_11BB64 == -1)
	{
		if (item_level)
			sprintf(TextList[TextNum], "%s %s +%d", GlobalText[620], TextName, item_level);
		else
	...
	if (tooltip->second.ItemLevelToolTipIndex == -1)
		TextBold[TextNum] = tooltip->second.is_bold;
	else
		TextBold[TextNum] = tooltip_level->second.is_bold;
	...
	if (ip->Type == GET_ITEM(13, 29)) //Ve CC
	{
		int startIndex = 0;
	...
	else if (ip->Type == GET_ITEM(14, 28))
	{
		TextNum = RenderHellasItemInfo(ip, TextNum);
	}
	else if (ip->Type == GET_ITEM(13, 70))
	{
		if (ip->Durability == 2)
		{
	...
	else
	{
		...
		else
		{
			if (tooltip->second.ItemLevelToolTipIndex == -1)
			{
				for (int i = 0; i < 12; i++)
				{
					auto text_index = tooltip->second.tooltip_text[i].text_index;
					auto it = m_ItemToolTipTextData.find(text_index);

					if (it == m_ItemToolTipTextData.end())
						continue;

					int text_color = tooltip->second.tooltip_text[i].color;
					int text_bold = tooltip->second.tooltip_text[i].is_bold;
					int text_type = it->second.type;
					...
					if (text_type == 215)
					{
						...
						if (tCount > 0)
						{
							...
							{
								sprintf(TextList[TextNum], (char*)text_.c_str(), tCount, GlobalText[COMGEM::GetJewelIndex(nJewelIndex, COMGEM::eGEM_NAME)]);
								TextListColor[TextNum] = TEXT_COLOR_WHITE; TextBold[TextNum] = false; TextNum++;
							}
						}
					}
					else if (text_type == 216) //fix option text khuong socket
					{
						int iSphereLevel = ip->Type - (ITEM_WING + 70) + 1;
						g_ConsoleDebug->Write(MCD_ERROR, "[%d]", iSphereLevel);
						sprintf(TextList[TextNum], (char*)text_.c_str(), GlobalText[2585 + iSphereLevel]);
						TextListColor[TextNum] = text_color; TextBold[TextNum] = text_bold; TextNum++;
					}
					else
					{
						...
						if (text_type != -1)
						{
							...
							if (!pSetTooltipValueText(&new_, text_, &TextNum, &text_color, text_type, ip, item, item_level))
							{
								continue;
							}
							v146 = 1;
							text_ = new_;
							//g_Console.AddMessage(5, "%s", new_.c_str());
						}
						TextNum = g_pNewUISystem->GetUI_NewMasterSkillTreeInterface()->SetDivideString((char*)text_.c_str(), 1, TextNum, text_color, text_bold, v146 != 0);
					}
			...
			else
			{
				for (int i = 0; i < 8; ++i)
				{
					auto text_index = tooltip_level->second.tooltip_text[i].text_index;
					auto it = m_ItemToolTipTextData.find(text_index);

					if (it == m_ItemToolTipTextData.end())
						continue;

					int text_color = tooltip_level->second.tooltip_text[i].color;
					int text_bold = tooltip_level->second.tooltip_text[i].is_bold;
					int text_type = it->second.type;
					...
					if (text_type != -1)
					{
						if (!pSetTooltipValueText(&test__, text_, &TextNum, &text_color, text_type, ip, item, item_level))
						{
							continue;
						}
					...
					TextNum = g_pNewUISystem->GetUI_NewMasterSkillTreeInterface()->SetDivideString((char*)text_.c_str(), 1, TextNum, text_color, text_bold, v139 != 0);
				}
			}
		}
	}
	...
	{
		sprintf(TextList[TextNum], "\n"); TextNum++; SkipNum++;
		sprintf(TextList[TextNum], GlobalText[1234], item->MagicPower);
		TextListColor[TextNum] = TEXT_COLOR_BLUE; TextBold[TextNum] = true; TextNum++;
	}
	if (ip->SpecialNum > 0)
	{
	...
	//==
	TextNum = GInfo.nInformationOP(TextNum, ip); //custom tool tip
		...
		if (ip->Jewel_Of_Harmony_Option != 0)
		{
			...
			if (type < SI_None)
			{
				if (g_pUIJewelHarmonyinfo->IsHarmonyJewelOption(type, ip->Jewel_Of_Harmony_Option))
				{
					...
					if (item_level >= ip->Jewel_Of_Harmony_OptionLevel) TextListColor[TextNum] = TEXT_COLOR_YELLOW;
	...
	for (int i = 0; i < ip->SpecialNum; i++)
	{
		...
		//===Fix opt wing 2 DL
		if (ip->Type ==  ITEM_WING+49 || ip->Type == ITEM_HELPER+30)
		{
			if(ip->Special[i] == AT_SET_OPTION_IMPROVE_DEFENCE) continue;
			if(ip->Special[i] == AT_SET_OPTION_IMPROVE_DAMAGE) continue;
			//sprintf(TextList[TextNum], "GetText %d -> %d", ip->Special[i],ip->SpecialValue[i]);
		...
		//gInterface.DrawMessage(1, "Item %d Socket %d Number %d <> %d", pItem->Type, pItem->SocketCount, GetNum, iTextNum);

		//char BuffT[255] = { 0 };
		//sprintf(BuffT, "Item %d Numb %d", ip->Type, i);
		//g_pChatListBox->AddText("", BuffT, SEASON3B::TYPE_ERROR_MESSAGE);
		...
		else if (ip->Special[i] == AT_SKILL_DARK_HORSE
			|| ip->Special[i] == MASTER_SKILL_ADD_EARTHQUAKE_IMPROVED
			|| ip->Special[i] == MASTER_SKILL_ADD_EARTHQUAKE_ENHANCED
			|| (AT_SKILL_ASHAKE_UP <= ip->Special[i] && ip->Special[i] <= AT_SKILL_ASHAKE_UP + 4))
		{
	...
	{
		if (bItemTextListBoxUse)
			RenderTipTextList(sx, sy, TextNum, 0, RT3_SORT_CENTER, STRP_BOTTOMCENTER, 1, Render3DItem);
		else
			RenderTipTextList(sx, sy, TextNum, 0, 3, 0, 1, Render3DItem);
	}
	if (Render3DItem)
	{
...
void RenderItemInfo(int sx,int sy,ITEM *ip,bool Sell, int Inventype, bool bItemTextListBoxUse, bool Render3DItem)
{
	...
	if(ip->Type==ITEM_POTION+13 || gCustomJewel.CheckCustomJewelByItem(ip->Type) || ip->Type==ITEM_POTION+14 || ip->Type==ITEM_WING+15 || ip->Type==ITEM_POTION+31 ||
...
//		 »ó½ÂÀÇ º¸¼®
    ...
    {
		//==WingSetIncDamage
		if (gCustomWing.CheckCustomWingByItem(ip->Type))
		{
...
void RenderRepairInfo(int sx,int sy,ITEM *ip,bool Sell)
{
	...
	if(ip->Type==ITEM_POTION+13 || gCustomJewel.CheckCustomJewelByItem(ip->Type) || ip->Type==ITEM_POTION+14 || ip->Type==ITEM_WING+15)
	{
		Color = TEXT_COLOR_YELLOW;
	}
...
bool GetAttackDamage ( int* iMinDamage, int* iMaxDamage )
{
    ...
    else if(gCharacterManager.GetBaseClass(Hero->Class) == CLASS_ELF )
    {
        ...
        {
            if ( ( l->Type==ITEM_BOW+7 && ((l->Level>>3)&15)>=1 ) || ( r->Type==ITEM_BOW+15 && ((r->Level>>3)&15)>=1 ) ) //CustomArrow
            {
                Alpha = true;
            }
        }
...
void RenderSkillInfo(int sx,int sy,int Type,int SkillNum, int iRenderPoint /*= STRP_NONE*/)
{
    ...
    if ( 
		CharacterAttribute->Skill[Type]==AT_SKILL_STRONG_PIER ||
		CharacterAttribute->Skill[Type]== MASTER_SKILL_ADD_FORCE_WAVE_IMPROVED
	...
	{
        if ( 
			CharacterAttribute->Skill[Type]==AT_SKILL_WIZARDDEFENSE ||
			CharacterAttribute->Skill[Type]== MASTER_SKILL_ADD_MANA_SHIELD_MASTERED ||
			CharacterAttribute->Skill[Type]== MASTER_SKILL_ADD_MANA_SHIELD_IMPROVED ||
			CharacterAttribute->Skill[Type]== MASTER_SKILL_ADD_MANA_SHIELD_ENHANCED
			|| (AT_SKILL_SOUL_UP <= CharacterAttribute->Skill[Type] && CharacterAttribute->Skill[Type] <= AT_SKILL_SOUL_UP+4))
        {
			int iDamageShield;
			if(CharacterAttribute->Skill[Type] == AT_SKILL_WIZARDDEFENSE
				|| (MASTER_SKILL_ADD_MANA_SHIELD_IMPROVED == CharacterAttribute->Skill[Type]) ||
				(CharacterAttribute->Skill[Type] == MASTER_SKILL_ADD_MANA_SHIELD_ENHANCED) ||
					(CharacterAttribute->Skill[Type] == MASTER_SKILL_ADD_MANA_SHIELD_MASTERED))
			{
			...
			SkillType != MASTER_SKILL_ADD_MAGIC_CIRCLE_IMPROVED &&
			SkillType != MASTER_SKILL_ADD_MAGIC_CIRCLE_ENHANCED &&
			SkillType != MASTER_SKILL_ADD_SLEEP_IMPROVED &&
			SkillType != AT_SKILL_ALICE_SLEEP	&& !(AT_SKILL_ALICE_SLEEP_UP <= SkillType && SkillType <= AT_SKILL_ALICE_SLEEP_UP + 4 ))
        {
	...
	break;
		case AT_SKILL_ATTACK :
			...
			sprintf(TextList[TextNum],GlobalText[173],Energy/7+3);break;
		case AT_SKILL_RECOVER:
			{
				int Cal = Energy/4;
    ...
    if ( gCharacterManager.GetBaseClass(Hero->Class) == CLASS_KNIGHT )
    {
		...
		if ( Hero->byExtensionSkill == 1 && CharacterAttribute->Level >= 220 )
		{
			if ( ( CharacterAttribute->Skill[Type] >= AT_SKILL_SWORD1 && CharacterAttribute->Skill[Type] <= AT_SKILL_SWORD5 ) 
				|| CharacterAttribute->Skill[Type]== MASTER_SKILL_ADD_FALLING_SLASH_IMPROVED
				|| CharacterAttribute->Skill[Type]== MASTER_SKILL_ADD_CYCLONE_IMPROVED1
				|| CharacterAttribute->Skill[Type]== MASTER_SKILL_ADD_CYCLONE_IMPROVED2
				|| CharacterAttribute->Skill[Type]== MASTER_SKILL_ADD_SLASH_IMPROVED
				|| CharacterAttribute->Skill[Type]== MASTER_SKILL_ADD_LUNGE_IMPROVED
				|| CharacterAttribute->Skill[Type]==AT_SKILL_WHEEL 
				|| CharacterAttribute->Skill[Type]==MASTER_SKILL_ADD_TWISTING_SLASH_IMPROVED1
				|| CharacterAttribute->Skill[Type]==MASTER_SKILL_ADD_TWISTING_SLASH_IMPROVED2
				|| CharacterAttribute->Skill[Type]==MASTER_SKILL_ADD_TWISTING_SLASH_ENHANCED
				   ...
				   )
			{
				...
				CharacterAttribute->Skill[Type] == AT_SKILL_BLOW_OF_DESTRUCTION
				||CharacterAttribute->Skill[Type] == MASTER_SKILL_ADD_FROZEN_STAB_IMPROVED
				||CharacterAttribute->Skill[Type] == MASTER_SKILL_ADD_FROZEN_STAB_ENHANCED
				||CharacterAttribute->Skill[Type] == MASTER_SKILL_ADD_FROZEN_STAB_MASTERED
				)
			{
	...
	if(
		CharacterAttribute->Skill[Type] == AT_SKILL_INFINITY_ARROW ||
		CharacterAttribute->Skill[Type] == MASTER_SKILL_ADD_INFINITY_ARROW_IMPROVED
		)
	{
...
void SetColorItemNameS6(int Type)
{
	switch (Type)
	{
	case TEXT_COLOR_YELLOW:  glColor3f(1.f, 0.8f, 0.1f); break;
	//case TEXT_COLOR_GREEN_BLUE:  glColor3f(0.7f, 0.4f, 1.0f); break;
	case TEXT_COLOR_VIOLET:  glColor3f(0.7f, 0.4f, 1.0f); break;
	case TEXT_COLOR_BLUE:  glColor3f(0.4f, 0.7f, 1.f); break; //xanh
	case TEXT_COLOR_GREEN:  glColor3f(0.1f, 1.f, 0.5f); break; //exl
	case TEXT_COLOR_PURPLE: glColor3f(1.f, 0.1f, 1.f); break; //thien tu kiem
	default:
		break;
	}
}

void RenderItemNameS6(int i, ITEM_t pItem, int ItemLevel, int ItemOption, int ItemExtOption, bool Sort)
{
	
	std::string NameString;
	char Name[99] = { 0 };
	NameString.clear();
	OBJECT* o = &pItem.Object;
	ITEM* ip = &pItem.Item;
	int ItemType = ip->Type;
	...
	if (tooltip->second.ItemLevelToolTipIndex != -1) //Get Name Item Level
	{
		...
		if (ip->Type == GET_ITEM(13, 37))
		{
			...
			int tooltip_lvl_index = tooltip->second.ItemLevelToolTipIndex + field_11BB64B;
			tooltip_level = m_ItemLevelToolTipData.find(tooltip_lvl_index);
		}
		else
		{
			int tooltip_lvl_index = tooltip->second.ItemLevelToolTipIndex + Level;
			tooltip_level = m_ItemLevelToolTipData.find(tooltip_lvl_index);
		}

		if (tooltip_level == m_ItemLevelToolTipData.end()) return;

		::wsprintf(Name, "%s", tooltip_level->second.name);
	}
	else
	{
		for (int i = 0; i < TOOLTIP_SIZE; ++i)
		...
		if (!pIsEnableOptionWing(ip->Type))
		{
			if ((ip->ExtOption % 0x04) == 1 || (ip->ExtOption % 0x04) == 2) //anc
				color = TEXT_COLOR_GREEN_BLUE;
			else if (g_SocketItemMgr.IsSocketItem(ip)) //socket
				color = TEXT_COLOR_VIOLET;
			else if( pIsAbsoluteWeapon(ip->Type))
				color = TEXT_COLOR_PURPLE;
			else if ((ip->Option1 & 63) > 0) //exl
				color = TEXT_COLOR_GREEN;
			else
			{
				if (tooltip->second.ItemLevelToolTipIndex == -1)
		...
		//=== Wing hoac jewel thi mau vang
		if (pIsEnableOptionWing(ItemType))
		{
			g_pRenderText->SetFont(g_hFontBold);
			color = TEXT_COLOR_YELLOW;
			//glColor3f(1.f, 0.8f, 0.1f);//vang
		...
		if (ItemType == 7183) //Zen
		{
			color = TEXT_COLOR_YELLOW;
			::wsprintf(Name, "%s %d", ItemAttribute[o->Type - MODEL_ITEM].Name, ItemLevel);
		}
		else
		{
			char TextName[128];
			...
			if (Level == 0)
			{
				::wsprintf(Name, "%s", TextName);
			}
			else
			{
				::wsprintf(Name, "%s +%d", TextName, Level);
			}

			if ((ItemLevel >> 7) & 1)
			{
				if (ItemType != ITEM_HELPER + 3)
	...
	if (!Sort)
	{
		g_pRenderText->RenderText(MouseX, MouseY - 15, NameString.c_str(), 0, 0, RT3_WRITE_CENTER);

	}
	else
	{
		g_pRenderText->RenderText(o->ScreenX, o->ScreenY - 15, NameString.c_str(), 0, 0, RT3_WRITE_CENTER);
	}
...
void ClearInventory()
{
	...
	for (int i = 0; i < 16; i++)
	{
		...
		CharacterMachine->EquipmentMuun[i].Number = 0;
	}
#endif
		...
		if(Index < 12)
		{
			...
			if(Index==EQUIPMENT_HELPER)
			{
                if ( gMapManager.InChaosCastle()==false )
                {
					...
					if (gCustomPet2.CheckCustomPetByItem(Hero->Helper.Type))
					{
						CreateBug(Hero->Helper.Type + 1171, o->Position, o);
						if (!Hero->SafeZone && gCustomPet2.GetInfoPetType(Hero-> Helper.Type) == 5) CreateEffect(BITMAP_MAGIC + 1, o->Position, o->Angle, o->Light, 1, o);
					}
                }
			}
...
#endif //PBG_ADD_NEWCHAR_MONK_ITEM
					Type==ITEM_WING+40 ||
					gCustomWing.CheckCustomWingByModelType(Type, 1))
				{
...
bool IsHighValueItem(ITEM* pItem)
{
	...
	if(
		...
		gCustomJewel.CheckCustomJewelByItem(pItem->Type) ||
		...
		|| gCustomWing.CheckCustomWingByItem(pItem->Type) ||
		...
		|| gCustomPet2.GetInfoPetType(pItem->Type) ==5
...
bool IsRepairBan(ITEM* pItem)
{
	...
	if(	(  pItem->Type >= ITEM_POTION+55 && pItem->Type <= ITEM_POTION+57)
		...
		|| pItem->Type == ITEM_BOW+7 //CustomArrow
		|| pItem->Type == ITEM_BOW+15 //CustomArrow
...
{	
	...
	//==Custom
	int TypePosition = 0;
	if (ItemTRSData.GetItemTRSData(Type - MODEL_ITEM)) TypePosition = 1;
	if (gCustomBow.CheckCustomBow(Type - MODEL_ITEM)) TypePosition = 2;
	if (gCustomBow.CheckCustomCross(Type - MODEL_ITEM)) TypePosition = 3;
	if (gCustomBattleGloves.CheckGloves(Type - MODEL_ITEM)) TypePosition = 4;
	if (gCustomPet2.CheckCustomPetByItem(Type - MODEL_ITEM))
	{
		Type = gCustomPet2.GetInfoByItemInventory(Type - MODEL_ITEM);
		TypePosition = 5;
	}
	...
	if (TypePosition > 0)
	{
		...
		case 2:
		{
			Vector(180.f, -90.0, 15.0, ObjectSelect.Angle);
		}
		break;
		case 3:
		{
			Vector(90.0, 180.0, 20.0, ObjectSelect.Angle);
		}
		break;
		case 4: //RF
		{
			...
			Position[0] += 0.002f;
			Position[1] += 0.02f;
			Vector(270.0, 0.0, 0.0, ObjectSelect.Angle);
		}
		break;
		case 5:
		{
			Position[0] += gCustomPet2.GetScalePosX(Type - MODEL_ITEM);
			Position[1] -= gCustomPet2.GetScalePosY(Type - MODEL_ITEM);
			Vector(270.0, gCustomPet2.GetScaleAngle(Type - MODEL_ITEM), 0.0, ObjectSelect.Angle);
			//Scale *= gCustomPet2.GetScaleInv(Type - MODEL_ITEM);
		}
		break;
		}
	}
	else if(Type == MODEL_SWORD+0)
	{
		Position[0] -= 0.02f;
		Position[1] += 0.03f;
...
//		Position[0] += 0.02f;
	...
	else if( Type == MODEL_BOW+24 || Type == MODEL_BOW + 28 || Type == MODEL_BOW + 31 || Type == MODEL_BOW + 36 || Type == MODEL_BOW + 38 || Type == MODEL_BOW + 40 || Type == MODEL_BOW + 42) //DAT FIX TOA DO CUNG
	{
		Position[0] -= 0.07f;
		Position[1] += 0.07f;
    	Vector(180.f,-90.f,15.f,ObjectSelect.Angle);
	...
	else if (Type == MODEL_HELPER+178 || Type >= MODEL_HELPER+181 && Type <= MODEL_HELPER+187 || Type == MODEL_HELPER+190 || Type == MODEL_HELPER+193
		 ||Type == MODEL_HELPER+171 || Type == MODEL_HELPER+151) //DAT FIX PEN
	{
		Position[1] -= 0.04f;
		Position[0] += 0.002f;
		Vector(270.0f, 0.0f, 0.0f, ObjectSelect.Angle);
	}
	else if ( Type >= MODEL_POTION+210 && Type <= MODEL_POTION+214 ) //DAT FIX NGOC CUSTOM
	{
		Position[1] += 0.05f;
		Position[0] += 0.002f;
		Vector(270.0f, 0.0f, 0.0f, ObjectSelect.Angle);
	}
	else if ( Type == MODEL_WING+150) //DAT FIX WING 2.5 DK
	{
		Position[1] += 0.10f;
		Position[0] -= 0.02f;
		Vector(270.0f, 0.0f, 0.0f, ObjectSelect.Angle);
	...
	else if(Type >= MODEL_SWORD+33 && Type <= MODEL_SWORD+35)
	{
		Position[0] += 0.002f;
		Position[1] += 0.02f;
		if (Type == MODEL_SWORD + 35)
		{
			Position[0] -= 0.0010f;//X
			//Position[1] += 0.089f;//Y
		}
		Vector(0.f,0.f,0.f,ObjectSelect.Angle);
	}
	...
	else
	{
		...
		else if ( Type >= MODEL_POTION+55 && Type <= MODEL_POTION+57 || Type >= MODEL_POTION+157 && Type <= MODEL_POTION+159) //DAT FIX SIZE ITEM INVSion
		{
			Scale = 0.0016f;
		}
		else if( Type==MODEL_HELPER+38 )
		...
		else if (Type == MODEL_HELPER + 150)		// °ñµç¸ÞÀÌÇÃÂü
		{
			Scale = 0.0040f;
		}
#endif	//LJH_ADD_ITEMS_EQUIPPED_FROM_INVENTORY_SYSTEM_PART_2
		...
		{
			if(Type == MODEL_HELPER+176 || Type == MODEL_HELPER+177 || Type == MODEL_HELPER+179 || Type == MODEL_HELPER+180
			|| Type == MODEL_HELPER+188 || Type == MODEL_HELPER+189 || Type == MODEL_HELPER+191 || Type == MODEL_HELPER+192) //DAT FIX RING SIZE
			{
				Scale = 0.0013f;
			}
			else if (Type == MODEL_HELPER+178 || Type == MODEL_HELPER+181 || Type == MODEL_HELPER+184 || Type == MODEL_HELPER+187 || Type == MODEL_HELPER+190 || Type == MODEL_HELPER+193) //DAT FIX PEN SIZE
			{
				Scale = 0.0025f;
			}
			else if (Type == MODEL_HELPER+182 || Type == MODEL_HELPER+183 || Type == MODEL_HELPER+185 || Type == MODEL_HELPER+186) //DAT FIX RING PEN
			{
				Scale = 0.003f;
			}
			else if (Type == MODEL_AXE+84)
			{
				Scale = 0.002f;
			}
	...
	float FixScaleItem = ItemTRSData.GetItemTRSDataScale(Type - MODEL_ITEM);
...
#ifdef PBG_ADD_NEWCHAR_MONK_ITEM
	if(Type>=ITEM_SWORD+32 && Type<=ITEM_SWORD+35 || gCustomBattleGloves.CheckGloves(Type))
	{
		sx -= Width*0.25f;
		sy -= Height*0.25f;
	}
...
void BCreateGuildMark(BYTE* MarkBuffer, bool blend)
{
	...
	{
		if (i % 2 == 0)
			GuildMark[i] = (MarkBuffer[i / 2] >> 4) & 0x0f;
		else
			GuildMark[i] = MarkBuffer[i / 2] & 0x0f;
	}
	BYTE * v7 = &*(BYTE*)GuildMark; //Mark
	for (int i = 0; i < Height; i++)
	{
		for (int j = 0; j < Width; j++)
		{
			*((unsigned int*)(Buffer)) = MarkColor[v7[0]];
			Buffer += 4;
			v7++;
		}
	}
