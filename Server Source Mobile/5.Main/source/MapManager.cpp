...
void CMapManager::LoadWorld(int Map)
{
	...
	{
		char szDebugOutput[160];
		sprintf(szDebugOutput, "LoadWorld OpenTerrainMapping enc world=%d result=%d file=%s", iMapWorld, iResult, FileName);
		OutputDebugStringA(szDebugOutput);
	}
	if(iResult == -1)
	{
		sprintf(FileName, "Data\\%s\\Terrain.map", WorldName);
		iResult = OpenTerrainMapping(FileName);
		char szDebugOutput[160];
		sprintf(szDebugOutput, "LoadWorld OpenTerrainMapping fallback result=%d file=%s", iResult, FileName);
		OutputDebugStringA(szDebugOutput);
	}
#endif

	if ( iMapWorld != iResult)
	{
		char Text[256];
   		sprintf(Text,"%s file corrupted1 (%d/%d).",FileName, iMapWorld, iResult);
	...
	{
		char szDebugOutput[160];
		sprintf(szDebugOutput, "LoadWorld OpenTerrainAttribute enc world=%d result=%d file=%s", iMapWorld, iResult, FileName);
		OutputDebugStringA(szDebugOutput);
	}
	if(iResult == -1)
	{
		...
		iResult = OpenTerrainAttribute(FileName);
		char szDebugOutput[160];
		sprintf(szDebugOutput, "LoadWorld OpenTerrainAttribute fallback result=%d file=%s", iResult, FileName);
		OutputDebugStringA(szDebugOutput);
	}
#endif
	if ( iMapWorld != iResult)
	{
		char Text[256];
   		sprintf(Text,"%s file corrupted2.",FileName);
		g_ErrorReport.Write( Text);
		g_ErrorReport.Write( "\r\n");
		MessageBox(g_hWnd,Text,NULL,MB_OK);
		SendMessage(g_hWnd,WM_DESTROY,0,0);
	...
	{
		char szDebugOutput[160];
		sprintf(szDebugOutput, "LoadWorld OpenObjectsEnc enc world=%d result=%d file=%s", iMapWorld, iResult, FileName);
		OutputDebugStringA(szDebugOutput);
	}
	if(iResult == -1)
	{
		...
		iResult = OpenObjectsEnc(FileName);
		char szDebugOutput[160];
		sprintf(szDebugOutput, "LoadWorld OpenObjectsEnc fallback result=%d file=%s", iResult, FileName);
		OutputDebugStringA(szDebugOutput);
	}
#endif
	if ( iMapWorld != iResult)
	{
		char Text[256];
   		sprintf(Text,"%s file corrupted3.",FileName);
		g_ErrorReport.Write( Text);
		g_ErrorReport.Write( "\r\n");
		MessageBox(g_hWnd,Text,NULL,MB_OK);
		SendMessage(g_hWnd,WM_DESTROY,0,0);
