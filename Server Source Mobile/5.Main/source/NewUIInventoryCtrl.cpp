...
STORAGE_TYPE CNewUIPickedItem::GetSourceStorageType() const
{
	if (m_pSrcInventory)
	{
		...
		return (STORAGE_TYPE)g_MixRecipeMgr.GetMixInventoryEquipmentIndex();
	}
...
bool SEASON3B::CNewUIInventoryCtrl::CanChangeItemColorState(ITEM * pItem)
{
	...
	if (pItem->Type == ITEM_BOW + 7 || pItem->Type == ITEM_BOW + 15) //CustomArrow
	{
		return false;
	}
...
int SEASON3B::CNewUIInventoryCtrl::FindEmptySlot(IN int cx, IN int cy,bool Ext)
{
...
void SEASON3B::CNewUIInventoryCtrl::Render()
{
	...
	const int frameTopLeft = m_Pos.x - WND_LEFT_EDGE + 14;
	const int frameTopWidth = m_Size.cx + WND_LEFT_EDGE - WND_RIGHT_EDGE - 14;
	if (frameTopWidth > 0)
	{
		RenderImage(IMAGE_ITEM_TABLE_TOP_PIXEL, frameTopLeft, m_Pos.y - WND_TOP_EDGE, frameTopWidth, 14);
		RenderImage(IMAGE_ITEM_TABLE_BOTTOM_PIXEL, frameTopLeft, m_Pos.y + m_Size.cy - WND_BOTTOM_EDGE, frameTopWidth, 14);
	}
	const int frameSideTop = m_Pos.y - WND_TOP_EDGE + 14;
	const int frameSideHeight = m_Size.cy + WND_TOP_EDGE - WND_BOTTOM_EDGE - 14;
	if (frameSideHeight > 0)
	{
		RenderImage(IMAGE_ITEM_TABLE_LEFT_PIXEL, m_Pos.x - WND_LEFT_EDGE, frameSideTop, 14, frameSideHeight);
		RenderImage(IMAGE_ITEM_TABLE_RIGHT_PIXEL, m_Pos.x + m_Size.cx - WND_RIGHT_EDGE, frameSideTop, 14, frameSideHeight);
	}
...
bool SEASON3B::CNewUIInventoryCtrl::CheckIsToolTip() const
{
	return m_pToolTipItem;
}
...
bool SEASON3B::CNewUIInventoryCtrl::AreItemsStackable(ITEM * pSourceItem, ITEM * pTargetItem)
{
	...
	if ((iSrcType == ITEM_BOW + 7 && iTarType == ITEM_BOW + 7) && (iSrcLevel == iTarLevel)) //CustomArrow
	{
		return true;
	}

	if ((iSrcType == ITEM_BOW + 15 && iTarType == ITEM_BOW + 15) && (iSrcLevel == iTarLevel)) //CustomArrow
	{
		return true;
	}
...
bool SEASON3B::CNewUIInventoryCtrl::CanUpgradeItem(ITEM * pSourceItem, ITEM * pTargetItem)
{
	...
	if (((pTargetItem->Type >= ITEM_SWORD && pTargetItem->Type < ITEM_WING)
		&& (pTargetItem->Type != ITEM_BOW + 7)//CustomArrow
		&& (pTargetItem->Type != ITEM_BOW + 15))//CustomArrow
