...
DWORD CUIWindowMgr::AddWindow(int iWindowType, int iPos_x, int iPos_y, const char* pszTitle, DWORD dwParentID, int iOption)
{
	...
	const BOOL bDefaultPosX = (iPos_x == UIWND_DEFAULT);
	const BOOL bDefaultPosY = (iPos_y == UIWND_DEFAULT);
	if (bDefaultPosX) iPos_x = 0;
	if (bDefaultPosY) iPos_y = 332;

	pbw->Init(pszTitle, dwParentID);

	if (bDefaultPosX && bDefaultPosY
		&& (iWindowType == UIWNDTYPE_QUESTION
			|| iWindowType == UIWNDTYPE_QUESTION_FORCE
			|| iWindowType == UIWNDTYPE_OK
			|| iWindowType == UIWNDTYPE_OK_FORCE))
	{
		iPos_x = (640 - pbw->GetWidth()) / 2;
		iPos_y = (480 - pbw->GetHeight()) / 2;
	}
...
void CUIPhotoViewer::ClearItem()
{
	...
	for (int i = 0; i < MAX_BODYPART; ++i)
	{
		...
		m_PhotoChar.BodyPart[i].Type = -1;
		m_PhotoChar.BodyPart[i].Level = 0;
		m_PhotoChar.BodyPart[i].Option1 = 0;
	}
	m_PhotoChar.BodyPart[1].Type = MODEL_BODY_HELM + gCharacterManager.GetSkinModelIndex(m_PhotoChar.Class);
	m_PhotoChar.BodyPart[2].Type = MODEL_BODY_ARMOR + gCharacterManager.GetSkinModelIndex(m_PhotoChar.Class);
	m_PhotoChar.BodyPart[3].Type = MODEL_BODY_PANTS + gCharacterManager.GetSkinModelIndex(m_PhotoChar.Class);
	m_PhotoChar.BodyPart[4].Type = MODEL_BODY_GLOVES + gCharacterManager.GetSkinModelIndex(m_PhotoChar.Class);
	m_PhotoChar.BodyPart[5].Type = MODEL_BODY_BOOTS + gCharacterManager.GetSkinModelIndex(m_PhotoChar.Class);

	m_PhotoChar.Weapon[0].Type = -1;
	m_PhotoChar.Weapon[0].Level = 0;
	m_PhotoChar.Weapon[0].Option1 = 0;

	m_PhotoChar.Weapon[1].Type = -1;
	m_PhotoChar.Weapon[1].Level = 0;
	m_PhotoChar.Weapon[1].Option1 = 0;

	m_PhotoChar.Wing.Type = -1;
	m_PhotoChar.Wing.Level = 0;
	m_PhotoChar.Wing.Option1 = 0;
	m_PhotoChar.Wing.m_pCloth[0] = NULL;
	m_PhotoChar.Wing.m_pCloth[1] = NULL;
	m_PhotoChar.Wing.m_byNumCloth = 0;

	m_PhotoChar.Helper.Type = -1;
	m_PhotoChar.Helper.Level = 0;
	m_PhotoChar.Helper.Option1 = 0;
	m_PhotoChar.Helper.m_pCloth[0] = NULL;
	m_PhotoChar.Helper.m_pCloth[1] = NULL;
	m_PhotoChar.Helper.m_byNumCloth = 0;
	m_PhotoHelper.Live = false;
}
void CUIPhotoViewer::UpdateItemChar(int Slot, int IndexItem, int Level, int Ex, int Ancient)
{
	// Kiểm tra nếu ứng dụng chưa được khởi tạo
	if (m_bIsInitialized == FALSE) return;

	// Kiểm tra nếu Slot nằm ngoài phạm vi hợp lệ
	if (Slot < 0 || Slot >= 9) {
		// Slot không hợp lệ, thoát hàm
		return;
	}

	switch (Slot)
	{
		// Xử lý các trường hợp của bộ phận cơ thể
	case 2: // helm
	case 3: // helm
	case 4: // helm
	case 5: // helm
	case 6: // helm
	{
		...
		if (adjustedSlot >= 0 && adjustedSlot < 6) {
			m_PhotoChar.BodyPart[adjustedSlot].Type = IndexItem;
			m_PhotoChar.BodyPart[adjustedSlot].Level = Level;
			m_PhotoChar.BodyPart[adjustedSlot].ExtOption = Ancient;
			m_PhotoChar.BodyPart[adjustedSlot].Option1 = Ex;
		}
		break;
	}

	// Xử lý các trường hợp của vũ khí
	case 0: // weapon 0
	case 1: // weapon 1
		if (Slot >= 0 && Slot < 2) {
			m_PhotoChar.Weapon[Slot].Type = IndexItem;
			m_PhotoChar.Weapon[Slot].Level = Level;
			m_PhotoChar.Weapon[Slot].ExtOption = Ancient;
			m_PhotoChar.Weapon[Slot].Option1 = Ex;
		}
		break;

		// Xử lý trường hợp của cánh
	case 7: // wings
		if (Hero != nullptr) {
			memcpy(&m_PhotoChar.Wing, &Hero->Wing, sizeof(PART_t));
			DeleteCloth(nullptr, &m_PhotoChar.Object, nullptr);
			m_PhotoChar.Wing.Type = IndexItem;
			m_PhotoChar.Wing.Level = Level;
			m_PhotoChar.Wing.ExtOption = Ancient;
			m_PhotoChar.Wing.Option1 = Ex;
		}
		break;

		// Xử lý trường hợp của trợ thủ
	case 8:
	{
		if (Hero != nullptr) {
			memcpy(&m_PhotoChar.Helper, &Hero->Helper, sizeof(PART_t));
			m_PhotoChar.Helper.Type = IndexItem;
			m_PhotoChar.Helper.Level = Level;
			m_PhotoChar.Helper.ExtOption = Ancient;
			m_PhotoChar.Helper.Option1 = Ex;

			// Đặt tư thế cho ảnh
			SetPhotoPose(AT_STAND1);
			m_iSettingAnimation = AT_STAND1;

			// Xử lý các loại trợ thủ khác nhau
			int helperType = m_PhotoChar.Helper.Type - MODEL_HELPER;
			switch (helperType)
			{
			case 0:
			...
			case 37:
				...
				break;
			default:
				// Xử lý trường hợp trợ thủ tùy chỉnh
				if (gCustomPet2.CheckCustomPetByItem(helperType)) {
					CreateBugSub(m_PhotoChar.Helper.Type, m_PhotoChar.Object.Position, &m_PhotoChar.Object, &m_PhotoHelper);
				}
				break;
			}

			// Điều chỉnh alpha và scale của trợ thủ
			m_PhotoHelper.Alpha = 0;
			m_fPhotoHelperScale = m_PhotoHelper.Scale * 0.7f;
		}
		break;
	}

	default:
		// Xử lý trường hợp không có Slot phù hợp
		break;
	}
}
...
void CUILetterWriteWindow::Init(const char* pszTitle, DWORD dwParentID)
{
	...
	m_TextInputBox.SetTextLimit(MAX_LETTER_TITLE_LENGTH - 1);
	m_TextInputBox.SetParentUIID(m_dwUIID);
	m_TextInputBox.SetFont(g_hFont);
	...
	case UI_MESSAGE_BTNLCLICK:
	{
		...
		case 1:
			if (m_bIsSend == FALSE)
			{
				...
				//wchar_t* szTitleUTF16 = new wchar_t[MAX_LETTER_TITLE_LENGTH];
				//wchar_t* szTextUTF16 = new wchar_t[MAX_LETTER_TITLE_LENGTH];
				...
				//m_MailtoInputBox.GetText(szMailto, MAX_ID_SIZE + 1);
				//m_TitleInputBox.GetText(szTitleUTF16, MAX_LETTER_TITLE_LENGTH);
				//m_TextInputBox.GetText(szTextUTF16, MAX_LETTER_TITLE_LENGTH);

				//for (k = 0; k < MAX_LETTER_TITLE_LENGTH + 1; k++)
				...
				else if (szTitle[0] == '\0' || wcslen(szTitle) == 0)
				{
				...
				else if (szTempText[0] == '\0' || wcslen(szTempText) == 0)
				{
...
void CUITextInputWindow::ReturnText()
{
	...
	g_pWindowMgr->SendUIMessageToWindow(m_dwReturnWindowUIID, UI_MESSAGE_TXTRETURN, GetUIID(), (DWORD)(uintptr_t)pszReturnText);
	g_pWindowMgr->SendUIMessage(UI_MESSAGE_CLOSE, GetUIID(), 0);
}
