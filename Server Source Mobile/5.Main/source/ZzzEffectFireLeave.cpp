...
bool IsPersistentAmbientFireObject(const OBJECT *o)
{
	if (o == nullptr)
	{
		return false;
...
bool ShouldEmitStableAmbientFrame(const OBJECT *o, int period, int phase)
{
	if (period <= 1)
	{
		return true;
	...
	if (o == nullptr)
	{
		return rand_fps_check(period);
	}

	const int worldTick = (int)(WorldTime * 0.025f);
	int seed = ((int)o->Position[0] * 3) ^ ((int)o->Position[1] * 5) ^ (o->Type * 7) ^ phase;
	seed %= period;
	if (seed < 0)
	{
		seed += period;
	...
	return ((worldTick + seed) % period) == 0;
}
...
{
	vec3_t Light;
	vec3_t p, BasePosition, Position;
	float Luminosity;
	float Matrix[3][4];
	const bool persistentAmbientFire = IsPersistentAmbientFireObject(o);
	AngleMatrix(o->Angle, Matrix);
	Vector(x, y, z, p);
	VectorRotate(p, Matrix, BasePosition);
	VectorAdd(BasePosition, o->Position, BasePosition)
	VectorCopy(BasePosition, Position);
	Position[0] += (float)(rand() % 16 - 8);
	Position[1] += (float)(rand() % 16 - 8);
	Position[2] += (float)(rand() % 16 - 8);
	switch (Type)
	...
	case 0:
		...
		{
			vec3_t GlowLight;
			Vector(Light[0] * 0.85f, Light[1] * 0.65f, Light[2] * 0.50f, GlowLight);
			CreateSprite(BITMAP_LIGHT, BasePosition, 0.9f + Luminosity * 0.35f, GlowLight, o);
		}
#endif
		if (persistentAmbientFire ? ShouldEmitStableAmbientFrame(o, 4, Type) : rand_fps_check(
#if defined(__ANDROID__) || defined(MU_IOS)
			2
#else
			2
		...
		{
			CreateParticle(BITMAP_FIRE, Position, o->Angle, Light, rand() % 4, 1.f, o);
		}
		AddTerrainLight(BasePosition[0], BasePosition[1], Light, 4, PrimaryTerrainLight);
		break;
	case 1:
		if (persistentAmbientFire ? ShouldEmitStableAmbientFrame(o, 3, 101) : rand_fps_check(2))
			CreateParticle(BITMAP_SMOKE, Position, o->Angle, o->Light, 0, 1.f, o);
		break;
	case 2:
		if (persistentAmbientFire ? ShouldEmitStableAmbientFrame(o, 3, 202) : rand_fps_check(2))
			CreateParticle(BITMAP_SMOKE, Position, o->Angle, o->Light, 2, 1.f, o);
		break;
	}
}
