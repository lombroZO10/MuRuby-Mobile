...
{\
	...
	g_ConsoleDebug->Write(MCD_NORMAL, "3 CBLoad  %s %s", Serial, Version);\
}
...
{\
	if(!FindText2(Hero->ID,"webzen") && ( p_Type==40 || p_Type==AT_SKILL_DARKSIDE || p_Type==MASTER_SKILL_ADD_DARK_SIDE_IMPROVED|| p_Type==MASTER_SKILL_ADD_DARK_SIDE_ENHANCED|| p_Type==AT_SKILL_STAMP || p_Type==MASTER_SKILL_ADD_UPPER_BEAST_IMPROVED || p_Type==MASTER_SKILL_ADD_UPPER_BEAST_ENHANCED || abs( (int)(GetTickCount() - g_dwLatestMagicTick)) > 300 ))\
	{\
...
__forceinline void BDataSend(BYTE* lpMsg, DWORD size) // OK
{
	g_ConsoleDebug->Write(MCD_ERROR, "DataSend %x %x %x", (BYTE)lpMsg[0], (BYTE)lpMsg[2], (BYTE)lpMsg[3] );
	//return;
	CStreamPacketEngine spe;
	if ((BYTE)lpMsg[0] == 0xC2 || (BYTE)lpMsg[0] == 0xC4)
	{
		spe.Init(lpMsg[0], lpMsg[3]);
...
#if(HAISLOTRING)
	if (pItem->ex_src_type == ITEM_EX_SRC_EQUIPMENT2 && pItem->bySelectedSlotIndex >= 12 && pItem->bySelectedSlotIndex <= 15)
	{
		iSrcType = 40;
	}

	//char Text[100];
	//sprintf(Text, "ex_src_type %d iSrcType %d iSrcIndex %d iDstType %d iDstIndex %d", pItem->ex_src_type, iSrcType, iSrcIndex, iDstType, iDstIndex);
	//CreateNotice(Text, 1);
#endif
	EquipmentItem = true;
...
__forceinline void SendRequestUse(int Index,int Target)
{
	...
	g_ConsoleDebug->Write(MCD_SEND, "0x26 [SendRequestUse(%d) -> %d]", Index, Target);
}
#endif //ENABLE_EDIT
...
{	\
	CStreamPacketEngine spe;\
	spe.Init( 0xC1, 0x86);\
    spe << ( BYTE)( ( p_Type)>>8) << ( BYTE)( ( p_Type)&0xff);\
    spe << ( BYTE)( p_SubType);\
	spe.Send();\
}
...
//	BYTE NumBerSend = (pPickedItem->GetItem()->ex_src_type== ITEM_EX_SRC_EQUIPMENT2) ?0x09: 0x01;\
//	CStreamPacketEngine spe;\
//	spe.Init( 0xC1, 0x3F);\
//	spe << ( BYTE)NumBerSend;\
//	spe << ( BYTE)offset;\
