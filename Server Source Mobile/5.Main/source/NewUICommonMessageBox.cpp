...
void SEASON3B::CNewUIMessageBoxButton::Render()
{
	...
	if(m_strText.size() > 0)
	{
		...
		//==Fix Size Text Button Test Wide
		Fontsize.cx = Fontsize.cx / ((float)WindowWidth / DisplayWin);
		Fontsize.cy = Fontsize.cy / ((float)WindowHeight / DisplayHeight);
...
bool SEASON3B::CMapEnterWerwolfMsgBoxLayout::SetLayout()
{
	...
	pMsgBox->SetPos((DisplayWin / 2) - (MSGBOX_WIDTH / 2), 50);
...
#if(CB_CUSTOMMIXINFO)
	if (gCB_InfoCustomMix)
	{
		if (!gCB_InfoCustomMix->m_DataInfoCustomMix.empty())
		{
			if (gCB_InfoCustomMix->m_DataInfoCustomMix[0].Name2 == 0)
			{
				unicode::_sprintf(strText, "%s", GlobalText[gCB_InfoCustomMix->m_DataInfoCustomMix[0].Name1]);
			}
			else if (gCB_InfoCustomMix->m_DataInfoCustomMix[0].Name3 == 0)
			{
				unicode::_sprintf(strText, "%s %s", GlobalText[gCB_InfoCustomMix->m_DataInfoCustomMix[0].Name1],
					GlobalText[gCB_InfoCustomMix->m_DataInfoCustomMix[0].Name2]);
			}
			else
			{
				unicode::_sprintf(strText, "%s %s %s", GlobalText[gCB_InfoCustomMix->m_DataInfoCustomMix[0].Name1],
					GlobalText[gCB_InfoCustomMix->m_DataInfoCustomMix[0].Name2],
					GlobalText[gCB_InfoCustomMix->m_DataInfoCustomMix[0].Name3]);
			}
		}
		else
		{
...
CALLBACK_RESULT SEASON3B::CMixCheckMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (gCB_InfoCustomMix)
	{
		if (!gCB_InfoCustomMix->m_DataInfoCustomMix.empty())
		{
			SendRequestMix(gCB_InfoCustomMix->m_DataInfoCustomMix[0].Index, g_MixRecipeMgr.GetMixSubType());
			goto NextSendRequestMix;
		}
	}
...
CALLBACK_RESULT SEASON3B::CPersonalShopItemValueCheckMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if(pPickedItem)
	{
		...
		if(pPickedItem->GetOwnerInventory() == g_pMyInventory->GetInventoryCtrl() || g_pMyInventoryExt->GetOwnerOf(pPickedItem))
		{
			...
			//g_ConsoleDebug->Write(MCD_ERROR, "@SendRequestEquipmentItem %d %d ", iSourceIndex, MAX_MY_INVENTORY_EX_INDEX + iTargetIndex);
		}
		...
		else if(pPickedItem->GetOwnerInventory() == g_pMyShopInventory->GetInventoryCtrl())
		{
			...
			SendRequestEquipmentItem(REQUEST_EQUIPMENT_MYSHOP, MAX_MY_INVENTORY_EX_INDEX +iSourceIndex, pItemObj,REQUEST_EQUIPMENT_MYSHOP, MAX_MY_INVENTORY_EX_INDEX +iTargetIndex);
		}
		
		AddPersonalItemPrice(MAX_MY_INVENTORY_EX_INDEX +iTargetIndex, pMsgBox->GetItemValue(), g_IsPurchaseShop);
	}
	else 
	{
		iSourceIndex = g_pMyShopInventory->GetSourceIndex();
		iSourceIndex += MAX_MY_INVENTORY_EX_INDEX;
...
CALLBACK_RESULT SEASON3B::CPersonalShopItemBuyMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if( pItem && pCha )
	{
		...
		g_ConsoleDebug->Write(MCD_RECEIVE, "[SendRequestPurchase] sourceIndex %d", sourceIndex);
	}
...
bool SEASON3B::CMaster_Level_Interface::SetLayout()
{
	...
	unicode::_sprintf( szText, GlobalText[1772], g_pMasterSkillTreeInterface->GetConsumePoint());
...
CALLBACK_RESULT SEASON3B::CMaster_Level_Interface::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	g_ConsoleDebug->Write(MCD_RECEIVE, "SendRequestMasterLevelSkill %d reqPoint %d", g_pMasterSkillTreeInterface->GetCurSkillID(), g_pMasterSkillTreeInterface->GetConsumePoint());
