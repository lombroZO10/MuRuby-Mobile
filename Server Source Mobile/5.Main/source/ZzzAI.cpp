...
void SetAction_Fenrir_Damage(CHARACTER* c, OBJECT* o)
{
	...
	else if(c->Weapon[0].Type != -1 && c->Weapon[1].Type != -1 && c->Weapon[1].Type ==MODEL_BOW+7) //CustomArrow No
		SetAction(o, PLAYER_FENRIR_DAMAGE_ONE_RIGHT);
	else if(c->Weapon[0].Type == -1 && c->Weapon[1].Type != -1)
...
{
	if(c->Weapon[0].Type != -1 && c->Weapon[1].Type != -1 && c->Weapon[0].Type !=MODEL_BOW+15 && c->Weapon[1].Type !=MODEL_BOW+7) //CustomArrow Cung,No
	{
...
void SetAction(OBJECT *o,int Action, bool bBlending)
{
	...
	//{
	//	//g_ErrorReport.Write("OldAction: [CurrentAction: %d][Enter Action: %d][MaxAction: %d]\r\n", o->CurrentAction, Action, b->NumActions);
	//	Models[MODEL_PLAYER].Actions[PLAYER_STOP_FLY].PlaySpeed = 0.4f;
	//	gInterface.DrawMessage(1, "OldAction: Speed %f [CurrentAction:% d] [Enter Action : % d] [MaxAction:% d] \r\n", Models[MODEL_PLAYER].Actions[Action].PlaySpeed, o->CurrentAction, Action, b->NumActions);
	//}


	if (Action < b->NumActions && o->CurrentAction != Action)
	{
