...
namespace
{
	inline unsigned long long MainScenePerfNow()
	{
		return static_cast<unsigned long long>(MU_MobilePerfNow());
	}

	inline unsigned long long MainScenePerfElapsed(unsigned long long start)
	{
...
namespace
{
float GetSceneFrustumScale(int viewportWidth, int viewportHeight)
{
#if defined(__ANDROID__) || defined(MU_IOS)
	return 1.0f;
#else
	return static_cast<float>(viewportWidth) / 640.0f;
#endif
}

bool SkipNonEssentialMobilePasses()
{
#if defined(__ANDROID__) || defined(MU_IOS)
	return false;
#else
...
bool IsAndroidHeavyOverlayVisible()
{
#if defined(__ANDROID__) || defined(MU_IOS)
...
float GetAndroidStableSceneViewFar()
{
#if defined(__ANDROID__) || defined(MU_IOS)
	return 4600.0f;
#else
...
bool NewRenderCharacterScene(HDC hDC)
{
	...
	if(!CUIMng::Instance().IsCursorOnUI()) SelectObjects();

	if(!skipNonEssentialPasses) RenderBugs();
	RenderBlurs();
	RenderJoints();
	RenderEffects();
	ThePetProcess().RenderPets();
	if(!skipNonEssentialPasses) RenderBoids();
...
// 	glFogf(GL_FOG_END, 4000.0f);
	...
	if (!CUIMng::Instance().m_CreditWin.IsShow())
	{
		...
		RenderTerrain(false);
#if defined(__ANDROID__) || defined(MU_IOS)
		CameraViewFar = skipNonEssentialPasses ? 3600.f : 4600.f;
#else
	...
	if (CCameraMove::GetInstancePtr()->IsTourMode())
	{
		...
		static bool s_loggedAndroidLogoAlpha = false;
		if(!s_loggedAndroidLogoAlpha && g_fMULogoAlpha > 1.0f)
		{
			char szDebugOutput[128];
			sprintf(szDebugOutput, "ZzzScene android logo alpha raw=%.3f clamp=%.3f glow=%.3f", g_fMULogoAlpha, fLogoAlpha, fLogoGlowAlpha);
			OutputDebugStringA(szDebugOutput);
			s_loggedAndroidLogoAlpha = true;
		}
		EnableAlphaBlend();
		...
		RenderBitmap(BITMAP_LOG_IN+17, DisplayWinMid -128.0f*0.8f,25.0f, 256.0f*0.8f,128.0f*0.8f);
	...
	g_pRenderText->RenderText((335+ DisplayWinExt) -Size.cx* DisplayWin /WindowWidth, DisplayHeight -Size.cy* DisplayWin /WindowWidth-1,Text);

	strcpy(Text,GlobalText[455]);
	
	g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), Text,lstrlen(Text),&Size);
	g_pRenderText->RenderText((335+ DisplayWinExt), DisplayHeight -Size.cy* DisplayWin /WindowWidth-1,Text);
...
bool MoveMainCamera()
{
	...
	if (
		...
		CameraFOV = 61.0f;
#endif //PJH_NEW_SERVER_SELECT_MAP
	else if (!Camera3DSet && !Camera3DEnable)
	{
	...
	else 
	{
		...
		if (Camera3DSetMove)
		{
			...
			//===
			if (m_CursorY > MouseY && CameraAngle[0] > -90.f)
			{
			...
			if (m_CursorY < MouseY && CameraAngle[0] < -45.f)
			{
	...
	//else if (Camera3DEnable)
	//{
	//	CameraViewFar = 3200.f;
	//	//CameraViewFar = 60000.f;
	//	CameraPosition[0] = Hero->Object.Position[0];
	//	CameraPosition[1] = Hero->Object.Position[1] + BonusTest;
	//	CameraPosition[2] = CameraViewFar;
	//}
	else
	{
        ...
        {
            CameraViewFar = (std::max)(CameraViewFar, GetAndroidStableSceneViewFar());
        }
#endif
		...
		CameraPosition[2] += CameraDistance-(150.f+ Camera3DPosZ);//700 //Camera3D
		//CameraPosition[0] += PosZ;
		//CameraPosition[2] += BonusTest;  //Goc Nhin Cao Thap
		...
		CameraAngle[0] += Camera3DPosY;

		//g_ConsoleDebug->Write(MCD_NORMAL, "CameraViewFar. %f CameraPosition[2] %f", CameraViewFar, CameraPosition[2]);
...
void MoveMainScene()
{
	...
	{
		if(MouseY>=(int)(DisplayHeight-48))
			MouseOnWindow = true;
...
bool RenderMainScene()
{
	...
	if (IsWindowVisible(g_hWnd) == 0)
	{
		if (EnableMainRender)
		{
			...
			//if (!CameraTopViewEnable) RenderItems();
...
void MainScene(HDC hDC)
{
	...
	g_PhysicsManager.Move(0.025f  * FPS_ANIMATION_FACTOR); //0.025f 
	MoveNotices();
	...
	try
	{
		...
		if (GrabEnable && addTimeStampToCapture == 0)
		{
			g_pChatListBox->AddText("", Text, SEASON3B::TYPE_SYSTEM_MESSAGE);
		}
		GrabEnable = false;
		...
		if (SceneFlag == MAIN_SCENE)
		{
			...
			case WD_10HEAVEN:
				...
				if ((rand_fps_check(100)) == 0)
				{
					//                PlayBuffer(SOUND_HEAVEN01);
				}
				else if ((rand_fps_check(10)) == 0)
				{
...
//	GrabEnable = false;
//
//#if(DEBUG==1) || defined(LDS_FOR_DEVELOPMENT_TESTMODE) || defined(LDS_UNFIXED_FIXEDFRAME_FORDEBUG)
//	BeginBitmap();
//	unicode::t_char szDebugText[128];
//	unicode::_sprintf(szDebugText, "FPS : %.1f Connected: %d", FPS,g_bGameServerConnected);
//	unicode::t_char szMousePos[128];
//	unicode::_sprintf(szMousePos, "MousePos : %d %d ", MouseX, MouseY);
//	g_pRenderText->SetFont(g_hFontBold);
//	g_pRenderText->SetBgColor(0, 0, 0, 100);
//	g_pRenderText->SetTextColor(255, 255, 255, 200);
//	g_pRenderText->RenderText(10, DisplayHeight -26, szDebugText);
...
void Scene(HDC hDC)
{
    ...
    try
    {
        ...
        // Reset window title when not in game
        {
