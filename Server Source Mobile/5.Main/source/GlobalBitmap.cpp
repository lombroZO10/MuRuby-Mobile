...
{
	return (mode == GL_REPEAT || mode == GL_CLAMP_TO_EDGE) ? mode : GL_CLAMP_TO_EDGE;
}

static GLuint NormalizeTextureFilter(GLuint filter)
{
...
bool CGlobalBitmap::OpenJpeg(GLuint uiBitmapIndex, const std::string& filename, GLuint uiFilter, GLuint uiWrapMode)
{
	...
	{
		ExchangeExt(filename, "ozj", filename_ozj);
		infile = fopen(filename_ozj.c_str(), "rb");
	}
	if(infile == NULL) 
	{
		return false;
	...
	std::vector<unsigned char> fileBuffer((size_t)fileSize);
	if(fread(fileBuffer.data(), 1, fileBuffer.size(), infile) != fileBuffer.size())
	{
		fclose(infile);
		return false;
	}
...
bool CGlobalBitmap::OpenTga(GLuint uiBitmapIndex, const std::string& filename, GLuint uiFilter, GLuint uiWrapMode)
{
	...
	{
		ExchangeExt(filename, "ozt", filename_ozt);
		fp = fopen(filename_ozt.c_str(), "rb");
	}
    if(fp == NULL)
	{
		return false;
...
#if defined(__ANDROID__) || defined(MU_IOS)
	if(filename.find("MU-logo") != std::string::npos)
	{
		unsigned int alphaMin = 255;
		unsigned int alphaMax = 0;
		unsigned int alphaZero = 0;
		const int pixelCount = Width * Height;
		for(int i=0;i<pixelCount;i++)
		{
			const unsigned int alpha = pNewBitmap->Buffer[i*pNewBitmap->Components+3];
			if(alpha < alphaMin) alphaMin = alpha;
			if(alpha > alphaMax) alphaMax = alpha;
			if(alpha == 0) alphaZero++;
		}
		char szDebugOutput[512];
		sprintf(szDebugOutput, "OpenTga logo file=%s size=%dx%d alphaMin=%u alphaMax=%u alphaZero=%u cornerRGBA=%u,%u,%u,%u",
			filename_ozt.c_str(), Width, Height, alphaMin, alphaMax, alphaZero,
			pNewBitmap->Buffer[0], pNewBitmap->Buffer[1], pNewBitmap->Buffer[2], pNewBitmap->Buffer[3]);
		OutputDebugStringA(szDebugOutput);
	}
#endif
	SAFE_DELETE_ARRAY(PakBuffer);
