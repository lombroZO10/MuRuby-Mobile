...
void RenderNotices()
{
	...
	for(int i=0;i<MAX_NOTICE;i++)
	{
		...
		{
			g_pRenderText->RenderText(DisplayWinMid, (300+ DisplayHeightExt )+i*13, n->Text, 0, 0, RT3_WRITE_CENTER);
		}
		else
		{
			g_pRenderText->RenderText(DisplayWinMid, (300+ DisplayHeightExt )+i*13, n->Text, 0, 0, RT3_WRITE_CENTER);
...
void RenderBoolean(int x, int y, CHAT* c)
{
	...
	{
		g_pRenderText->SetFont(g_hFontBoldName);
		g_pRenderText->SetBgColor(0);
	}
	else
	{
	...
	if (IsShopInViewport(c->Owner))
	{
		...
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, GlobalText[1104], 0, iLineHeight, RT3_SORT_CENTER, &TextSize);
		RenderPos.x += TextSize.cx;

		g_pRenderText->SetTextColor(GetShopText2Color(c->Owner));
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->szShopTitle, RenderBoxSize.cx - TextSize.cx, iLineHeight, RT3_SORT_CENTER, &TextSize);
		g_pRenderText->SetFont(g_hFont);
	...
	{
		//g_pRenderText->SetBgColor(60, 100, 0, 150);
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->Union, RenderBoxSize.cx, iLineHeight, RT3_SORT_CENTER);
		RenderPos.y += iLineHeight;
	}
	if (c->Guild && c->Guild[0])
	{
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->Guild, RenderBoxSize.cx, iLineHeight, RT3_SORT_CENTER);
		RenderPos.y += iLineHeight;
	}
	...
	{
		g_pRenderText->SetFont(g_hFontBold);
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->ID, RenderBoxSize.cx, iLineHeight, RT3_SORT_CENTER);
		RenderPos.y += iLineHeight;
		g_pRenderText->SetFont(g_hFont);
	}
	else
	...
	*/
	...
	if (c->LifeTime[1] > 0)
	{
		...
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->Text[1], RenderBoxSize.cx, iLineHeight, RT3_SORT_CENTER);
		RenderPos.y += iLineHeight;

		g_pRenderText->SetTextColor(dwTextColor[0]);
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->Text[0], RenderBoxSize.cx, iLineHeight, RT3_SORT_CENTER);
	}
	else if (c->LifeTime[0] > 0)
	{
		g_pRenderText->SetTextColor(dwTextColor[0]);
	...
	if (KIND_PLAYER == c->Owner->Object.Kind && MODEL_PLAYER == c->Owner->Object.Type)
	{
		...
		if (1 == c->Owner->m_byGensInfluence)
			g_pNewUIGensRanking->RanderMark(x + RenderBoxSize.cx + 30, y, c->Owner->m_byGensInfluence, c->Owner->m_byRankIndex, SEASON3B::CNewUIGensRanking::MARK_BOOLEAN, (float)RenderPos.y);
		else if (2 == c->Owner->m_byGensInfluence)
			g_pNewUIGensRanking->RanderMark(x + RenderBoxSize.cx + 30, y, c->Owner->m_byGensInfluence, c->Owner->m_byRankIndex, SEASON3B::CNewUIGensRanking::MARK_BOOLEAN, (float)RenderPos.y);
#if(ShowLogoGuild)
		//===Show GUILD LOGO
		if (gProtect.m_MainInfo.PlayerShowGuildLogo)
		{
			if (c->Guild && c->Guild[0])
			{
...
void RenderBoolean(int x, int y, CHAT* c)
{
	...
	if (gMapManager.InChaosCastle() == true) //DAT FIX CC NAME
	{
		return;
	}
...
#endif
		gCustomRankUser.DrawInfo(c->Owner->Key,x, y, RenderBoxSize.cx, iLineHeight);
...
bool CheckAttack_Fenrir(CHARACTER* c)
{
	...
	{	
		if(((strcmp(GuildMark[Hero->GuildMarkIndex].GuildName,GuildMark[c->GuildMarkIndex].GuildName)==NULL) || g_pBCustomMenuInfo->AutoCtrlPK || (g_pPartyManager->IsPartyMember(SelectedCharacter))) && (HIBYTE(GetAsyncKeyState(VK_CONTROL))==128))
		{
			return true;
		}
	...
	else if(c->Object.Kind==KIND_PLAYER)
	{
		...
		else if ( c->PK >= PVP_MURDERER2 || g_pBCustomMenuInfo->AutoCtrlPK || ( HIBYTE(GetAsyncKeyState(VK_CONTROL))==128 && c!=Hero ) )
		{
			return true;
		}
...
bool CheckAttack()
{
	...
	{	
		...
		if(HIBYTE(GetAsyncKeyState(VK_CONTROL))==128 || g_pBCustomMenuInfo->AutoCtrlPK)
		{
			if(EnableGuildWar)
			{
				if(c->GuildTeam==2 && c!=Hero)
...
int	getTargetCharacterKey ( CHARACTER* c, int selected )
{
	...
	else if ( sc->PK>=PVP_MURDERER2 || g_pBCustomMenuInfo->AutoCtrlPK || ( HIBYTE(GetAsyncKeyState(VK_CONTROL))==128 && sc!=Hero ) )
	{
		return sc->Key;
	}
...
bool SkillWarrior(CHARACTER *c,ITEM *p)
{
	...
	if( Skill == AT_SKILL_RIDER 
		...
		|| ( Skill == AT_SKILL_SPEAR && ( Hero->Helper.Type == MODEL_HELPER+2 || Hero->Helper.Type == MODEL_HELPER+3 || Hero->Helper.Type == MODEL_HELPER+4 || Hero->Helper.Type == MODEL_HELPER+37  /*|| gCustomPet2.GetInfoPetType(Hero->Helper.Type - 1171) == 6 || gCustomPet2.GetInfoPetType(Hero->Helper.Type - 1171) == 5*/))
...
void UseSkillRagefighter(CHARACTER* pCha, OBJECT* pObj)
{
	...
	{
		g_ConsoleDebug->Write(MCD_ERROR, "UseSkillRagefighter 1");
		return;
	}

	if(g_CMonkSystem.IsRideNotUseSkill(iSkill, pCha->Helper.Type))
	{
		g_ConsoleDebug->Write(MCD_ERROR, "UseSkillRagefighter 2");
		return;
	}

	if(!g_CMonkSystem.IsSwordformGlovesUseSkill(iSkill))
	{
		g_ConsoleDebug->Write(MCD_ERROR, "UseSkillRagefighter 3");
		return;
	}
	...
	case MASTER_SKILL_ADD_UPPER_BEAST_ENHANCED:
		{
			...
			SendRequestRageAtt(iSkill, (WORD)wTargetKey);
...
void AttackRagefighter(CHARACTER *pCha, int nSkill, float fDistance)
{
	...
	g_ConsoleDebug->Write(MCD_ERROR, "AttackRagefighter Get SKill RF %d", nSkill);
	int iMana, iSkillMana;
	...
	if (iSkillMana > CharacterAttribute->SkillMana)
	{
		g_ConsoleDebug->Write(MCD_ERROR, "AttackRagefighter 1");
		return;
	}
	if (!gSkillManager.CheckSkillDelay(Hero->CurrentSkill))
	{
		g_ConsoleDebug->Write(MCD_ERROR, "AttackRagefighter 2");
		return;
	}
			...
			if( (gCharacterManager.GetEquipedBowType( lp ) == BOWTYPE_BOW) && (rp->Type == -1) )
				{
					...
					if(pItem)
					{
						SendRequestEquipmentItem(REQUEST_EQUIPMENT_INVENTORY, Index, pItem, REQUEST_EQUIPMENT_INVENTORY, EQUIPMENT_WEAPON_RIGHT);	
					}
					...
					if((gCharacterManager.GetEquipedBowType( rp ) == BOWTYPE_CROSSBOW) && (lp->Type == -1) )
				{
					...
					if(pItem)
					{
						SendRequestEquipmentItem(REQUEST_EQUIPMENT_INVENTORY, Index, pItem, REQUEST_EQUIPMENT_INVENTORY, EQUIPMENT_WEAPON_LEFT);	
					}
					g_pMyInventory->DeleteItem(Index);
					g_pChatListBox->AddText("", GlobalText[250], SEASON3B::TYPE_SYSTEM_MESSAGE);
				}
...
bool CheckArrow()
{
	...
	{
		if ( (Left!=ITEM_BOW+7) || (CharacterMachine->Equipment[EQUIPMENT_WEAPON_LEFT].Durability <= 0) ) //CustomArrow
		{
			ReloadArrow ();
			return false;
		}
	...
	{
		if ( (Right!=ITEM_BOW+15) || (CharacterMachine->Equipment[EQUIPMENT_WEAPON_RIGHT].Durability <= 0) ) //CustomArrow
		{
			ReloadArrow ();
			return false;
		}
...
bool SkillElf(CHARACTER *c,ITEM *p)
{
	...
	for(int i=0;i<p->SpecialNum;i++)
	{
		...
		if(
			...
			&& (AT_SKILL_MANY_ARROW_UP <= CharacterAttribute->Skill[Hero->CurrentSkill] && CharacterAttribute->Skill[Hero->CurrentSkill] <= AT_SKILL_MANY_ARROW_UP+4))
	...
	case MOVEMENT_OPERATE:
		if(max(abs(( Hero->PositionX)-TargetX),abs(( Hero->PositionY)-TargetY))<=1)
		{
			...
			else
			{
				...
				if(Sit)
				{
					if((!c->SafeZone) && (c->Helper.Type == MODEL_HELPER+37 || c->Helper.Type == MODEL_HELPER+2 || c->Helper.Type == MODEL_HELPER+3 || c->Helper.Type == MODEL_HELPER+4 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6))
						return;
...
bool CheckCommand(char *Text, bool bMacroText )
{
	...
	//g_csItemOption.CheckRenderOptionHelper ( Text ); //fix crash /xoado
...
bool CheckTarget(CHARACTER *c)
{
	//if (g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Started == 1)
	//{
	//	if(SelectedCharacter == FindCharacterIndex(Hero->Key)) return true;
	//}
	...
	case MASTER_SKILL_ADD_INFINITY_ARROW_IMPROVED:
		{
			...
			if(g_isCharacterBuff((&Hero->Object), eBuff_InfinityArrow)==false && g_isCharacterBuff((&Hero->Object), EFFECT_INFINITY_ARROW_IMPROVED)==false)
			{
				...
				if((c->Helper.Type == MODEL_HELPER+37) 
					...
					|| gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6
					|| gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 7
...
void AttackKnight(CHARACTER *c, int Skill, float Distance)
{
		...
		) )
    {
		...
		if(g_csItemOption.Special_Option_Check() == false && (
			...
			(AT_SKILL_POWER_SLASH_UP<= Skill && AT_SKILL_POWER_SLASH_UP + 5 > Skill)))
			Success = false;
        ...
        if ( Success )
        {
            ...
            case MASTER_SKILL_ADD_POWER_SLASH_IMPROVED:
                if ( (c->Helper.Type<MODEL_HELPER+2 || c->Helper.Type>MODEL_HELPER+4 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6) && c->Helper.Type != MODEL_HELPER+37)
                {
                    o->Angle[2] = CreateAngle(o->Position[0],o->Position[1],c->TargetPosition[0],c->TargetPosition[1]);
...
//									SetPlayerMagic(c);
						...
						else if((c->Helper.Type>=MODEL_HELPER+2) && (c->Helper.Type<=MODEL_HELPER+4) || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6)
						{
            ...
            case MASTER_SKILL_ADD_ELECTRIC_SPARK_IMPROVED:
                if ( CheckTile( c, o, Distance ) )
                {
                    ...
                    {
                        if ( (c->Helper.Type==MODEL_HELPER+ 4 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6) && !c->SafeZone )
                        {
                    ...
                    else if ( Skill==AT_SKILL_DARK_HORSE 
						|| Skill == MASTER_SKILL_ADD_EARTHQUAKE_IMPROVED
						|| Skill == MASTER_SKILL_ADD_EARTHQUAKE_ENHANCED
						|| (AT_SKILL_ASHAKE_UP <= Skill && Skill <= AT_SKILL_ASHAKE_UP+4))
                    {
...
{
	if ((MouseOnWindow || !SEASON3B::CheckMouseIn(0, 0, BGetScreenWidth(), 429 + DisplayHeightExt)) && MouseLButtonPush)//==Fix Size Test Wide
	{
 		MouseRButtonPop = false;
 		MouseRButtonPush= false;
 		MouseRButton	= false;
	...
	if(!EnableFastInput)
	{
		...
		else if((MouseRButtonPush || MouseRButton || g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Stage > 1) && !(MouseLButtonPush || MouseLButton) && !c->Movement)
		{
			if ( Skill==AT_SKILL_BLAST_HELL )
			{
			...
			if(g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Stage == 0) SEASON3B::CNewUIInventoryCtrl::BackupPickedItem();
			MouseRButtonPush = false;
			Success = true;
			if (g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Stage > 1) g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Stage--;
		}
        if( 
		...
		if (g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Stage > 0 && SelectedCharacter != -1)
		{
...
int ExecuteSkill(CHARACTER* c, int Skill, float Distance)
{
	...
	if (
		g_pOption->IsAutoAttack() || g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Started == 1
		&& gMapManager.WorldActive != WD_6STADIUM
		//&& gMapManager.InChaosCastle() == false || g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Started == 1)
		&& gMapManager.InChaosCastle() == false)
	{
				...
				if(( LoadingWorld)==0 && Hero->JumpTime==0)
				{
					...
					if(Success)
					{
                            ...
                            ( ( CharacterMachine->Equipment[EQUIPMENT_HELPER].Type>=ITEM_HELPER+2  && CharacterMachine->Equipment[EQUIPMENT_HELPER].Type<=ITEM_HELPER+3 ) ) )
						{
							...
							|| gCustomPet2.GetInfoPetType(CharacterMachine->Equipment[EQUIPMENT_HELPER].Type) == 5
		...
		if(!MouseOnWindow && false == g_pNewUISystem->CheckMouseUse())
		{
				...
				) )
			{
				...
				if ( !pPickedItem && RightType==-1 &&
					...
					else if ( SelectedOperate != -1 && ( c->SafeZone || ( c->Helper.Type<MODEL_HELPER+2 || c->Helper.Type>MODEL_HELPER+4 || c->Helper.Type != MODEL_HELPER+37 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6)))
					{
...
void SelectObjects()
{
    ...
    if (g_pOption->IsAutoAttack() && gMapManager.WorldActive != WD_6STADIUM && gMapManager.InChaosCastle()==false || g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Started == 1)
    {
		...
		else
		{
			...
			if ( Attacking!=-1 )
			{
				if ( MouseLButton || MouseLButtonPush || MouseRButton || MouseRButtonPush || g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Stage || Hero->Dead>0 )
				{
					SelectedCharacter = -1;
				}
			}
	...
	SelectedNpc       = -1;
	SelectedOperate   = -1;
	if (g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Started == 1 && SceneFlag != 4)
	{
		if (SEASON3B::CNewUIInventoryCtrl::GetPickedItem() == NULL) SelectedItem = SelectItem();
		goto NextSelectObj;
	}
...
void SendMacroChat(char *Text)
{
	...
	{
		if( ( Hero->Helper.Type<MODEL_HELPER+2 || Hero->Helper.Type>MODEL_HELPER+4 || gCustomPet2.GetInfoPetType(Hero->Helper.Type - 1171) == 6) || Hero->SafeZone)
			CheckChatText(Text);
...
bool IsCanBCSkill(int Type)
{
	...
	if (gProtect.m_MainInfo.EnableCsSkillsAllMaps) return true;
...
bool CheckSkillUseCondition ( OBJECT* o, int Type )
{
	...
	if (g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Started == 1) return true;
...
void RenderHPBar()
{
	...
	char    SD[100];
	char    Name[100];
	if (gProtect.m_MainInfo.OnHealBar != 1)  return;
	if (mShowHPBar)
	{
		for (int j = 0; j < MAX_CHARACTERS_CLIENT; ++j)
		{
			...
			if (o->Kind == c->InfoHealBar.Type)
			{
				...
				EnableAlphaTest();
				glColor4f(0.f, 0.f, 0.f, 0.5f);
				RenderColor((float)(ScreenX + 1), (float)(ScreenY + 1), Width + 4.f, Height);
				if (o->Kind == KIND_PLAYER)
				{
					RenderColor((float)(ScreenX + 1), (float)(ScreenY + 7), Width + 4.f, Height);
				}
				EnableAlphaBlend();
				glColor3f(0.2f, 0.0f, 0.0f);
				RenderColor((float)ScreenX, (float)ScreenY, Width + 4.f, 5.f);
				if (o->Kind == KIND_PLAYER)
				{
					RenderColor((float)(ScreenX), (float)(ScreenY + 6), Width + 4.f, Height);
				}
				glColor3f(50.f / 255.f, 10 / 255.f, 0.f);
				RenderColor((float)(ScreenX + 2), (float)(ScreenY + 2), Width, 1.f);
				if (o->Kind == KIND_PLAYER)
				{
					RenderColor((float)(ScreenX + 2), (float)(ScreenY + 8), Width + 4.f, Height);
				}
				glColor3f(250.f / 255.f, 10 / 255.f, 0.f);
				RenderColor((float)(ScreenX + 3), (float)(ScreenY + 2), (Width * c->InfoHealBar.Life) / 100, Height - 3);
				if (o->Kind == KIND_PLAYER)
				{
					glColor3f(151 / 255.f, 114 / 255.f, 0.f);
					RenderColor((float)(ScreenX + 3), (float)(ScreenY + 8), (Width * c->InfoHealBar.Life) / 100, Height - 3);
				}
				::EnableAlphaTest();
				::glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
				//if ((MouseX >= ScreenX && MouseX < ScreenX + Width && MouseY >= ScreenY - 2 && MouseY < ScreenY + 6))
				{
					...
					if (o->Kind == KIND_MONSTER)
					{
						g_pRenderText->RenderText(ScreenX + 6, ScreenY + 0.5, TextLV, Width + 6, 0, RT3_SORT_LEFT); //Level
					}
	...
	else
	{
			...
			CHARACTER* c = &CharactersClient[SelectedCharacter]; //SelectedCharacter
			OBJECT* o = &c->Object;
			if (!c || c == Hero || !o->Live || c->InfoHealBar.Life < 1) return;
			if (o->Kind == c->InfoHealBar.Type)
			{
				OBJECT* o = &c->Object;
				vec3_t      Position;
...
void RenderCursor()
{
	...
	{
		if(CheckAttack() && !MouseOnWindow && g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Stage == 0)
        {
...
bool IsIllegalMovementByUsingMsg(const char *szChatText)
{
	...
	//===Move Icarus
	if (gCustomPet2.GetInfoPetType(pEquipedWingType - 1171) == 5 ||  gCustomPet2.GetInfoPetType(pEquipedWingType - 1171)==6)
	{
		bCantFly = false;
	}
