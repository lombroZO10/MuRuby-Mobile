...
static TerrainPerfSnapshot g_terrainPerfSnapshot = {};
...
TerrainPerfSnapshot ConsumeTerrainPerfSnapshot()
{
    const TerrainPerfSnapshot snapshot = g_terrainPerfSnapshot;
    g_terrainPerfSnapshot = {};
    return snapshot;
}
...
bool OpenTerrainHeightNew(const char* strFilename)
{
	...
	int iBytes = 0;
	if(false == ReadWholeFileBytes(fp, pbyData, iBytes))
	{
		fclose(fp);
		return false;
	}
	...
	const size_t dwMinimumBytes =
		4u + sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) +
		((size_t)TERRAIN_SIZE * (size_t)TERRAIN_SIZE * 3u);
	if((size_t)iBytes < dwMinimumBytes)
	{
		delete [] pbyData;
		return false;
	}
	...
	const size_t dwHeightBytes = (size_t)TERRAIN_SIZE * (size_t)TERRAIN_SIZE * 3u;
	if((size_t)dwCurPos + dwHeightBytes > (size_t)iBytes)
	{
		delete [] pbyData;
		return false;
	}
...
extern int TerrainIndex1;
extern int TerrainIndex2;
extern int TerrainIndex3;
extern int TerrainIndex4;
...
static std::vector<int> s_terrainBatchTouchedKeys;

static inline int TerrainBatch_MakeLookupKey(const int textureId, const TerrainBlendState blendState)
{
    return ((int)blendState << 16) | ((int)((uint16_t)textureId));
}

static inline void TerrainBatch_Begin()
{
    if (s_terrainBatchLookup.empty())
    {
        s_terrainBatchLookup.assign(3 * 65536, -1);
    }
    for (const int key : s_terrainBatchTouchedKeys)
    {
        s_terrainBatchLookup[key] = -1;
    }
    s_terrainBatchTouchedKeys.clear();

    for (TerrainBatchBucket& bucket : s_terrainBatchBuckets)
    {
        bucket.quadCount = 0;
        bucket.verts.clear();
    }

    for (int i = 0; i < (int)s_terrainBatchBuckets.size(); ++i)
    {
        const TerrainBatchBucket& bucket = s_terrainBatchBuckets[i];
        const int key = ((int)bucket.blendState << 16) | (int)bucket.textureId;
        s_terrainBatchLookup[key] = i;
        s_terrainBatchTouchedKeys.push_back(key);
    }
    s_terrainBatchActive = true;
}

static TerrainBatchBucket& TerrainBatch_GetBucket(int textureId, TerrainBlendState blendState)
{
    const uint16_t tex = (uint16_t)textureId;
    const uint8_t blend = (uint8_t)blendState;
    if (s_terrainBatchLookup.empty())
    {
        s_terrainBatchLookup.assign(3 * 65536, -1);
    }

    const int key = TerrainBatch_MakeLookupKey(textureId, blendState);
    const int bucketIndex = s_terrainBatchLookup[key];
    if (bucketIndex >= 0 && bucketIndex < (int)s_terrainBatchBuckets.size())
    {
        return s_terrainBatchBuckets[bucketIndex];
    }

    TerrainBatchBucket bucket;
    bucket.textureId = tex;
    bucket.blendState = blend;
    bucket.quadCount = 0;
    bucket.verts.reserve(256 * 36);
    s_terrainBatchBuckets.push_back(std::move(bucket));
    s_terrainBatchLookup[key] = (int)s_terrainBatchBuckets.size() - 1;
    s_terrainBatchTouchedKeys.push_back(key);
    return s_terrainBatchBuckets.back();
}

static void TerrainBatch_AddQuad(int textureId, TerrainBlendState blendState, uint8_t primType,
                                 const float pos[4][3], const float uv[4][2], const float col[4][4])
{
    (void)primType;
    TerrainBatchBucket& bucket = TerrainBatch_GetBucket(textureId, blendState);
    for (int i = 0; i < 4; i++)
    {
        bucket.verts.push_back(pos[i][0]);
        bucket.verts.push_back(pos[i][1]);
        bucket.verts.push_back(pos[i][2]);
        bucket.verts.push_back(col[i][0]);
        bucket.verts.push_back(col[i][1]);
        bucket.verts.push_back(col[i][2]);
        bucket.verts.push_back(col[i][3]);
        bucket.verts.push_back(uv[i][0]);
        bucket.verts.push_back(uv[i][1]);
    }
    ++bucket.quadCount;
}

static void TerrainBatch_CollectFace(int Texture, TerrainBlendState blendState, uint8_t primType)
{
    float pos[4][3], uv[4][2], col[4][4];
    for (int i = 0; i < 4; i++)
    {
        pos[i][0] = TerrainVertex[i][0];
        pos[i][1] = TerrainVertex[i][1];
        pos[i][2] = TerrainVertex[i][2];
        uv[i][0] = TerrainTextureCoord[i][0];
        uv[i][1] = TerrainTextureCoord[i][1];
    }

    int indices[4] = { TerrainIndex1, TerrainIndex2, TerrainIndex3, TerrainIndex4 };
    if (blendState == TB_ALPHA_TEST)
    {
        for (int i = 0; i < 4; i++)
        {
            col[i][0] = PrimaryTerrainLight[indices[i]][0];
            col[i][1] = PrimaryTerrainLight[indices[i]][1];
            col[i][2] = PrimaryTerrainLight[indices[i]][2];
            col[i][3] = TerrainMappingAlpha[indices[i]];
        }
    }
    else if (blendState == TB_ALPHA_BLEND)
    {
        for (int i = 0; i < 4; i++)
        {
            float a = TerrainMappingAlpha[indices[i]];
            col[i][0] = a;
            col[i][1] = a;
            col[i][2] = a;
            col[i][3] = 1.0f;
    ...
    TerrainBatch_AddQuad(BITMAP_MAPTILE + Texture, blendState, primType, pos, uv, col);
}

static void TerrainBatch_CollectGrassFace(int textureId, TerrainBlendState blendState)
{
    float pos[4][3], uv[4][2], col[4][4];
    const int indices[4] = { TerrainIndex1, TerrainIndex2, TerrainIndex3, TerrainIndex4 };
    for (int i = 0; i < 4; ++i)
    {
        pos[i][0] = TerrainVertex[i][0];
        pos[i][1] = TerrainVertex[i][1];
    ...
    TerrainBatch_AddQuad(textureId, blendState, 0, pos, uv, col);
}

static void TerrainBatch_Flush()
{
    bool hasQuads = false;
    for (const TerrainBatchBucket& bucket : s_terrainBatchBuckets)
    {
        if (bucket.quadCount > 0)
        {
            hasQuads = true;
            break;
        }
    ...
    std::sort(s_terrainBatchBuckets.begin(), s_terrainBatchBuckets.end(),
        [](const TerrainBatchBucket& a, const TerrainBatchBucket& b)
        {
    ...
    for (const TerrainBatchBucket& bucket : s_terrainBatchBuckets)
    {
        if (bucket.quadCount <= 0 || bucket.verts.empty())
        {
            continue;
        }
...
static inline bool IsTerrainHeavyOverlayVisible()
{
#if defined(__ANDROID__) || defined(MU_IOS)
...
static inline int GetTerrainFrustumPaddingTiles()
{
#if defined(__ANDROID__) || defined(MU_IOS)
	...
	int padding = 18 + static_cast<int>(extraWide * 32.0f + extraNarrow * 46.0f + (edgeZoomScale - 1.0f) * 26.0f + 0.5f);
	if (padding < 18) padding = 18;
	if (padding > 56) padding = 56;
	return padding;
#else
	return 0;
#endif
...
static inline float GetTerrainTileFrustumRange()
{
#if defined(__ANDROID__) || defined(MU_IOS)
	const float width = (OpenglWindowHeight > 0)
		? static_cast<float>(OpenglWindowWidth)
	...
	float extraNarrow = (4.0f / 3.0f) - aspect;
	if (extraNarrow < 0.0f)
	{
		extraNarrow = 0.0f;
	}
	...
	const float zoomRatio = zoom / 1000.0f;
	float range = -55.0f - extraWide * 140.0f - extraNarrow * 120.0f - (zoomRatio - 1.0f) * 120.0f;
	if (range < -180.0f)
	{
		range = -180.0f;
	}
...
static inline float GetTerrainBlockFrustumRange(float baseRange)
{
#if defined(__ANDROID__) || defined(MU_IOS)
	const float width = (OpenglWindowHeight > 0)
		? static_cast<float>(OpenglWindowWidth)
	...
	const float zoomRatio = zoom / 1000.0f;
	float range = baseRange - 70.0f - extraWide * 220.0f - extraNarrow * 180.0f - (zoomRatio - 1.0f) * 140.0f;
	if (range < (baseRange - 260.0f))
	{
		range = baseRange - 260.0f;
	}
	return range;
#else
	return baseRange;
...
void CreateFrustrum2D(vec3_t Position)
{
    ...
    else
    {
        ...
        {
            Width = screenWidth/640;//640.0f// * 0.1f;
        ...
        else
        {
            ...
            case 0:
				...
				{
					Width = screenWidth/640 * 9.1f * 0.404998f;
				}
				else if(g_Direction.m_CKanturu.IsMayaScene() )
				{
					Width = screenWidth/500.f * 10.0f * 0.115f;
				}
				else
				{
					Width = screenWidth/500.f;
				}
                ...
                break;
            case 1:
                Width = screenWidth/500+0.1f;// * 0.1f;
 ...
 #if defined(__ANDROID__) || defined(MU_IOS)
    {
        ...
        if (g_androidZoomOverride > 0.0f)
        {
            ...
            if (zoomScale > 1.01f)
            {
                WidthFar *= zoomScale;
                WidthNear *= zoomScale;
                CameraViewFar *= zoomScale;
                CameraViewNear *= zoomScale;
                CameraViewTarget *= zoomScale;
            }
        }

        if (SceneFlag == MAIN_SCENE && !CameraTopViewEnable)
        {
            const float minStableCameraViewFar = 4600.0f;
            if (CameraViewFar < minStableCameraViewFar)
            {
                const float stableScale = minStableCameraViewFar / (std::max)(CameraViewFar, 1.0f);
                WidthFar *= stableScale;
                WidthNear *= stableScale;
                CameraViewNear *= stableScale;
                CameraViewTarget *= stableScale;
                CameraViewFar = minStableCameraViewFar;
            }
        }
    }
#endif
...
void CreateFrustrum(float xAspect, float yAspect, vec3_t position)
{
	...
	const int extraPadding = GetTerrainFrustumPaddingTiles();
	FrustrumBoundMinX -= extraPadding;
	FrustrumBoundMinY -= extraPadding;
	FrustrumBoundMaxX += extraPadding;
	FrustrumBoundMaxY += extraPadding;
...
void RenderTerrainBlock(float xf,float yf,int xi,int yi,bool EditFlag)
{
	...
	for(int i=0;i<4;i+=lodi)
	{
		...
		{
            if(TestFrustrum2D(xf+0.5f,yf+0.5f,GetTerrainTileFrustumRange()) || CameraTopViewEnable || Camera3DEnable || Camera3DSet || gMapManager.InBattleCastle() == true)
			{
                if (TerrainFlag == TERRAIN_MAP_GRASS)
                {
                    ++g_terrainPerfSnapshot.grassTiles;
                }
                else
                {
                    ++g_terrainPerfSnapshot.normalTiles;
                }
...
void RenderTerrainFrustrum(bool EditFlag)
{
    ...
    for(;yi<=FrustrumBoundMaxY;yi+=4,yf+=4.f)
	{
        ...
        {
            if(TestFrustrum2D(xf+2.f,yf+2.f,GetTerrainBlockFrustumRange(g_fFrustumRange)) || CameraTopViewEnable || Camera3DEnable || Camera3DSet || gMapManager.InBattleCastle() == true)
			{
                if (TerrainFlag == TERRAIN_MAP_GRASS)
                {
                    ++g_terrainPerfSnapshot.grassBlocks;
                }
                else
                {
                    ++g_terrainPerfSnapshot.normalBlocks;
                }
#ifdef PJH_NEW_SERVER_SELECT_MAP
				if (gMapManager.WorldActive == WD_73NEW_LOGIN_SCENE)
#else
...
void RenderTerrainBlock_After(float xf,float yf,int xi,int yi,bool EditFlag)
{
	...
	for(int i=0;i<4;i+=lodi)
	{
		...
		for(int j=0;j<4;j+=lodi)
		{
            if(TestFrustrum2D(xf+0.5f,yf+0.5f,GetTerrainTileFrustumRange()) || CameraTopViewEnable || Camera3DEnable || Camera3DSet || gMapManager.InBattleCastle() == true)
			{
                ++g_terrainPerfSnapshot.afterTiles;
...
void RenderTerrainFrustrum_After(bool EditFlag)
{
    ...
    for(;yi<=FrustrumBoundMaxY;yi+=4,yf+=4.f)
	{
		...
		for(;xi<=FrustrumBoundMaxX;xi+=4,xf+=4.f)
        {
            if(TestFrustrum2D(xf+2.f,yf+2.f,GetTerrainBlockFrustumRange(-80.f)) || CameraTopViewEnable || Camera3DEnable || Camera3DSet || gMapManager.InBattleCastle() == true)
			{
                ++g_terrainPerfSnapshot.afterBlocks;
...
void RenderTerrain(bool EditFlag)
{
	...
	if ( !EditFlag )
	{
		if (g_pNewUISystem->GetUI_NewOptionWindow()->OnOffGrap[g_pNewUISystem->GetUI_NewOptionWindow()->eRenderTerrain] || SceneFlag != MAIN_SCENE) EnableAlphaTest();
    ...
    g_terrainPerfSnapshot.renderTicks += static_cast<unsigned long long>(MU_MobilePerfNow() - renderTicksStart);
}
