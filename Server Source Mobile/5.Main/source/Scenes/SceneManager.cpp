...
{
#if !defined(__ANDROID__) && !defined(MU_IOS)
    // Windows only: nếu VSync đang bật và target = refresh rate thì bỏ software cap,
    // để VSync tự handle. Trên Android IsVSyncEnabled() luôn trả true (stub) nên
    // điều kiện này sẽ luôn override về -1, phá vỡ software cap.
...
#if defined(__ANDROID__) || defined(MU_IOS)
    constexpr int FPS_OVERLAY_WIDTH = 110;
    constexpr int FPS_OVERLAY_MARGIN_RIGHT = 8;
    const int x = 640 - FPS_OVERLAY_WIDTH - FPS_OVERLAY_MARGIN_RIGHT;
    g_pRenderText->RenderText(x, y, szLine, FPS_OVERLAY_WIDTH, 0, RT3_SORT_RIGHT);
#else
...
static void ManageMainSceneAudio()
{
    ...
    static int s_lastWorld = -1;
    static int s_lastHeroTile = -1;
    static bool s_lastSafeZone = false;
    static double s_lastAudioUpdate = 0.0;

    const int world = gMapManager.WorldActive;
    const int heroTile = HeroTile;
    const bool safeZone = (Hero != nullptr) ? Hero->SafeZone : false;
    const bool stateChanged = (world != s_lastWorld) || (heroTile != s_lastHeroTile) || (safeZone != s_lastSafeZone);
    const bool timeSliceReached = ((WorldTime - s_lastAudioUpdate) >= 120.0);

    if (!stateChanged && !timeSliceReached)
    {
        return;
    }

    s_lastAudioUpdate = WorldTime;
    s_lastWorld = world;
    s_lastHeroTile = heroTile;
    s_lastSafeZone = safeZone;
...
void MainScene(HDC hDC)
{
    ...
    catch (const std::exception& e)
    {
        ...
        sprintf_s(errorMsg, sizeof(errorMsg),
            "Exception in MainScene(scene=%d world=%d): %s",
            static_cast<int>(SceneFlag),
            static_cast<int>(gMapManager.WorldActive),
            e.what());
...
void RenderScene(HDC hDC)
{
    ...
    catch (const std::exception& e)
    {
        ...
        char errorMsg[256];
        sprintf_s(errorMsg, sizeof(errorMsg),
            "Exception in RenderScene(scene=%d world=%d): %s",
            static_cast<int>(SceneFlag),
            static_cast<int>(gMapManager.WorldActive),
            e.what());
        OutputDebugStringA(errorMsg);
