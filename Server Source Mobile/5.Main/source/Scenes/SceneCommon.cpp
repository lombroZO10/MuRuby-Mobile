...
float GetSceneFrustumScale(int viewportWidth, int viewportHeight)
{
    ...
    const float safeViewportWidth = (viewportWidth > 1) ? static_cast<float>(viewportWidth) : 1.0f;
    const float safeViewportHeight = (viewportHeight > 1) ? static_cast<float>(viewportHeight) : 1.0f;
    const float safeRateX = (g_fScreenRate_x > 0.001f) ? g_fScreenRate_x : 0.001f;
    const float safeRateY = (g_fScreenRate_y > 0.001f) ? g_fScreenRate_y : 0.001f;
    const float physicalAspect = (safeViewportWidth * safeRateX) / (safeViewportHeight * safeRateY);
    const float frustumScale = physicalAspect / (4.0f / 3.0f);
    return (frustumScale > 1.0f) ? frustumScale : 1.0f;
#else
