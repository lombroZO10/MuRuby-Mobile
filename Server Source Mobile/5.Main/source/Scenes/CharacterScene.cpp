...
static void SetupCharacterSceneViewport(int& outWidth, int& outHeight)
{
    ...
    CreateFrustrum((float)outWidth / 640.0f, (float)(outHeight - 50) / 480.0f, pos);
...
static bool ShouldRunCharacterSelectionPass()
{
    static int sLastMouseX = -1;
    static int sLastMouseY = -1;
    static int sIdleFrames = 0;

    const bool mouseMoved = (MouseX != sLastMouseX) || (MouseY != sLastMouseY);
    if (mouseMoved)
    {
        sLastMouseX = MouseX;
        sLastMouseY = MouseY;
...
static void RenderCharacterScene3D()
{
    ...
    if (!CUIMng::Instance().IsCursorOnUI() && ShouldRunCharacterSelectionPass())
        SelectObjects();
