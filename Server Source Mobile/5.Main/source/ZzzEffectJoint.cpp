...
void AppendJointQuadVertex(std::vector<float>& vertices, const vec3_t position, float u, float v, const float color[4])
{
...
void QueueJointQuad(JointQuadBatch& batch,
    const vec3_t v0,
    const vec3_t v1,
    const vec3_t v2,
    const vec3_t v3,
    ...
    const float color[4])
{
    AppendJointQuadVertex(batch.vertices, v0, u0, v0_uv, color);
    AppendJointQuadVertex(batch.vertices, v1, u1, v1_uv, color);
    AppendJointQuadVertex(batch.vertices, v2, u2, v2_uv, color);
    AppendJointQuadVertex(batch.vertices, v3, u3, v3_uv, color);
    ++batch.quadCount;
}
...
void GetJointQuadColor(const JOINT* o, int tailIndex, float outColor[4])
{
    outColor[0] = o->Light[0];
    outColor[1] = o->Light[1];
    outColor[2] = o->Light[2];
    outColor[3] = 1.f;

    if (o->Type == BITMAP_JOINT_THUNDER + 1 && o->SubType == 0)
    {
        const int activeTail = static_cast<int>(o->Light[2]);
        if (activeTail == tailIndex)
        {
            const float light = o->Light[2] - tailIndex;
            outColor[0] = light;
            outColor[1] = light;
            outColor[2] = light;
        }
        else if (activeTail < tailIndex)
        {
            outColor[0] = 0.f;
            outColor[1] = 0.f;
            outColor[2] = 0.f;
...
{
#if defined(__ANDROID__) || defined(MU_IOS)
	if (ShouldThrottleAdaptiveEffectSpawn(ADAPTIVE_EFFECT_JOINT, Type, Position, SubType, Scale, Target))
	{
		return;
	}
#endif

	//g_CreateJoint_lock->lock();
	for (int i = 0; i < MAX_JOINTS; i++)
	{

		JOINT* o = &Joints[i];
		//== Fix Effect
		if ((i >= MAX_JOINTS - 2) && o->Live)
		{
			o->Live = false;
		}
		if (!o->Live)
	...
	case BITMAP_JOINT_SPIRIT2:
		...
		if (0 == o->SubType || o->SubType == 5 || o->SubType == 19)
		{
			if (o->Scale == 80.f)
			{
				...
				{
					if (o->Target == &Hero->Object) AttackCharacterRange(o->Skill, o->Position, 150.f, o->Weapon, o->PKKey, o->m_bySkillSerialNum);
				}
    ...
    for (int i = 0; i < MAX_JOINTS; i++)
    {
        ...
        if (o->Live && o->NumTails > 0 && o->RenderFace != 0)
        {
            ...
            if (o->Type == MODEL_SPEARSKILL)
            {
                ...
                case 10:
                {
                    const int alphaLifetime = (o->LifeTime < 20) ? static_cast<int>(o->LifeTime) : 20;
                    fAlpha = (float)alphaLifetime * 0.05f;
            ...
            {
                const int alphaLifetime = (o->LifeTime < 20) ? static_cast<int>(o->LifeTime) : 20;
                float fAlpha = (float)alphaLifetime * 0.1f;
                glColor3f(fAlpha * o->Light[0], fAlpha * o->Light[1], fAlpha * o->Light[2]);
            }
