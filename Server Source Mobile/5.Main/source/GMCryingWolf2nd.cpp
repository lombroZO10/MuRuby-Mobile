...
bool M34CryingWolf2nd::RenderCryingWolf2ndObjectVisual(OBJECT* pObject, BMD* pModel)
{
	...
	{
	case 2:
		if (ShouldEmitStableAmbientFrame(pObject, 3, 2))
		{
			Vector(1.f, 1.f, 1.f, Light);
			CreateParticle(BITMAP_SMOKE, pObject->Position, pObject->Angle, Light, 21, pObject->Scale, pObject);
		}
		break;
	case 3:
		if (ShouldEmitStableAmbientFrame(pObject, 4, 3)) {
			Vector(1.f, 1.f, 1.f, Light);
			CreateParticle(BITMAP_TRUE_FIRE, pObject->Position, pObject->Angle, Light, 5, pObject->Scale, pObject);
		}
		break;
	case 5:
	{
		Vector(1.f, 1.f, 1.f, Light);
		if (ShouldEmitStableAmbientFrame(pObject, 2, 51)) {
			if ((int)((pObject->Timer += FPS_ANIMATION_FACTOR) + 2) % 4 == 0)
			{
				CreateParticle(BITMAP_ADV_SMOKE + 1, pObject->Position, pObject->Angle, Light, 0, 1.f, pObject);
				CreateParticle(BITMAP_ADV_SMOKE, pObject->Position, pObject->Angle, Light, 0, 1.f, pObject);
			}
		}
		if (ShouldEmitStableAmbientFrame(pObject, 2, 52)) {
			if ((int)(pObject->Timer += FPS_ANIMATION_FACTOR) % 4 == 0)
			{
				CreateParticle(BITMAP_CLOUD, pObject->Position, pObject->Angle, Light, 6, 1.f, pObject);
				CreateParticle(BITMAP_ADV_SMOKE, pObject->Position, pObject->Angle, Light, 1, 1.f, pObject);
			}
		}
	}
	break;
