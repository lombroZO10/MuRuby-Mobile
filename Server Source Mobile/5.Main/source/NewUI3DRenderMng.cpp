...
bool GetUi3DScissorRect(float zOrder, int& outX, int& outY, int& outWidth, int& outHeight)
{
	...
	const float logicalLeft = static_cast<float>(GetScreenWidth());
	const float logicalRight = static_cast<float>(GetWindowsX());
	if (logicalLeft <= 0.0f || logicalLeft >= logicalRight)
	{
		return false;
	}

	outX = static_cast<int>(logicalLeft * g_fScreenRate_x);
	outY = 0;
...
bool SEASON3B::CNewUI3DCamera::Render()
{
	...
	int scissorWidth = 0;
	int scissorHeight = 0;
	const bool useScissor = GetUi3DScissorRect(m_fZOrder, scissorX, scissorY, scissorWidth, scissorHeight);
	...
	//DAT Fix Size Item Wide Inventory
	float SizeItem = 1.0f;
	//if ((int)m_Resolution >= 3) { SizeItem += 0.70f; } // GỐC
	//if ((int)m_Resolution >= 3) { SizeItem += 0.25f; }
    ...
    gluPerspective2(SizeItem, (float)(m_uiWidth)/(float)(m_uiHeight), RENDER_ITEMVIEW_NEAR, RENDER_ITEMVIEW_FAR);
