...
void CProtocolSend::RecvJoinServerNew(PMSG_CONNECT_CLIENT_RECV* pMsg )
{
	...
	g_ErrorReport.Write("[AndroidLogin] RecvJoinServerNew result=0x%02X logIn=%d heroKey=%d\r\n", pMsg->result, LogIn, HeroKey);
...
{
	g_ErrorReport.Write("[AndroidLogin] RecvLoginNew result=0x%02X\r\n", pMsg->result);
	switch (pMsg->result)
	{
		case 0x20:
