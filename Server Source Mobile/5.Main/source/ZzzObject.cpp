...
namespace
{
    enum class AdaptiveDistanceBucket : int
    {
        Near = 0,
        Mid = 1,
        Far = 2,
    ...
    constexpr float kObjectNearDistance = 1250.0f;
    constexpr float kObjectMidDistance = 2300.0f;
    constexpr float kObjectVeryFarDistance = 3400.0f;
    constexpr float kObjectHardCullDistance = 4300.0f;
    constexpr float kObjectNearAdaptiveMinDistance = 850.0f;
    constexpr float kObjectDefaultCameraDistance = 1000.0f;
    constexpr float kObjectMinZoomScale = 0.70f;
    constexpr float kObjectMaxZoomScale = 1.75f;

    struct AdaptiveObjectDistanceThresholds
    {
        float nearSq = kObjectNearDistance * kObjectNearDistance;
        float midSq = kObjectMidDistance * kObjectMidDistance;
        float veryFarSq = kObjectVeryFarDistance * kObjectVeryFarDistance;
        float hardCullSq = kObjectHardCullDistance * kObjectHardCullDistance;
    };

    uint32_t g_objectAdaptiveFrameId = 0;
    ObjectPerfSnapshot g_objectPerfSnapshot = {};

    struct ObjectRenderLodContext
    {
        bool active = false;
        bool allowSupplementalPasses = true;
        bool basePassRendered = false;
    };

    ObjectRenderLodContext g_objectRenderLodContext = {};

    inline void ResetObjectRenderLodContext()
    {
        g_objectRenderLodContext = {};
    }

    inline bool IsSupplementalObjectRenderFlag(const int renderFlag)
    {
        const int supplementalMask =
            RENDER_COLOR |
            RENDER_BRIGHT |
            RENDER_CHROME |
            RENDER_CHROME2 |
            ...
            RENDER_METAL |
            RENDER_OIL;
        return (renderFlag & supplementalMask) != 0;
    }

    inline bool CanMarkObjectBaseRenderPass(const int renderFlag, const float alpha)
    {
        if (alpha < 0.85f)
        {
            return false;
        ...
        const int nonBaseMask =
            ...
            RENDER_WAVE |
            RENDER_SHADOWMAP;
        return (renderFlag & nonBaseMask) == 0;
    }

    inline bool IsObjectAdaptiveEnabled()
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        return (SceneFlag == MAIN_SCENE || SceneFlag == LOG_IN_SCENE) &&
            Hero != nullptr &&
            !CameraTopViewEnable;
#else
        return false;
    ...
    inline bool IsAndroidFpsBoostProfile()
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        return true;
#else
    ...
    inline bool IsAndroidFullObjectVisibilityMode()
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        return true;
#else
    ...
    inline bool IsStadiumStaticWorldObject(const OBJECT* o)
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        return o != nullptr &&
    ...
    inline bool IsStadiumHeavyDecorObject(const OBJECT* o)
    {
        if (!IsStadiumStaticWorldObject(o))
        {
            return false;
        }
    ...
    inline float GetObjectDistanceSq2D(const OBJECT* o)
    {
    ...
    inline float GetObjectZoomScale()
    {
        float distanceTarget = CameraDistanceTarget;
        if (!(distanceTarget > 1.0f))
        {
            distanceTarget = kObjectDefaultCameraDistance;
        }

        const float rawScale = distanceTarget / kObjectDefaultCameraDistance;
        return std::clamp(rawScale, kObjectMinZoomScale, kObjectMaxZoomScale);
    }

    inline AdaptiveObjectDistanceThresholds GetObjectDistanceThresholds()
    {
        const float zoomScale = GetObjectZoomScale();
        const float nearDistance = kObjectNearDistance * zoomScale;
        float midDistance = kObjectMidDistance * zoomScale;
        float veryFarDistance = kObjectVeryFarDistance * zoomScale;
        float hardCullDistance = kObjectHardCullDistance * zoomScale;

        if (IsAndroidFpsBoostProfile())
        {
            midDistance *= 0.82f;
            veryFarDistance *= 0.76f;
            hardCullDistance *= 0.72f;
        }

        AdaptiveObjectDistanceThresholds thresholds;
    ...
    inline bool IsWideSideOverlayVisible()
    {
#if defined(__ANDROID__) || defined(MU_IOS)
    ...
    inline float GetWideZoomFrustumWeight()
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        float zoom = CameraDistanceTarget;
        if (zoom < 800.0f)
        {
            zoom = 800.0f;
        }
        else if (zoom > 1600.0f)
        {
            zoom = 1600.0f;
        }

        return std::clamp((zoom - 1000.0f) / 450.0f, 0.0f, 1.0f);
#else
        return 0.0f;
#endif
    }

    inline float GetObjectBlockFrustumRange();

    inline float GetObjectBlockFrustumRange()
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        const float width = (OpenglWindowHeight > 0)
            ? static_cast<float>(OpenglWindowWidth)
            : static_cast<float>(WindowWidth);
        const float height = (OpenglWindowHeight > 0)
            ? static_cast<float>(OpenglWindowHeight)
            : static_cast<float>(WindowHeight);
        const float aspect = (height > 1.0f) ? (width / height) : (16.0f / 9.0f);
        const float desktopAspect = 4.0f / 3.0f;

        float extraWide = aspect - (16.0f / 9.0f);
        if (extraWide < 0.0f)
        {
            extraWide = 0.0f;
        }
        float extraNarrow = desktopAspect - aspect;
        if (extraNarrow < 0.0f)
        {
            extraNarrow = 0.0f;
        }
        ...
        const float zoomRatio = zoom / 1000.0f;
        const float edgeZoomScale = (zoomRatio >= 1.0f) ? zoomRatio : (1.0f / zoomRatio);
        const float wideZoomWeight = GetWideZoomFrustumWeight();

        float range = -260.0f
            - extraWide * 160.0f
            - extraNarrow * 220.0f
            - (edgeZoomScale - 1.0f) * 150.0f
            - wideZoomWeight * 65.0f;
        if (range < -560.0f)
        {
            range = -560.0f;
        }
    ...
    inline float GetObjectBlockVisibleRange(const bool wasVisible)
    {
        float range = GetObjectBlockFrustumRange();
#if defined(__ANDROID__) || defined(MU_IOS)
        if (wasVisible)
        {
            range -= 20.0f + GetWideZoomFrustumWeight() * 40.0f;
        }
#else
        (void)wasVisible;
#endif
    ...
    inline float GetObjectInstanceFrustumRange(const float baseRange)
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        const float width = (OpenglWindowHeight > 0)
            ? static_cast<float>(OpenglWindowWidth)
        ...
        float range = baseRange
            ...
            - extraWide * 120.0f
            - extraNarrow * 140.0f
            - (edgeZoomScale - 1.0f) * 125.0f
            - wideZoomWeight * 48.0f;
        const float minRange = baseRange - 260.0f;
        if (range < minRange)
        {
            range = minRange;
        }
    ...
    inline float GetObjectExtentBias(const OBJECT* o)
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        if (o == nullptr)
        {
        ...
        float extraBias = dominantExtent * 0.07f;
        if (extentZ > 140.0f)
        {
            extraBias += (extentZ - 140.0f) * (0.045f + wideZoomWeight * 0.04f);
        }
        if (footprintExtent > 120.0f)
        {
            extraBias += (footprintExtent - 120.0f) * (0.015f + wideZoomWeight * 0.015f);
        }
        if (wideZoomWeight > 0.0f)
        {
            extraBias += wideZoomWeight * ((std::min)(extentZ, 480.0f) * 0.045f + 12.0f);
            if (extentZ > footprintExtent * 1.2f)
            {
                extraBias += 10.0f * wideZoomWeight;
            }
        }
    ...
    inline float GetStaticObjectScreenImportance(const OBJECT* o, const float distSq)
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        if (o == nullptr)
        {
        ...
        const float dominantExtent = (std::max)((std::max)(extentX, extentY), extentZ);
        const float footprintExtent = (std::max)(extentX, extentY);
        const float weightedExtent = (std::max)(dominantExtent, footprintExtent * 0.92f);
    ...
    inline bool ShouldCullStaticObjectByImportance(const OBJECT* o,
        ...
        const float distSq,
        const int crowdPressure,
        const float currentFps)
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        if (IsAndroidFullObjectVisibilityMode() ||
            !IsObjectAdaptiveEnabled() || o == nullptr || !IsAdaptiveStaticSceneObject(o))
        {
            return false;
        }

        if (IsPersistentAmbientFireObject(o))
        {
            return false;
        }
        ...
        if (bucket == AdaptiveDistanceBucket::Far)
        {
            if (crowdPressure > 48)
            {
                minImportance = 0.0058f;
            }
            if (crowdPressure > 76 && stressedFrame)
            {
                minImportance = 0.0105f;
            }
            if (crowdPressure > 108 && severeFrame)
            {
                minImportance = 0.0175f;
            }
        }
        else if (bucket == AdaptiveDistanceBucket::Mid)
        {
            if (crowdPressure > 70 && stressedFrame && distSq > nearAdaptiveMinDistanceSq)
            {
                minImportance = 0.0035f;
            }
            if (crowdPressure > 108 && severeFrame && distSq > nearAdaptiveMinDistanceSq * 1.35f)
            {
                minImportance = 0.0072f;
            }
        }

        if (stadiumStaticObject)
        {
            if (bucket == AdaptiveDistanceBucket::Far && crowdPressure > 96)
            {
                minImportance = (std::max)(minImportance, stressedFrame ? 0.0090f : 0.0062f);
            }
            else if (bucket == AdaptiveDistanceBucket::Mid &&
                crowdPressure > 148 &&
                severeFrame &&
            ...
            {
                minImportance = (std::max)(minImportance, 0.0048f);
            }

            if (stadiumHeavyDecorObject)
            {
                if (bucket == AdaptiveDistanceBucket::Far)
                {
                    minImportance = (std::max)(minImportance, stressedFrame ? 0.0115f : 0.0084f);
                }
                else if (bucket == AdaptiveDistanceBucket::Mid &&
                    crowdPressure > 156 &&
                    severeFrame &&
                    distSq > nearAdaptiveMinDistanceSq * 2.0f)
                {
                    minImportance = (std::max)(minImportance, 0.0058f);
                }
            }
        }
    ...
    inline float GetObjectVisibilityRange(const OBJECT* o, const float baseRange)
    {
        float range = GetObjectInstanceFrustumRange(baseRange);
#if defined(__ANDROID__) || defined(MU_IOS)
        range -= GetObjectExtentBias(o);
#else
        (void)o;
#endif
        return range;
    ...
    inline float GetObjectVisibilityRange(const OBJECT* o, const float baseRange, const bool wasVisible)
    {
        float range = GetObjectVisibilityRange(o, baseRange);
#if defined(__ANDROID__) || defined(MU_IOS)
        if (wasVisible)
        {
            const float extentBias = GetObjectExtentBias(o);
            range -= 28.0f + GetWideZoomFrustumWeight() * 38.0f + extentBias * 0.30f;
        }
#else
        (void)wasVisible;
#endif
    ...
    inline AdaptiveDistanceBucket ClassifyObjectDistance(const float distSq, const AdaptiveObjectDistanceThresholds& thresholds)
    {
        if (distSq <= thresholds.nearSq)
        {
            return AdaptiveDistanceBucket::Near;
        }

        if (distSq <= thresholds.midSq)
        {
            return AdaptiveDistanceBucket::Mid;
        }

        return AdaptiveDistanceBucket::Far;
    }

    inline void TrackObjectMoveBucket(const AdaptiveDistanceBucket bucket)
    {
        switch (bucket)
        {
        case AdaptiveDistanceBucket::Near:
            ++g_objectPerfSnapshot.moveNearCandidates;
            break;
        case AdaptiveDistanceBucket::Mid:
            ++g_objectPerfSnapshot.moveMidCandidates;
            break;
        case AdaptiveDistanceBucket::Far:
            ++g_objectPerfSnapshot.moveFarCandidates;
            break;
        }
    }

    inline void TrackObjectRenderBucket(const AdaptiveDistanceBucket bucket)
    {
        switch (bucket)
        {
        case AdaptiveDistanceBucket::Near:
            ++g_objectPerfSnapshot.renderNearCandidates;
            break;
        case AdaptiveDistanceBucket::Mid:
            ++g_objectPerfSnapshot.renderMidCandidates;
            break;
        case AdaptiveDistanceBucket::Far:
            ++g_objectPerfSnapshot.renderFarCandidates;
            break;
        }
    }

    inline bool ShouldRunObjectStride(const OBJECT* o, const uint32_t period, const uint32_t salt)
    {
        if (period <= 1u)
        {
            return true;
        ...
        const uint32_t typeHash = static_cast<uint32_t>(o->Type + 2048) * 73856093u;
        const uint32_t blockHash = static_cast<uint32_t>(o->Block + 1) * 19349663u;
        const uint32_t hash = typeHash ^ blockHash ^ salt;
        return ((g_objectAdaptiveFrameId + hash) % period) == 0u;
    }

    inline bool ShouldReuseObjectAnimationPose(const OBJECT* o, const int select)
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        if (!IsObjectAdaptiveEnabled() ||
            o == nullptr ||
            select != 0 ||
            !o->EnableBoneMatrix ||
        ...
        const AdaptiveObjectDistanceThresholds thresholds = GetObjectDistanceThresholds();
        const AdaptiveDistanceBucket bucket = ClassifyObjectDistance(GetObjectDistanceSq2D(o), thresholds);
        uint32_t period = 1u;
        if (bucket == AdaptiveDistanceBucket::Far)
        {
        ...
        return period > 1u && !ShouldRunObjectStride(o, period, 0x51f2a9u);
#else
        (void)o;
        (void)select;
        return false;
    ...
    inline void ConfigureObjectRenderLodContext(const OBJECT* o,
        const AdaptiveDistanceBucket bucket,
        const int crowdPressure)
    {
        ResetObjectRenderLodContext();
        ...
        const bool nearBucket = (bucket == AdaptiveDistanceBucket::Near);
...
{
    const ObjectPerfSnapshot snapshot = g_objectPerfSnapshot;
    g_objectPerfSnapshot = {};
    return snapshot;
}
...
bool ShouldSkipAdaptiveObjectRenderPass(int renderFlag, float alpha)
{
    ...
    return g_objectRenderLodContext.active &&
        !g_objectRenderLodContext.allowSupplementalPasses &&
        g_objectRenderLodContext.basePassRendered &&
        IsSupplementalObjectRenderFlag(renderFlag);
}

void NotifyAdaptiveObjectRenderPassRendered(int renderFlag, float alpha)
{
    if (!g_objectRenderLodContext.active)
    {
        return;
    }

    if (CanMarkObjectBaseRenderPass(renderFlag, alpha))
    {
        g_objectRenderLodContext.basePassRendered = true;
    }
}
...
void BeginAdaptiveObjectRenderPassBudget(bool allowSupplementalPasses)
{
    ResetObjectRenderLodContext();
    g_objectRenderLodContext.active = true;
    g_objectRenderLodContext.allowSupplementalPasses = allowSupplementalPasses;
}
...
void Draw_RenderObject(OBJECT *o,bool Translate,int Select, int ExtraMon)
{
	...
	{
		if (!g_pNewUISystem->GetUI_NewOptionWindow()->OnOffGrap[g_pNewUISystem->GetUI_NewOptionWindow()->eBMDMonter] && SceneFlag == MAIN_SCENE && o->Kind == KIND_MONSTER)
		{
			return;
		}
	}
	...
	{
		if (!g_pNewUISystem->GetUI_NewOptionWindow()->OnOffGrap[g_pNewUISystem->GetUI_NewOptionWindow()->eRenderObjects] && SceneFlag == MAIN_SCENE)
		{
			return;
		}
	}
    if((EditFlag!=EDIT_NONE) || (EditFlag==EDIT_NONE&&o->HiddenMesh!=-2))
	{
		if(ExtraMon == 10)
		{
			...
			if(View == true)
			{
				...
				for(int i = 0; i < Models[o->Type].NumMeshs; i++) b->RenderMesh(i,RENDER_BRIGHT|RENDER_CHROME5, Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU*8.f, o->BlendMeshTexCoordV*2.f,BITMAP_CHROME2 );
			}
		...
		//if (o->Type == MODEL_WIND_SPIN03)
		//{
		//	g_ErrorReport.Write("Render Efect Model Wind Spin03\r\n");
...
//			b->RenderBody(RENDER_TEXTURE,o->Alpha,o->BlendMesh,o->BlendMeshLight,o->BlendMeshTexCoordU,o->BlendMeshTexCoordV);
		...
		else if(o->Type == MODEL_FENRIR_BLACK || o->Type == MODEL_FENRIR_BLUE || o->Type == MODEL_FENRIR_RED || o->Type == MODEL_FENRIR_GOLD || gCustomPet2.GetInfoPetType(o->Type) == 5)
		{
...
//					Vector(0.7f,0.5f,0.8f,b->BodyLight);
            ...
            else if ( o->Type==MODEL_MONSTER01+60 )
            {
                if (  o->CurrentAction==MONSTER01_DIE || 
					(WorldTime - o->InitialSceneFrame) < 1000
					)
                {
					if ( o->CurrentAction==MONSTER01_DIE)
					{
			...
			{
				//
				if (o->Type == MODEL_phoenix_shot_effect)
				{
					Vector(1.f, 0.6f, 0.3f, b->BodyLight);
					b->RenderBody(RENDER_TEXTURE, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV, o->HiddenMesh);
					b->RenderBody(RENDER_BRIGHT | RENDER_CHROME, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV, 1);
...
ExitIfElse:
	//==Render Effect CUstom Pet
	if (gCustomPet2.CheckCustomPetByItem(o->Type - 1171))
	{
		for (std::vector<PET_EFFECT_CUSTOM>::iterator it = gCustomCEffectPet.m_PetCEffect.begin(); it != gCustomCEffectPet.m_PetCEffect.end(); it++)
		{
			if (it->ItemIndex == (o->Type - 1171))
			{
				...
				if (it->TypeEffect == 1)
				{
					CreateParticle(it->EffectCode, o->Position, o->Angle, o->Light, 0, it->EffectSize, o);
				}
...
static bool RenderObjectWithAdaptiveBudget(OBJECT* o)
{
    if (o == NULL || !o->Live)
    {
        return false;
    ...
    const float distSq = GetObjectDistanceSq2D(o);
    const AdaptiveObjectDistanceThresholds thresholds = GetObjectDistanceThresholds();
    const AdaptiveDistanceBucket bucket = ClassifyObjectDistance(distSq, thresholds);
    const bool adaptiveEnabled = IsObjectAdaptiveEnabled();
    const bool fullObjectVisibility = IsAndroidFullObjectVisibilityMode();
    const bool sideOverlayVisible = IsWideSideOverlayVisible();
    const float zoomedOutWideWeight = GetWideZoomFrustumWeight();
    const float objectExtentBias = GetObjectExtentBias(o);
    const bool stadiumStaticObject = IsStadiumStaticWorldObject(o);
    const bool stadiumHeavyDecorObject = IsStadiumHeavyDecorObject(o);
    const bool persistentAmbientFire = IsPersistentAmbientFireObject(o);
    const bool edgeSensitiveObject =
        !stadiumStaticObject &&
        (zoomedOutWideWeight > 0.05f && objectExtentBias >= 34.0f);
    const bool stabilizeFarObjectRender =
        !stadiumStaticObject &&
    ...
    if (adaptiveEnabled && !fullObjectVisibility)
    {
        if (ShouldCullStaticObjectByImportance(o, bucket, distSq, crowdPressure, currentFps))
        {
            ++g_objectPerfSnapshot.renderDistanceCulled;
            return false;
        }

        uint32_t cullPeriod = 1u;
        if (bucket == AdaptiveDistanceBucket::Far)
        {
            if (crowdPressure > 55 && heavyFrame)
            {
                cullPeriod = 2u;
            }
            if (crowdPressure > 90 && stressedFrame)
            {
                cullPeriod = (std::max)(cullPeriod, uint32_t{3u});
            }
            if (crowdPressure > 125 && severeFrame)
            {
                cullPeriod = (std::max)(cullPeriod, uint32_t{4u});
            }
            if (distSq > thresholds.hardCullSq)
            {
                if (crowdPressure > 120)
                {
                    cullPeriod = (std::max)(cullPeriod, severeFrame ? uint32_t{5u} : uint32_t{3u});
                }
                if (crowdPressure > 180 && severeFrame)
                {
                    cullPeriod = (std::max)(cullPeriod, uint32_t{6u});
                }
            }
            else if (distSq > thresholds.veryFarSq)
            {
                if (crowdPressure > 90 && stressedFrame)
                {
                    cullPeriod = (std::max)(cullPeriod, uint32_t{3u});
                }
                if (crowdPressure > 140 && severeFrame)
                {
                    cullPeriod = (std::max)(cullPeriod, uint32_t{5u});
                }
            }
            else if (crowdPressure > 150 && severeFrame)
            {
                cullPeriod = (std::max)(cullPeriod, uint32_t{3u});
            }
        }
        ...
        {
            if (crowdPressure > 85 && stressedFrame && distSq > thresholds.nearSq * 1.20f)
            {
                cullPeriod = 2u;
            }
            if (crowdPressure > 135 && severeFrame && distSq > thresholds.nearSq * 1.45f)
            {
                cullPeriod = 3u;
            }
        }

        if (stadiumStaticObject && distSq > stadiumCullStartSq)
        {
            if (bucket == AdaptiveDistanceBucket::Far)
            {
                if (crowdPressure > 100 && heavyFrame)
                {
                    cullPeriod = (std::max)(cullPeriod, uint32_t{2u});
                }
                if (crowdPressure > 132 && stressedFrame)
                {
                    cullPeriod = (std::max)(cullPeriod, uint32_t{3u});
                }
                if (stadiumHeavyDecorObject && crowdPressure > 145 && severeFrame)
                {
                    cullPeriod = (std::max)(cullPeriod, uint32_t{4u});
                }
                if (crowdPressure > 190 && stressedFrame)
                {
                    cullPeriod = (std::max)(cullPeriod, uint32_t{4u});
                }
                if (stadiumHeavyDecorObject && crowdPressure > 210 && stressedFrame)
                {
                    cullPeriod = (std::max)(cullPeriod, uint32_t{5u});
                }
            }
            ...
            {
                if (crowdPressure > 118 && stressedFrame)
                {
                    cullPeriod = (std::max)(cullPeriod, uint32_t{2u});
                }
                if (stadiumHeavyDecorObject && crowdPressure > 138 && severeFrame)
                {
                    cullPeriod = (std::max)(cullPeriod, uint32_t{3u});
                }
                if (crowdPressure > 210 && severeFrame)
                {
                    cullPeriod = (std::max)(cullPeriod, uint32_t{3u});
                }
            }
            else if (stadiumHeavyDecorObject &&
                crowdPressure > 165 &&
                severeFrame &&
                distSq > thresholds.nearSq * 0.55f)
            {
                cullPeriod = (std::max)(cullPeriod, uint32_t{2u});
            }
        }

        if (zoomedOutWideWeight > 0.01f)
        {
            if (bucket != AdaptiveDistanceBucket::Far || distSq <= thresholds.veryFarSq)
            {
                cullPeriod = 1u;
            }
            else if (zoomedOutWideWeight > 0.55f)
            {
                cullPeriod = (std::min)(cullPeriod, uint32_t{2u});
            }
        }

        if (persistentAmbientFire || edgeSensitiveObject || stabilizeFarObjectRender)
        {
            cullPeriod = 1u;
        }

        if (cullPeriod > 1u && !ShouldRunObjectStride(o, cullPeriod, 0x9e3779b9u))
        {
            ++g_objectPerfSnapshot.renderDistanceCulled;
            return false;
        }
    ...
    ConfigureObjectRenderLodContext(o, bucket, crowdPressure);
    const Uint64 baseTicksStart = static_cast<Uint64>(MU_MobilePerfNow());
    RenderObject(o);
    ResetObjectRenderLodContext();
    g_objectPerfSnapshot.renderBaseTicks += static_cast<unsigned long long>(MU_MobilePerfNow() - baseTicksStart);
    ++g_objectPerfSnapshot.renderBaseCalls;
    ++g_objectPerfSnapshot.renderRendered;

    bool deferVisual = false;
    if (adaptiveEnabled && !fullObjectVisibility)
    {
        uint32_t visualPeriod = 1u;
        if (bucket == AdaptiveDistanceBucket::Far)
        {
            if (distSq > thresholds.veryFarSq && crowdPressure > 60)
            {
                visualPeriod = 2u;
            }
            if (crowdPressure > 45 && stressedFrame)
            {
                visualPeriod = (std::max)(visualPeriod, uint32_t{2u});
            }
            if (crowdPressure > 70 && heavyFrame)
            {
                visualPeriod = (std::max)(visualPeriod, uint32_t{3u});
            }
            if (crowdPressure > 110 && heavyFrame)
            {
                ++visualPeriod;
            }
            if (crowdPressure > 150 && severeFrame)
            {
                ++visualPeriod;
            }
        }
        ...
        {
            if (crowdPressure > 55 && stressedFrame)
            {
                visualPeriod = 2u;
            }
            if (crowdPressure > 80 && severeFrame)
            {
                visualPeriod = sideOverlayVisible ? 2u : 3u;
            }
            if (crowdPressure > 105 && heavyFrame)
            {
                visualPeriod = 2u;
            }
            if (!sideOverlayVisible && crowdPressure > 145 && severeFrame)
            {
                visualPeriod = 3u;
            }
            else if (sideOverlayVisible && crowdPressure > 180 && severeFrame)
            {
                visualPeriod = 2u;
            }
        }
        else if (!sideOverlayVisible && bucket == AdaptiveDistanceBucket::Near && severeFrame && crowdPressure > 120)
        {
            visualPeriod = 2u;
        }
        ...
        if (stadiumStaticObject && distSq > stadiumCullStartSq)
        {
            if (bucket == AdaptiveDistanceBucket::Far && crowdPressure > 96 && heavyFrame)
            {
                visualPeriod = (std::max)(visualPeriod, uint32_t{2u});
            }
            else if (bucket == AdaptiveDistanceBucket::Mid && stadiumHeavyDecorObject && crowdPressure > 118 && stressedFrame)
            {
                visualPeriod = (std::max)(visualPeriod, uint32_t{2u});
            }
            ...
            if (bucket == AdaptiveDistanceBucket::Far)
            {
                if (crowdPressure > 180 && heavyFrame)
                {
                    visualPeriod = (std::max)(visualPeriod, uint32_t{4u});
                }
                if (crowdPressure > 210 && stressedFrame)
                {
                    visualPeriod = (std::max)(visualPeriod, uint32_t{5u});
                }
                if (stadiumHeavyDecorObject && crowdPressure > 220 && stressedFrame)
                {
                    visualPeriod = (std::max)(visualPeriod, uint32_t{6u});
                }
            }
            else if (bucket == AdaptiveDistanceBucket::Mid)
            {
                ...
                if (stadiumHeavyDecorObject && crowdPressure > 210 && severeFrame)
                {
                    visualPeriod = (std::max)(visualPeriod, uint32_t{4u});
                }
            }
                ...
                crowdPressure > 220 &&
                severeFrame &&
                distSq > thresholds.nearSq * 0.60f)
            {
                visualPeriod = (std::max)(visualPeriod, uint32_t{2u});
            }
        }

        if (persistentAmbientFire)
        {
            visualPeriod = 1u;
        }
        else if (edgeSensitiveObject)
        {
            if (bucket == AdaptiveDistanceBucket::Far && crowdPressure > 220 && severeFrame)
            {
                visualPeriod = (std::min)(visualPeriod, uint32_t{2u});
            }
            else
            {
                visualPeriod = 1u;
        ...
        deferVisual = !ShouldRunObjectStride(o, visualPeriod, 0x85ebca6bu);
    }

    if (deferVisual)
    {
        ++g_objectPerfSnapshot.visualDeferred;
    }
    else
    {
        ++g_objectPerfSnapshot.visualRendered;
        const Uint64 visualTicksStart = static_cast<Uint64>(MU_MobilePerfNow());
        RenderObjectVisual(o);
        g_objectPerfSnapshot.renderVisualTicks += static_cast<unsigned long long>(MU_MobilePerfNow() - visualTicksStart);
        ++g_objectPerfSnapshot.visualCalls;
    }
...
void RenderObjects()
{
    ...
    if( gMapManager.WorldActive==WD_10HEAVEN )
    {
	    range = GetObjectInstanceFrustumRange(-10.f);
    }
	...
	for(int i=0;i<16;i++)
	{
		...
		{
			OBJECT_BLOCK *ob = &ObjectBlock[i*16+j];
            const bool wasBlockVisible = ob->Visible;
			ob->Visible = TestFrustrum2D((float)(i*16+8),(float)(j*16+8),GetObjectBlockVisibleRange(wasBlockVisible));
			if(g_Direction.m_CKanturu.IsMayaScene()
				...
				)
			{
				...
				while(1)
				{
					if(o != NULL)
					{
						...
						{
							RenderObjectWithAdaptiveBudget(o);
						}
						else 
#ifdef PJH_NEW_SERVER_SELECT_MAP
			...
			if(ob->Visible || CameraTopViewEnable)
			{
				...
				while(1)
				{
					if(o != NULL) 
					{
						if(o->Live)
						{
                            const bool wasVisible = o->Visible;
							const float visibilityRange = o->CollisionRange + GetObjectVisibilityRange(o, range, wasVisible);
							o->Visible = TestFrustrum2D(o->Position[0]*0.01f,o->Position[1]*0.01f,visibilityRange);
...
void RenderObject_AfterCharacter(OBJECT *o,bool Translate,int Select, int ExtraMon)
{
    ...
    g_objectPerfSnapshot.renderAfterTicks += static_cast<unsigned long long>(MU_MobilePerfNow() - afterTicksStart);
    ++g_objectPerfSnapshot.renderAfterCalls;
}
...
void MoveObjects()
{
    ...
    visibleObject = 0;
    const AdaptiveObjectDistanceThresholds adaptiveThresholds = GetObjectDistanceThresholds();
    const bool fullObjectVisibility = IsAndroidFullObjectVisibilityMode();
	for(int i=0;i<16;i++)
	{
		for(int j=0;j<16;j++)
			...
			if(ob->Visible)
			{
				...
				while(1)
				{
					if(o != NULL) 
					{
						if(o->Live && o->Visible)
						{
                            ++g_objectPerfSnapshot.moveCandidates;
                            ...
                            if (IsObjectAdaptiveEnabled() && !fullObjectVisibility)
                            {
                                ...
                                uint32_t crowdBudgetPeriod = 1u;
                                if (movePressure > 180 && heavyFrame)
                                {
                                    crowdBudgetPeriod = 2u;
                                }
                                if (movePressure > 240 && severeFrame)
                                {
                                    crowdBudgetPeriod = 3u;
                                }
                                if (movePressure > 320 && severeFrame)
                                {
                                    crowdBudgetPeriod = 4u;
                                }

                                if (crowdBudgetPeriod > 1u)
                                {
                                    if (bucket == AdaptiveDistanceBucket::Far)
                                    {
                                        updatePeriod = (std::max)(updatePeriod, static_cast<uint32_t>(crowdBudgetPeriod + 1u));
                                    }
                                    else if (bucket == AdaptiveDistanceBucket::Mid)
                                    {
                                        updatePeriod = (std::max)(updatePeriod, crowdBudgetPeriod);
                                    }
                                    else if (distSq > nearAdaptiveMinDistanceSq)
                                    {
                                        updatePeriod = (std::max)(updatePeriod, uint32_t{2u});
                                    }
                                }
                                ...
                                if (severeFrame)
                                {
                                    if (bucket == AdaptiveDistanceBucket::Far)
                                    {
                                        updatePeriod = (std::max)(updatePeriod, uint32_t{3u});
                                    }
                                    else if (bucket == AdaptiveDistanceBucket::Mid)
                                    {
                                        updatePeriod = (std::max)(updatePeriod, uint32_t{2u});
                                    }
                                    else if (distSq > nearAdaptiveMinDistanceSq && movePressure > 64)
                                    {
                                        updatePeriod = (std::max)(updatePeriod, uint32_t{2u});
                                    }
                                }
                                else if (heavyFrame && bucket == AdaptiveDistanceBucket::Far)
                                {
                                    updatePeriod = (std::max)(updatePeriod, uint32_t{2u});
                                }

                                if (bucket == AdaptiveDistanceBucket::Near)
                                {
                                    updatePeriod = (std::min)(updatePeriod, uint32_t{2u});
                                }
                                else if (bucket == AdaptiveDistanceBucket::Mid)
                                {
                                    updatePeriod = (std::min)(updatePeriod, uint32_t{4u});
                                }
                                else
                                {
                                    updatePeriod = (std::min)(updatePeriod, uint32_t{6u});
                                }

                                deferMove = !ShouldRunObjectStride(o, updatePeriod, 0x7f4a7c15u);
                            }

                            if (deferMove)
                            {
                                ++g_objectPerfSnapshot.moveDeferred;
                            }
                            else
                            {
    ...
    g_iActionTime--;
    g_objectPerfSnapshot.moveTicks += static_cast<unsigned long long>(MU_MobilePerfNow() - moveTicksStart);
}
...
int OpenObjects(char *FileName)
{
	...
	int EncBytes = 0;
	if(false == ReadWholeFileBytes(fp, EncData, EncBytes))
	{
		fclose(fp);
		return ( -1);
	}
...
void ItemAngle(OBJECT *o)
{
	...
	else if(o->Type==MODEL_POTION+52 || o->Type >= MODEL_POTION+55 && o->Type <= MODEL_POTION+57 || o->Type >= MODEL_POTION+157 && o->Type <= MODEL_POTION+159) //DAT FIX SIZE ITEM DROP
	{
		o->Angle[2] = -10.f;
		o->Scale = 0.4f;
	}
...
void CreateItem(ITEM_t *ip,BYTE *Item,vec3_t Position,int CreateFlag)
{
	...
	else
	{
		...
		if(CreateFlag)
		{
			if(Type==ITEM_POTION+13 || gCustomJewel.CheckCustomJewelByItem(Type) || Type==ITEM_POTION+14 ||  Type==ITEM_POTION+16 || Type==ITEM_WING+15 || Type==ITEM_POTION+22 || Type==ITEM_POTION+31)
...
void PartObjectColor(int Type,float Alpha,float Bright,vec3_t Light,bool ExtraMon)
{
	...
	|| Type == MODEL_SWORD_35_WING
	|| Type == MODEL_HELM + 73
	|| Type == MODEL_ARMOR + 73
	|| Type == MODEL_PANTS + 73
	|| Type == MODEL_GLOVES + 73
	|| Type == MODEL_BOOTS + 73
	)
		Color = 45;
...
void RenderPartObjectBody(BMD *b,OBJECT *o,int Type,float Alpha,int RenderType)
{
	...
	else if( Type==MODEL_WING+37 || gCustomWing.CheckCustomWingByItem(Type - MODEL_ITEM) && !gCustomWing.CheckCustomWingByModelType(Type - MODEL_ITEM, 1))	// 시공날개(법사)
    {
		Vector(1.f,1.f,1.f,b->BodyLight);
		b->RenderBody(RENDER_TEXTURE,o->Alpha,o->BlendMesh,o->BlendMeshLight,o->BlendMeshTexCoordU,o->BlendMeshTexCoordV,o->HiddenMesh);
    }
	...
	else if ( Type==MODEL_BOW+7 || Type==MODEL_BOW+15) //CustomArrow Cung,No
	{
		if( 
	...
	//===Sum Effect ArchangelusWeapon_render 32500 
	else if (Type == MODEL_STAFF + 36)
	{
		float fLumi = sinf(WorldTime * 0.001500000013038516);
		Vector(1.f, 1.f, 1.f, b->BodyLight);
		b->RenderMesh(0, RENDER_TEXTURE, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);

		Vector(1.f, 0.1f, 0.1f, b->BodyLight);
		glColor3fv(b->BodyLight);
		b->RenderMesh(0, 64, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV, 32500);
		Vector(1.f, 1.f, 1.f, b->BodyLight);
		b->RenderMesh(1, 70, o->Alpha, 1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
	}
	...
	else if (g_CMonkSystem.EqualItemModelType(Type) == MODEL_SWORD + 35) //ok
	{
		...
		b->RenderBody(RENDER_TEXTURE, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
		b->RenderMesh(0, RENDER_TEXTURE, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
		b->RenderMesh(0, RENDER_BRIGHT, o->Alpha * fLumi, 0, o->BlendMeshLight * fLumi, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV, 32502);
	...
	else if(Type == MODEL_WING+50 || gCustomWing.CheckCustomWingByModelType(Type - MODEL_ITEM, 1))
	{
		if(b->BodyLight[0] == 1 && b->BodyLight[1] == 1 && b->BodyLight[2] == 1)
		{
			Vector(1.f,1.f,1.f,b->BodyLight);
...
void RenderPartObjectBodyColor(BMD *b,OBJECT *o,int Type,float Alpha,int RenderType,float Bright,int Texture, int iMonsterIndex)
{
	...
	else if (gCloak.isCEffect(Type - MODEL_ITEM)) //Fix Vien Den
	{
		//gCloak.RenderCEffect(Type - MODEL_ITEM, o, b);
...
//	int Level;
//	PART_t *w;
//	if (!g_pNewUISystem->GetUI_NewOptionWindow()->OnOffGrap[g_pNewUISystem->GetUI_NewOptionWindow()->eBMDImg]) return;
//	for(int i=0; i<2; i++)
//	{
//		w = &c->Weapon[i];
//		Level = w->Level;
