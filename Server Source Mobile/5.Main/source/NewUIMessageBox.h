...
namespace SEASON3B
{
	...
	{
		TMsgBoxLayoutContainer<_L>& mutableContainer = const_cast<TMsgBoxLayoutContainer<_L>&>(container);
		if(false == mutableContainer.Create()) //. MessageBox Layout
			return false;
		
		return mutableContainer.SetLayout();
	}
