...
void SEASON3B::CGemIntegrationDisjointMsgBox::RenderTexts()
{
	...
	x = GetPos().x-3; y = GetPos().y + (MSGBOX_TEXT_TOP_BLANK / 2);
	type_vector_msgdata::iterator vi = m_MsgDataList.begin();
	for(; vi != m_MsgDataList.end(); vi++)
	{
		g_pRenderText->SetTextColor((*vi)->dwColor);
...
void SEASON3B::CChaosMixMenuMsgBox::SetButtonInfo()
{
	...
	if(!gProtect.m_MainInfo.DisableMix380)
	{
		y = GetPos().y + 225;
	...
	else
	{
		...
		if(pPickedItem)
		{
			...
			if(pPickedItem->GetOwnerInventory() == g_pMyInventory->GetInventoryCtrl() || g_pMyInventoryExt->GetOwnerOf(pPickedItem))
			{
				...
				SendRequestEquipmentItem(REQUEST_EQUIPMENT_INVENTORY, iSourceIndex, pItemObj,REQUEST_EQUIPMENT_MYSHOP, MAX_MY_INVENTORY_EX_INDEX +iTargetIndex);
			...
			else if(pPickedItem->GetOwnerInventory() == g_pMyShopInventory->GetInventoryCtrl())
			{
				...
				SendRequestEquipmentItem(REQUEST_EQUIPMENT_MYSHOP, iSourceIndex, pItemObj,REQUEST_EQUIPMENT_MYSHOP, MAX_MY_INVENTORY_EX_INDEX +iTargetIndex);
			}
			
			AddPersonalItemPrice(MAX_MY_INVENTORY_EX_INDEX +iTargetIndex, iInputZen, g_IsPurchaseShop);
		}
		else 
		{
			iSourceIndex = g_pMyShopInventory->GetSourceIndex();
			iSourceIndex = MAX_MY_INVENTORY_EX_INDEX + iSourceIndex;
...
bool SEASON3B::CAddStatPointMsgBoxLayout::SetLayout()
{
	CNewUITextInputMsgBox* pMsgBox = GetMsgBox();
	if (!pMsgBox) return false;

	if (!pMsgBox->Create(MSGBOX_COMMON_TYPE_OKCANCEL, INPUTBOX_TYPE_NUMBER, 60, 14, 5))
		return false;

	pMsgBox->SetInputBoxOption(UIOPTION_NUMBERONLY | UIOPTION_PAINTBACK);
	pMsgBox->AddMsg(GlobalText[3192]);

	pMsgBox->AddCallbackFunc(CAddStatPointMsgBoxLayout::ReturnDown, MSGBOX_EVENT_PRESSKEY_RETURN);
	pMsgBox->AddCallbackFunc(CAddStatPointMsgBoxLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CAddStatPointMsgBoxLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);
...
CALLBACK_RESULT SEASON3B::CAddStatPointMsgBoxLayout::ReturnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CAddStatPointMsgBoxLayout::OkBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CAddStatPointMsgBoxLayout::CancelBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
	return CALLBACK_BREAK;
...
bool SEASON3B::CMasterSkillPointMsgBoxLayout::SetLayout()
{
	auto* pMsgBox = GetMsgBox();
	if (!pMsgBox) return false;

	if (!pMsgBox->Create(MSGBOX_COMMON_TYPE_OKCANCEL, INPUTBOX_TYPE_NUMBER, 60, 14, 2))
		return false;

	pMsgBox->SetInputBoxOption(UIOPTION_NUMBERONLY | UIOPTION_PAINTBACK);
	pMsgBox->AddMsg(GlobalText[3193]);

	pMsgBox->AddCallbackFunc(ReturnDown, MSGBOX_EVENT_PRESSKEY_RETURN);
...
CALLBACK_RESULT SEASON3B::CMasterSkillPointMsgBoxLayout::ProcessOk(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (iValue > 0 && iValue <= 20)
	{
		for (int i = 0; i < iValue; ++i)
			SendRequestMasterLevelSkill(g_pMasterSkillTreeInterface->GetCurSkillID());
...
CALLBACK_RESULT SEASON3B::CMasterSkillPointMsgBoxLayout::ReturnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CMasterSkillPointMsgBoxLayout::OkBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CMasterSkillPointMsgBoxLayout::CancelBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
	return CALLBACK_BREAK;
