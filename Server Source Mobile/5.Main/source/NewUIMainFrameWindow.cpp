...
constexpr int kAndroidHiddenLegacyButtonX = -10000;
constexpr int kAndroidHiddenLegacyButtonY = -10000;
constexpr float kAndroidLegacyMainFrameButtonCutX = 519.0f;
#endif
...
#if defined(__ANDROID__) || defined(MU_IOS)
	m_BtnFriend.ChangeButtonInfo(kAndroidHiddenLegacyButtonX, kAndroidHiddenLegacyButtonY, x_Add, y_Add);
	m_BtnWindow.ChangeButtonInfo(kAndroidHiddenLegacyButtonX, kAndroidHiddenLegacyButtonY, x_Add, y_Add);
	m_BtnMyInven.ChangeButtonInfo(kAndroidHiddenLegacyButtonX, kAndroidHiddenLegacyButtonY, x_Add, y_Add);
	m_BtnChaInfo.ChangeButtonInfo(kAndroidHiddenLegacyButtonX, kAndroidHiddenLegacyButtonY, x_Add, y_Add);
#endif
...
int SEASON3B::CNewUIMainFrameWindow::GetItemHotKeyInventoryIndex(int iHotKey, bool bItemCount)
{
	return m_ItemHotKey.GetHotKeyInventoryIndex(iHotKey, bItemCount);
}

bool SEASON3B::CNewUIMainFrameWindow::UseItemHotKey(int iHotKey)
{
	return m_ItemHotKey.UseHotKey(iHotKey);
}
...
int SEASON3B::CNewUIItemHotKey::GetHotKeyInventoryIndex(int iHotKey, bool bItemCount)
{
	return GetHotKeyItemIndex(iHotKey, bItemCount);
}

bool SEASON3B::CNewUIItemHotKey::UseHotKey(int iHotKey)
{
	...
	const int iIndex = GetHotKeyItemIndex(iHotKey);
	if (iIndex == -1)
	{
		return false;
	}
...
void SEASON3B::CNewUISkillList::RenderSkillIcon(int iIndex, float x, float y, float width, float height, int TypeMuHelper, bool drawHotKeyNumber)
{
	...
	float fU, fV;
	int iKindofSkill = 0;
	const float iconSourceW = 20.0f;
	const float iconSourceH = 28.0f;
	const float iconStepU = iconSourceW / 256.f;
	const float iconStepV = iconSourceH / 256.f;
	...
	if (bySkillType >= AT_PET_COMMAND_DEFAULT && bySkillType <= AT_PET_COMMAND_END)
	{
		fU = ((bySkillType - AT_PET_COMMAND_DEFAULT) % 8) * iconStepU;
		fV = ((bySkillType - AT_PET_COMMAND_DEFAULT) / 8) * iconStepV;
	...
	{
		fU = ((bySkillType - AT_SKILL_ALICE_DRAINLIFE) % 8) * iconStepU;
		fV = 3 * iconStepV;
		iKindofSkill = KOS_SKILL2;
	}
	else if (bySkillType >= AT_SKILL_ALICE_SLEEP && bySkillType <= AT_SKILL_ALICE_BLIND)
	{
		fU = ((bySkillType - AT_SKILL_ALICE_SLEEP + 4) % 8) * iconStepU;
		fV = 3 * iconStepV;
		iKindofSkill = KOS_SKILL2;
	}
	...
	{
		fU = (bySkillType - AT_SKILL_ALICE_WEAKNESS + 8) * iconStepU;
		fV = 3 * iconStepV;
		iKindofSkill = KOS_SKILL2;
	}
	else if (bySkillType >= AT_SKILL_SUMMON_EXPLOSION && bySkillType <= AT_SKILL_SUMMON_REQUIEM)
	{
		fU = ((bySkillType - AT_SKILL_SUMMON_EXPLOSION + 6) % 8) * iconStepU;
		fV = 3 * iconStepV;
		iKindofSkill = KOS_SKILL2;
	}
	...
	{
		fU = iconStepU * (Skill_Icon % 12);
		fV = iconStepV * ((Skill_Icon / 12) + 4);
		iKindofSkill = KOS_SKILL2;
	}
#ifdef PBG_ADD_NEWCHAR_MONK_SKILL
	else if (bySkillType >= AT_SKILL_THRUST)
	{
		fU = ((bySkillType - 260) % 12) * iconStepU;
		fV = ((bySkillType - 260) / 12) * iconStepV;
		iKindofSkill = KOS_SKILL3;
	}
#endif //PBG_ADD_NEWCHAR_MONK_SKILL
	else if (bySkillType >= 57)
	{
		fU = ((bySkillType - 57) % 8) * iconStepU;
		fV = ((bySkillType - 57) / 8) * iconStepV;
		iKindofSkill = KOS_SKILL2;
	}
	else
	{
		fU = ((bySkillType - 1) % 8) * iconStepU;
		fV = ((bySkillType - 1) / 8) * iconStepV;
