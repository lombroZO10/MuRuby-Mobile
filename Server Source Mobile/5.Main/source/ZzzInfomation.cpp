	...
	if(fp != NULL)
	{
		...
		//else
		{
			...
			for(int i=0;i<MAX_SKILLS;i++)
			{
				...
				//g_ConsoleDebug->Write(MCD_ERROR, "%d %s %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d",i, SkillAttribute[i].Name,
				//	SkillAttribute[i].Level,
				//	SkillAttribute[i].Damage,
				//	SkillAttribute[i].Mana,
				//	SkillAttribute[i].AbilityGuage,
				//	SkillAttribute[i].Distance,
				//	SkillAttribute[i].Delay,
				//	SkillAttribute[i].Energy,
				//	SkillAttribute[i].Charisma,
				//	SkillAttribute[i].MasteryType,
				//	SkillAttribute[i].SkillUseType,
				//	SkillAttribute[i].SkillBrand,
				//	SkillAttribute[i].KillCount,
				//	SkillAttribute[i].RequireDutyClass[0],
				//	SkillAttribute[i].RequireDutyClass[1],
				//	SkillAttribute[i].RequireDutyClass[2],
				//	SkillAttribute[i].RequireClass[0],
				//	SkillAttribute[i].RequireClass[1],
				//	SkillAttribute[i].RequireClass[2],
				//	SkillAttribute[i].RequireClass[3],
				//	SkillAttribute[i].RequireClass[4],
				//	SkillAttribute[i].RequireClass[5],
				//	SkillAttribute[i].RequireClass[6],
				//	SkillAttribute[i].SkillRank,
				//	SkillAttribute[i].SkillGroup,
				//	SkillAttribute[i].Unknown3,
				//	SkillAttribute[i].Unknown4);
...
//===Tool Tip Item
_ITEM_TOOLTIP tooltip_data[MAX_ITEM];
_ITEM_LEVEL_TOOLTIP tooltip_level_data[TOOLTIP_LEVEL_SIZE];
_ITEM_TOOLTIP_TEXT tooltip_text_data[TOOLTIP_TEXT_SIZE];

std::map<int, _ITEM_TOOLTIP> m_ItemToolTipData;
std::map<int, _ITEM_LEVEL_TOOLTIP> m_ItemLevelToolTipData;
std::map<int, _ITEM_TOOLTIP_TEXT> m_ItemToolTipTextData;

void set_item_tooltip()
{
	m_ItemToolTipData.clear();

	for (size_t i = 0; i < MAX_ITEM; i++)
	{
		if (!tooltip_data[i].name[0])
			break;

		int item_index = tooltip_data[i].index + (LOBYTE(tooltip_data[i].type) << 9);

		m_ItemToolTipData.insert(std::pair<int, _ITEM_TOOLTIP>(item_index, tooltip_data[i]));
	}
}

void set_item_level_tooltip()
{
	m_ItemLevelToolTipData.clear();

	for (size_t i = 0; i < TOOLTIP_LEVEL_SIZE; i++)
	{
		if (!tooltip_level_data[i].name[0])
			break;

		m_ItemLevelToolTipData.insert(std::pair<int, _ITEM_LEVEL_TOOLTIP>(tooltip_level_data[i].index, tooltip_level_data[i]));
	}
}

void set_item_text_tooltip()
{
	m_ItemToolTipTextData.clear();

	for (size_t i = 0; i < TOOLTIP_TEXT_SIZE; i++)
	{
		if (!tooltip_text_data[i].text[0])
			break;

		m_ItemToolTipTextData.insert(std::pair<int, _ITEM_TOOLTIP_TEXT>(tooltip_text_data[i].index, tooltip_text_data[i]));
	}
}


void load_item_tooltip(char* path)
{
	const int size = sizeof(_ITEM_TOOLTIP);
	memset(tooltip_data, 0, size * MAX_ITEM);
	...
	if (fp)
	{
		...
		//else
		{
			...
			{
				BuxConvert(pSeek, size);
				memcpy(&tooltip_data[i], pSeek, size);
				pSeek += size;
			}
		}
		delete[] Buffer;
...
void load_item_level_tooltip(char* path)
{
	//sBlockSize = 102;
	const int size = sizeof(_ITEM_LEVEL_TOOLTIP);
	memset(tooltip_level_data, 0, size * TOOLTIP_LEVEL_SIZE);
	FILE* fp = fopen(path, "rb");
	if (fp)
	{
		BYTE* Buffer = new BYTE[size * TOOLTIP_LEVEL_SIZE];
		fread(Buffer, size * TOOLTIP_LEVEL_SIZE, 1, fp);
		DWORD dwCheckSum;
		fread(&dwCheckSum, sizeof(DWORD), 1, fp);
		fclose(fp);

		//if (dwCheckSum != GenerateCheckSum2(Buffer, size * TOOLTIP_LEVEL_SIZE, 0xE2F1))
		//{
		//	char Text[256];
		//	sprintf(Text, "%s - File corrupted.", path);
		//	g_ErrorReport.Write(Text);
		...
		{
			BYTE* pSeek = Buffer;
			for (int i = 0; i < TOOLTIP_LEVEL_SIZE; i++)
			{
				BuxConvert(pSeek, size);
				memcpy(&tooltip_level_data[i], pSeek, size);
				pSeek += size;
			}
		}
		delete[] Buffer;
...
{
	//sBlockSize = 260;
	const int size = sizeof(_ITEM_TOOLTIP_TEXT);
	memset(tooltip_text_data, 0, size * TOOLTIP_TEXT_SIZE);
	FILE* fp = fopen(path, "rb");
	if (fp)
	{
		...
		/*if (dwCheckSum != GenerateCheckSum2(Buffer, size * TOOLTIP_TEXT_SIZE, 0xE2F1))
		{
			char Text[256];
			sprintf(Text, "%s - File corrupted.", path);
			g_ErrorReport.Write(Text);
...
void ItemConvert(ITEM *ip,BYTE Attribute1,BYTE Attribute2, BYTE Attribute3 )
{
    ...
    if ( ( ip->Type>=ITEM_WING+3 && ip->Type<=ITEM_WING+6 )
		...
		|| gCustomWing.CheckCustomWingByItem(ip->Type) //Custom Wing
		|| ip->Type==ITEM_BOW+18	
	...
	if ( excel>0 )
	{
		if(ip->RequireLevel > 0 && ip->Type != ITEM_HELPER+37 || !gCustomPet2.GetInfoPetType(ip->Type) ==5 )
      		ip->RequireLevel += 20;
	}
	...
	else if (gCustomWing.CheckCustomWingByItem(ip->Type)) //Custom Wing Opt Exc
	{
		if (excelWing & 0x01)
		{
			ip->SpecialValue[ip->SpecialNum] = gCustomWing.GetCustomWingNewOptionValue(ip->Type, 0);
			ip->Special[ip->SpecialNum] = gCustomWing.GetCustomWingNewOptionIndex(ip->Type,0); ip->SpecialNum++;
		}
		if ((excelWing >> 1) & 0x01)
		{
			ip->SpecialValue[ip->SpecialNum] = gCustomWing.GetCustomWingNewOptionValue(ip->Type,1);
			ip->Special[ip->SpecialNum] = gCustomWing.GetCustomWingNewOptionIndex(ip->Type, 1); ip->SpecialNum++;
		}
		if ((excelWing >> 2) & 0x01)
		{
			ip->SpecialValue[ip->SpecialNum] = gCustomWing.GetCustomWingNewOptionValue(ip->Type, 2);
			ip->Special[ip->SpecialNum] = gCustomWing.GetCustomWingNewOptionIndex(ip->Type, 2); ip->SpecialNum++;
		}
		if ((excelWing >> 3) & 0x01)
		{
			ip->SpecialValue[ip->SpecialNum] = gCustomWing.GetCustomWingNewOptionValue(ip->Type,3);
			ip->Special[ip->SpecialNum] = gCustomWing.GetCustomWingNewOptionIndex(ip->Type, 3); ip->SpecialNum++;
		}
	}
	if((Attribute1>>7)&1)
	{        
	...
	if((Attribute1>>2)&1)
	{
		...
		{
			if(ip->Type!=ITEM_BOW+7 && ip->Type!=ITEM_BOW+15) //CustomArrow
			{
	...
	if(Option3)
	{
        ...
        else
        {
			...
			//=== Get Custom Wing option life
			else if (gCustomWing.CheckCustomWingByItem(ip->Type))
			{
				if ((excelWing >> 4) & 0x01)
				{
					ip->SpecialValue[ip->SpecialNum] = gCustomWing.GetCustomWingOptionValue(ip->Type,0) * 4;
					ip->Special[ip->SpecialNum] = gCustomWing.GetCustomWingOptionIndex(ip->Type, 0); ip->SpecialNum++;
				}
				else if ((excelWing >> 5) & 0x01)
				{
					ip->SpecialValue[ip->SpecialNum] = gCustomWing.GetCustomWingOptionValue(ip->Type, 1) * 4;
	...
	//== Custom Ring Pen
	if ((ip->Type >= ITEM_SHIELD && ip->Type < ITEM_BOOTS + MAX_ITEM_INDEX) || (ip->Type >= ITEM_HELPER + 8 && ip->Type <= ITEM_HELPER + 9) || (ip->Type >= ITEM_HELPER + 21 && ip->Type <= ITEM_HELPER + 24) || gCustomItem.IsCustomRingPenType(ip->Type) == 0) //custom ring
	{
		if((Attribute2>>5)&1)
		{
	...
	//== Custom Ring Pen
	if ((ip->Type >= ITEM_SWORD && ip->Type < ITEM_STAFF + MAX_ITEM_INDEX) || (ip->Type >= ITEM_HELPER + 12 && ip->Type <= ITEM_HELPER + 13) || (ip->Type >= ITEM_HELPER + 25 && ip->Type <= ITEM_HELPER + 28) || gCustomItem.IsCustomRingPenType(ip->Type) == 1)//custom pedan
	{
		if((Attribute2>>5)&1)
		{
...
int ItemValue(ITEM *ip,int goldType)
{
	...
	if(ip->Type == ITEM_HELPER+32)
		...
		Gold = 30000;
	else if(ip->Type == ITEM_HELPER+37 || gCustomPet2.GetInfoPetType(ip->Type) == 5)
		Gold = 50000;
	...
	if(goldType==1 && !(ip->Type >= ITEM_HELPER+32 && ip->Type <= ITEM_HELPER+37) || gCustomPet2.GetInfoPetType(ip->Type) == 5)
    {
//        char Text[100];
...
void OpenMonsterScript(char *FileName)
{
	...
	while(true)
	{
		...
		if(Token == END) break;
		if(Token == NAME && strcmp("end",TokenString)==NULL) break;
		if(EditMonsterNumber >= MAX_MONSTER)
		{
			break;
		}
...
char*   getMonsterName ( int type )
{
    ...
    char* customMonsterName = gCustomMonster.GetNameMonterCustom(type);

    if (customMonsterName != NULL && customMonsterName[0] != '\0')
    {
        return customMonsterName;
    }

    static char s_UnknownMonsterName[32];
    sprintf(s_UnknownMonsterName, "Monster %d", type);
    return s_UnknownMonsterName;
}
...
void CHARACTER_MACHINE::CalculateDamage()
{
    ...
    if ( ( Right->Type>=ITEM_BOW && Right->Type<ITEM_BOW+MAX_ITEM_INDEX ) && ( Left->Type>=ITEM_BOW && Left->Type<ITEM_BOW+MAX_ITEM_INDEX ))
    {
        ...
        if ( Left->Type==ITEM_BOW+7 && LLevel>=1 ) //CustomArrow
        {
        ...
        if ( Right->Type==ITEM_BOW+15 && RLevel>=1 ) //CustomArrow
        {
...
void CHARACTER_MACHINE::CalculateMagicDamage()
{
	...
	//===Fix Info View
	Character.MagicDamageMin = (WORD)Character.PrintPlayer.ViewMagicDamageMin;
	Character.MagicDamageMax = (WORD)Character.PrintPlayer.ViewMagicDamageMax;
}
...
void CHARACTER_MACHINE::CalculateCurseDamage()
{
	...
	//===Fix Info View
	Character.CurseDamageMin = (WORD)Character.PrintPlayer.ViewCurseDamageMin;
	Character.CurseDamageMax = (WORD)Character.PrintPlayer.ViewCurseDamageMax;
}
...
void CHARACTER_MACHINE::CalculateAttackRating()
{
	...
	//===Fix Info View
	Character.AttackRating = (WORD)Character.PrintPlayer.ViewAttackSuccessRate;
}
...
void CHARACTER_MACHINE::CalculateAttackRatingPK()
{
	...
	//===Fix Info View
	Character.AttackRatingPK = (WORD)Character.PrintPlayer.ViewAttackSuccessRatePvP;
}

void CHARACTER_MACHINE::CalculateAttackSpeed()
{
	//===Fix Info View Attack Speed
	Character.AttackSpeed = (WORD)CharacterAttribute->PrintPlayer.ViewPhysiSpeed;
	Character.MagicSpeed = (WORD)CharacterAttribute->PrintPlayer.ViewMagicSpeed;
	return;
	WORD Dexterity;
	...
	bool RingLeft	= false;
	bool Helper     = false;
    if((r->Type!=ITEM_BOW+7 && r->Type!=ITEM_BOW+15 ) && r->Type>=ITEM_SWORD && r->Type<ITEM_STAFF+MAX_ITEM_INDEX) { //CustomArrow
    ...
    if((l->Type!=ITEM_BOW+7 && l->Type!=ITEM_BOW+15 ) && l->Type>=ITEM_SWORD && l->Type<ITEM_STAFF+MAX_ITEM_INDEX) { //CustomArrow
...
void CHARACTER_MACHINE::CalculateSuccessfulBlockingPK()
{
	...
	//== Fix Info View 
	Character.SuccessfulBlockingPK = (WORD)CharacterAttribute->PrintPlayer.ViewDefenseSuccessRatePvP;
}
...
void CHARACTER_MACHINE::CalculateSuccessfulBlocking()
{
	...
	//== Fix Info View 
	Character.SuccessfulBlocking = (WORD)CharacterAttribute->PrintPlayer.ViewDefenseSuccessRate;
}
...
void CHARACTER_MACHINE::CalculateDefense()
{
	...
	//== Fix Info View 
	Character.Defense = (WORD)CharacterAttribute->PrintPlayer.ViewDefense;
}
...
void CHARACTER_MACHINE::CalculateBasicState()
{
	...
	//===Fix Info View
	Character.AddStrength = (WORD)Character.PrintPlayer.ViewAddStrength;
	Character.AddDexterity = (WORD)Character.PrintPlayer.ViewAddDexterity;
	Character.AddVitality = (WORD)Character.PrintPlayer.ViewAddVitality;
	Character.AddEnergy = (WORD)Character.PrintPlayer.ViewAddEnergy;
	Character.AddCharisma = (WORD)Character.PrintPlayer.ViewAddLeadership;
}
