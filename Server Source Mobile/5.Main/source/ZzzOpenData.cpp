...
void OpenPlayers()
{
	...
	gLoadData.AccessModel(MODEL_GLOVES + 73, "Data\\Player\\", "GloveMale74", -1);
...
void OpenPlayerTextures()
{
	...
	gLoadData.OpenTexture(MODEL_GLOVES + 73, "Player\\");
...
void LoadItemModel(int index, const char* folder, const char* name, int value) // OK
{
	if (name[0] == 0)
	{
		return;
	...
	gLoadData.AccessModel(index, path, const_cast<char*>(name), value);
...
//	gLoadData.AccessModel(MODEL_HELPER+132, "Data\\Item\\", "goldenmaple_cham");
	...
	gLoadData.AccessModel(MODEL_POTION+56, "Data\\Item\\", "roboxpurple");
	gLoadData.AccessModel(MODEL_POTION+57, "Data\\Item\\", "roboxred");
	for(int k=0; k<6; k++)
	{
	...
	gLoadData.AccessModel(MODEL_STAFF+36, "Data\\Item\\", "Archangelus");
	...
	//==SS 6
	gLoadData.AccessModel(8490, "Data\\Item\\", "rollofpaper", -1);
	gLoadData.AccessModel(8491, "Data\\Item\\", "rollofpaper", -1);
	gLoadData.AccessModel(8492, "Data\\Item\\", "songss", -1);
	gLoadData.AccessModel(8493, "Data\\Item\\", "hydra_jewel", -1);
	//gLoadData.AccessModel(8494, "Data\\Item\\", (char*)& off_D30668, -1);
	gLoadData.AccessModel(8495, "Data\\Item\\", "Mace01", -1);
	gLoadData.AccessModel(8496, "Data\\Item\\", "roboxgreen", -1);
	gLoadData.AccessModel(8497, "Data\\Item\\", "roboxred", -1);
	gLoadData.AccessModel(8498, "Data\\Item\\", "roboxpurple", -1);
	gLoadData.AccessModel(8501, "Data\\Item\\", "Mbag", -1);
	gLoadData.AccessModel(8502, "Data\\Item\\", "EXbond", -1);
	gLoadData.AccessModel(8503, "Data\\Item\\", "AncientStatue", -1);
	gLoadData.AccessModel(8504, "Data\\Item\\", "MagicCloth", -1);
	gLoadData.AccessModel(8505, "Data\\Item\\", "SpaceCloth", -1);
	gLoadData.AccessModel(8506, "Data\\Item\\", "PhaseCloth", -1);
	gLoadData.AccessModel(8507, "Data\\Item\\", "DimensionCloth", -1);

	auto setNoneBlendMeshSafe = [](int modelIndex, int meshIndex, const char* label)
	{
		...
		model->Meshs[meshIndex].NoneBlendMesh = true;
	};

    setNoneBlendMeshSafe(MODEL_SPEAR, 1, "MODEL_SPEAR");
    setNoneBlendMeshSafe(MODEL_SWORD+10, 1, "MODEL_SWORD+10");
    setNoneBlendMeshSafe(MODEL_STAFF+6, 2, "MODEL_STAFF+6");
    setNoneBlendMeshSafe(MODEL_MACE+6, 1, "MODEL_MACE+6");
	setNoneBlendMeshSafe(MODEL_EVENT+9, 1, "MODEL_EVENT+9");

	//=== Load Model Item Custom
	for (std::map<int, CUSTOM_ITEM_INFO>::iterator it = gCustomItem.m_CustomItemInfo.begin(); it != gCustomItem.m_CustomItemInfo.end(); it++)
	{
		LoadItemModel((it->second.ItemIndex + MODEL_ITEM), ((it->second.ItemIndex >= BConverITEM(7, 0) && it->second.ItemIndex < BConverITEM(12, 0)) ? "Player\\" : "Item\\"), it->second.ModelName, -1);
	}
	...
	for (int n = 0; n < MAX_CUSTOM_JEWEL; n++)
	{
		...
		{
			LoadItemModel((gCustomJewel.m_CustomJewelInfo[n].ItemIndex + MODEL_ITEM), "Item\\", gCustomJewel.m_CustomJewelInfo[n].ModelName, -1);
		}
	}
	...
	for (int n = 0; n < MAX_CUSTOM_WING; n++)
	{
		...
		{
			LoadItemModel((gCustomWing.m_CustomWingInfo[n].ItemIndex + MODEL_ITEM), "Item\\", gCustomWing.m_CustomWingInfo[n].ModelName, -1);
		}
	}
	...
	for (std::map<int, CUSTOM_RF_GLOVES>::iterator it = gCustomBattleGloves.m_CustomGlovesInfo.begin(); it != gCustomBattleGloves.m_CustomGlovesInfo.end(); it++)
	{
		...
		{
			LoadItemModel(it->second.ItemIndex + MODEL_ITEM, "Item\\", it->second.ModelName, -1);
			LoadItemModel(it->second.TextureLeftIndex + MODEL_ITEM, "Item\\", it->second.TextureLeftName, -1);
			LoadItemModel(it->second.TextureRightIndex + MODEL_ITEM, "Item\\", it->second.TextureRightName, -1);
		}
	}
...
void LoadItemTexture(int index, const char* folder, const char* name, int value) // OK
{
	if (name[0] == 0)
	{
		return;
	...
	gLoadData.OpenTexture(index, const_cast<char*>(folder), 0x2901, 0x2600, 0x01);
...
//---------------------------------------------------------------------------------
	...
	gLoadData.OpenTexture(MODEL_STAFF+36, "Item\\");
	...
	::LoadBitmap("Item\\ArchangelusWeapon_render.jpg", 32500, 9729, 10497, 1, 0);
	::LoadBitmap("Effect\\pin_star.jpg", 32501, 9729, 10497, 1, 0);
	::LoadBitmap("Item\\PhoenixSoul_render.JPG", 32502, 9729, 10497, 1, 0);
	//==Ss6
	for (int i13 = 0; i13 < 9; ++i13)
	{
		gLoadData.OpenTexture(i13 + 8490, "Item\\");
	}
		
	gLoadData.OpenTexture(8501, "Item\\");
	gLoadData.OpenTexture(8502, "Item\\");
	gLoadData.OpenTexture(8503, "Item\\");
	gLoadData.OpenTexture(8504, "Item\\");
	gLoadData.OpenTexture(8505, "Item\\");
	gLoadData.OpenTexture(8506, "Item\\");
	gLoadData.OpenTexture(8507, "Item\\");

	//==========================================
	//==Custom load Item New 
	//==========================================
	gCloak.LoadTexture();

	for (std::map<int, CUSTOM_ITEM_INFO>::iterator it = gCustomItem.m_CustomItemInfo.begin(); it != gCustomItem.m_CustomItemInfo.end(); it++)
	{
		LoadItemTexture((it->second.ItemIndex + MODEL_ITEM), ((it->second.ItemIndex >= BConverITEM(7, 0) && it->second.ItemIndex < BConverITEM(12, 0)) ? "Player\\" : "Item\\"), it->second.ModelName, -1);
	}
	...
	for (int n = 0; n < MAX_CUSTOM_JEWEL; n++)
	{
		if (gCustomJewel.m_CustomJewelInfo[n].Index != -1)
		{
			LoadItemTexture((gCustomJewel.m_CustomJewelInfo[n].ItemIndex + MODEL_ITEM), "Item\\", gCustomJewel.m_CustomJewelInfo[n].ModelName, -1);
		}
	}
	...
	for (int n = 0; n < MAX_CUSTOM_WING; n++)
	{
		if (gCustomWing.m_CustomWingInfo[n].Index != -1)
		{
			LoadItemTexture((gCustomWing.m_CustomWingInfo[n].ItemIndex + MODEL_ITEM), "Item\\", gCustomWing.m_CustomWingInfo[n].ModelName, -1);
		}
	}
	...
	for (std::map<int, CUSTOM_RF_GLOVES>::iterator it = gCustomBattleGloves.m_CustomGlovesInfo.begin(); it != gCustomBattleGloves.m_CustomGlovesInfo.end(); it++)
	{
		if (it->second.ItemIndex != -1)
		{
			LoadItemTexture(it->second.ItemIndex + MODEL_ITEM, "Item\\", it->second.ModelName, -1);
			LoadItemTexture(it->second.TextureLeftIndex + MODEL_ITEM, "Item\\", it->second.TextureLeftName, -1);
			LoadItemTexture(it->second.TextureRightIndex + MODEL_ITEM, "Item\\", it->second.TextureRightName, -1);
		}
	}
	...
	for (std::map<int, CUSTOM_PET_INFO>::iterator it = gCustomPet2.m_CustomPetInfo.begin(); it != gCustomPet2.m_CustomPetInfo.end(); it++)
	{
		//Console.AddMessage(1, "%d %s", it->second.ItemIndex, it->second.ModelName);
		LoadItemTexture((it->second.ItemIndex + MODEL_ITEM), "Item\\", it->second.ModelName, -1);
	}
	//==========================================
}
...
void OpenMonsterModel(int Type)
{
	...
	if (lpInfo != nullptr)
	{
		gLoadData.AccessModel(Index, lpInfo->Dir, lpInfo->BMDFile);
		gLoadData.OpenTexture(Index, lpInfo->Folder);
	}
	else
	{
...
void OpenSkills()
{
	...
	gLoadData.AccessModel(MODEL_DEATH_SPI_SKILL   ,"Data\\Skill\\","elf_skill");
	...
	//===NEW effect S6
	gLoadData.AccessModel(MODEL_phoenix_shot_effect, "Data\\Effect\\", "phoenix_shot_effect");
	gLoadData.OpenTexture(MODEL_phoenix_shot_effect, "Effect\\");
	gLoadData.AccessModel(MODEL_phoenix_shot_effect+1, "Data\\Effect\\", "wind_spin01");
	gLoadData.OpenTexture(MODEL_phoenix_shot_effect+1, "Effect\\");
	gLoadData.AccessModel(MODEL_phoenix_shot_effect+2, "Data\\Effect\\", "wind_spin02");
	gLoadData.OpenTexture(MODEL_phoenix_shot_effect+2, "Effect\\");
	gLoadData.AccessModel(MODEL_phoenix_shot_effect+3, "Data\\Effect\\", "wind_spin03");
	gLoadData.OpenTexture(MODEL_phoenix_shot_effect+3, "Effect\\");
	char Buffer[256] = { 0 };
	for (int kk = 0; kk < 6; ++kk)
	{
		sprintf(Buffer, "Effect\\xsuper000%d.jpg", kk + 1);
		LoadBitmap(Buffer, kk + BITMAP_XSUPER, GL_LINEAR, GL_CLAMP_TO_EDGE);
	}
...
void OpenBasicData(HDC hDC)
{
	...
	LoadBitmap("Effect\\2.jpg"     , BITMAP_2JPG,GL_LINEAR,GL_CLAMP_TO_EDGE);
...
//	LoadBitmap("Monster\\gigantiscorn_R.jpg", BITMAP_GIGANTIS_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	...
	::LoadBitmap("Item\\Ingameshop\\kacama_R.jpg", BITMAP_CHARACTERCARD_R_RA, GL_LINEAR, GL_CLAMP_TO_EDGE);
	...
	LoadBitmap("Interface\\InGameShop\\IGS_Storage_Page.tga", BITMAP_ANDROID_IGS_STORAGE_PAGE, GL_LINEAR);
	LoadBitmap("Interface\\InGameShop\\IGS_Storage_Page_Left.tga", BITMAP_ANDROID_IGS_STORAGE_PAGE_LEFT, GL_LINEAR);
	LoadBitmap("Interface\\InGameShop\\IGS_Storage_Page_Right.tga", BITMAP_ANDROID_IGS_STORAGE_PAGE_RIGHT, GL_LINEAR);
#endif
