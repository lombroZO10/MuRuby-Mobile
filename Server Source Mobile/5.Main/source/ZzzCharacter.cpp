...
namespace
{
    enum class CharacterDistanceBucket : int
    {
        Near = 0,
        Mid = 1,
        Far = 2,
    ...
    constexpr float kCharacterNearDistance = 1200.0f;
    constexpr float kCharacterMidDistance = 2600.0f;
    constexpr float kCharacterVeryFarDistance = 3900.0f;
    constexpr float kCharacterHardCullDistance = 5400.0f;
    constexpr float kCharacterNearAdaptiveMinDistance = 700.0f;
    constexpr float kCharacterDefaultCameraDistance = 1000.0f;
    constexpr float kCharacterMinZoomScale = 0.70f;
    constexpr float kCharacterMaxZoomScale = 1.75f;

    struct AdaptiveCharacterDistanceThresholds
    {
        float nearSq = kCharacterNearDistance * kCharacterNearDistance;
        float midSq = kCharacterMidDistance * kCharacterMidDistance;
        float veryFarSq = kCharacterVeryFarDistance * kCharacterVeryFarDistance;
        float hardCullSq = kCharacterHardCullDistance * kCharacterHardCullDistance;
    };

    uint32_t g_characterAdaptiveFrameId = 0;
    int g_characterFarRenderCursor = 0;
    CharacterPerfSnapshot g_characterPerfSnapshot = {};
    int g_characterVisibleRenderCount = 0;
    int g_characterVisibleMonsterRenderCount = 0;
    int g_characterOccupiedTerrainIndices[MAX_CHARACTERS_CLIENT] = {};
    int g_characterOccupiedTerrainCount = 0;
    bool g_characterTerrainOccupancyInitialized = false;

    struct CharacterRenderLodContext
    {
        bool active = false;
        bool priority = false;
        bool allowShadow = true;
        bool allowSupplemental = true;
        bool allowAttachments = true;
        bool reuseAnimationPose = false;
    };

    CharacterRenderLodContext g_characterRenderLodContext = {};

    inline void ClearCharacterTerrainOccupancy()
    {
        if (!g_characterTerrainOccupancyInitialized)
        {
            for (int i = 0; i < TERRAIN_SIZE * TERRAIN_SIZE; ++i)
            {
                TerrainWall[i] &= static_cast<WORD>(~TW_CHARACTER);
            }
            g_characterTerrainOccupancyInitialized = true;
        }

        for (int i = 0; i < g_characterOccupiedTerrainCount; ++i)
        {
            const int index = g_characterOccupiedTerrainIndices[i];
            TerrainWall[index] &= static_cast<WORD>(~TW_CHARACTER);
        }
        g_characterOccupiedTerrainCount = 0;
    }

    inline void MarkCharacterTerrainOccupancy(const int index)
    {
        if ((TerrainWall[index] & TW_CHARACTER) == TW_CHARACTER)
        {
            return;
        }

        if (g_characterOccupiedTerrainCount < MAX_CHARACTERS_CLIENT)
        {
            g_characterOccupiedTerrainIndices[g_characterOccupiedTerrainCount++] = index;
        }
        TerrainWall[index] |= TW_CHARACTER;
    }

    inline bool IsCharacterAdaptiveEnabled()
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        return SceneFlag == MAIN_SCENE && Hero != nullptr && !CameraTopViewEnable;
#else
        return false;
#endif
    }
    ...
    inline float GetCharacterDistanceSq2D(const OBJECT* o)
    {
        const float dx = CameraPosition[0] - o->Position[0];
        const float dy = CameraPosition[1] - o->Position[1];
        return dx * dx + dy * dy;
    }

    inline float GetCharacterZoomScale()
    {
        float distanceTarget = CameraDistanceTarget;
        if (!(distanceTarget > 1.0f))
        {
            distanceTarget = kCharacterDefaultCameraDistance;
        }

        const float rawScale = distanceTarget / kCharacterDefaultCameraDistance;
        return std::clamp(rawScale, kCharacterMinZoomScale, kCharacterMaxZoomScale);
    }

    inline AdaptiveCharacterDistanceThresholds GetCharacterDistanceThresholds()
    {
        const float zoomScale = GetCharacterZoomScale();
        const float nearDistance = kCharacterNearDistance * zoomScale;
        const float midDistance = kCharacterMidDistance * zoomScale;
        const float veryFarDistance = kCharacterVeryFarDistance * zoomScale;
        const float hardCullDistance = kCharacterHardCullDistance * zoomScale;

        AdaptiveCharacterDistanceThresholds thresholds;
        thresholds.nearSq = nearDistance * nearDistance;
        thresholds.midSq = midDistance * midDistance;
        thresholds.veryFarSq = veryFarDistance * veryFarDistance;
        thresholds.hardCullSq = hardCullDistance * hardCullDistance;
        return thresholds;
    }

    inline CharacterDistanceBucket ClassifyCharacterDistance(const float distSq, const AdaptiveCharacterDistanceThresholds& thresholds)
    {
        if (distSq <= thresholds.nearSq)
        {
            return CharacterDistanceBucket::Near;
        }

        if (distSq <= thresholds.midSq)
        {
            return CharacterDistanceBucket::Mid;
        }

        return CharacterDistanceBucket::Far;
    }

    inline void TrackCharacterMoveBucket(const CharacterDistanceBucket bucket)
    {
        switch (bucket)
        {
        case CharacterDistanceBucket::Near:
            ++g_characterPerfSnapshot.moveNearCandidates;
            break;
        case CharacterDistanceBucket::Mid:
            ++g_characterPerfSnapshot.moveMidCandidates;
            break;
        case CharacterDistanceBucket::Far:
            ++g_characterPerfSnapshot.moveFarCandidates;
            break;
        }
    }

    inline void TrackCharacterRenderBucket(const CharacterDistanceBucket bucket)
    {
        switch (bucket)
        {
        case CharacterDistanceBucket::Near:
            ++g_characterPerfSnapshot.renderNearCandidates;
            break;
        case CharacterDistanceBucket::Mid:
            ++g_characterPerfSnapshot.renderMidCandidates;
            break;
        case CharacterDistanceBucket::Far:
            ++g_characterPerfSnapshot.renderFarCandidates;
            break;
        }
    }

    inline bool IsPriorityCharacter(const int index, const CHARACTER* c)
    {
        if (c == Hero)
        {
            return true;
        ...
        if (index == SelectedCharacter || index == SelectedNpc)
        {
            return true;
        }

        return Hero != nullptr && Hero->TargetCharacter == c->Key;
    }

    inline bool IsAdaptiveMonsterCharacter(const CHARACTER* c)
    {
        return c != nullptr && c->Object.Kind == KIND_MONSTER;
    }

    inline float GetCharacterScreenImportance(const CHARACTER* c, const float distSq)
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        if (c == nullptr)
        {
        ...
        const float extentX = fabsf(o->BoundingBoxMax[0] - o->BoundingBoxMin[0]) * scale;
        const float extentY = fabsf(o->BoundingBoxMax[1] - o->BoundingBoxMin[1]) * scale;
        const float extentZ = fabsf(o->BoundingBoxMax[2] - o->BoundingBoxMin[2]) * scale;
        const float dominantExtent = (std::max)((std::max)(extentX, extentY), extentZ);
        const float footprintExtent = (std::max)(extentX, extentY);
        const float weightedExtent = (std::max)(dominantExtent, footprintExtent * 0.95f);
        const float safeDistSq = (std::max)(distSq, 1.0f);
        return (weightedExtent * weightedExtent) / safeDistSq;
#else
        (void)c;
        (void)distSq;
        return 1.0f;
    ...
    inline bool ShouldCullMonsterByImportance(const CHARACTER* c,
        const CharacterDistanceBucket bucket,
        const float distSq,
        const bool isPriority)
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        if (IsAndroidFullCharacterVisibilityMode() ||
            !IsCharacterAdaptiveEnabled() || c == nullptr || isPriority || !IsAdaptiveMonsterCharacter(c))
        {
            return false;
        }

        const float currentFps = static_cast<float>(FPS);
        if (currentFps <= 0.0f || currentFps >= 26.0f)
        {
            return false;
        }

        const int visibleMonsterCount = g_characterVisibleMonsterRenderCount;
        if (visibleMonsterCount < 10)
        {
            return false;
        }

        const bool stressedFrame = (currentFps < 22.0f);
        const bool severeFrame = (currentFps < 19.0f);
        const bool stadiumScene = (gMapManager.WorldActive == WD_6STADIUM);
        const float nearAdaptiveMinDistanceSq =
            kCharacterNearAdaptiveMinDistance * kCharacterNearAdaptiveMinDistance;
        const float importance = GetCharacterScreenImportance(c, distSq);

        float minImportance = -1.0f;
        if (bucket == CharacterDistanceBucket::Far)
        {
            minImportance = 0.0048f;
            if (visibleMonsterCount >= 14 && stressedFrame)
            {
                minImportance = 0.0074f;
            }
            if (stadiumScene)
            {
                minImportance = (std::max)(minImportance, stressedFrame ? 0.0096f : 0.0068f);
            }
            if (visibleMonsterCount >= 16 && severeFrame)
            {
                minImportance = (std::max)(minImportance, 0.0118f);
            }
        }
        else if (bucket == CharacterDistanceBucket::Mid &&
            stadiumScene &&
            visibleMonsterCount >= 15 &&
            severeFrame &&
            distSq > nearAdaptiveMinDistanceSq * 1.8f)
        {
            minImportance = 0.0044f;
        }

        return minImportance > 0.0f && importance < minImportance;
#else
        (void)c;
        (void)bucket;
        (void)distSq;
    ...
    inline bool ShouldRunCharacterStride(const int index, const int key, const uint32_t period, const uint32_t salt)
    {
        if (period <= 1u)
        {
            return true;
        ...
        const uint32_t keyHash = static_cast<uint32_t>(key + 2048) * 2654435761u;
        const uint32_t indexHash = static_cast<uint32_t>(index + 1) * 2246822519u;
        const uint32_t hash = keyHash ^ indexHash ^ salt;
        return ((g_characterAdaptiveFrameId + hash) % period) == 0u;
    }

    inline void ResetCharacterRenderLodContext()
    {
        g_characterRenderLodContext = {};
    }

    inline void ConfigureCharacterRenderLodContext(const CHARACTER* c,
        const CharacterDistanceBucket bucket,
        const bool isPriority)
    {
        ResetCharacterRenderLodContext();

#if defined(__ANDROID__) || defined(MU_IOS)
        if (IsAndroidFullCharacterVisibilityMode() ||
            !IsCharacterAdaptiveEnabled() || c == nullptr)
        {
            return;
        }
        ...
        const bool isMonster = IsAdaptiveMonsterCharacter(c);
        const bool heavyFrame = (currentFps < 32.0f);
        const bool severeFrame = (currentFps < 28.0f);
        const bool criticalFrame = (currentFps < 24.0f);
        const bool nearBucket = (bucket == CharacterDistanceBucket::Near);
        const bool farBucket = (bucket == CharacterDistanceBucket::Far);
        const bool midBucket = (bucket == CharacterDistanceBucket::Mid);
        const bool crowdHeavy =
            (g_characterVisibleRenderCount >= 18) ||
            (g_characterVisibleMonsterRenderCount >= 14);
        const bool crowdExtreme =
            (g_characterVisibleRenderCount >= 24) ||
            (g_characterVisibleMonsterRenderCount >= 20);
        const bool monsterCrowdHeavy = (g_characterVisibleMonsterRenderCount >= 12);
        const bool monsterCrowdExtreme = (g_characterVisibleMonsterRenderCount >= 16);
        const bool stadiumMonsterCrowd =
            (gMapManager.WorldActive == WD_6STADIUM && g_characterVisibleMonsterRenderCount >= 10);

        g_characterRenderLodContext.active = true;
        g_characterRenderLodContext.priority = isPriority;
        ...
        if (isMonster)
        {
            if ((heavyFrame && (monsterCrowdHeavy || stadiumMonsterCrowd) && !nearBucket) ||
                (criticalFrame && (monsterCrowdHeavy || stadiumMonsterCrowd)))
            {
                g_characterRenderLodContext.allowShadow = false;
            }

            if ((stadiumMonsterCrowd && heavyFrame) ||
                (monsterCrowdHeavy && severeFrame) ||
                (monsterCrowdExtreme && criticalFrame))
            {
                g_characterRenderLodContext.allowSupplemental = false;
            }

            if ((farBucket && heavyFrame) ||
                (midBucket && severeFrame) ||
                (stadiumMonsterCrowd && criticalFrame))
            {
                g_characterRenderLodContext.allowAttachments = false;
            }

            if (!nearBucket || criticalFrame)
            {
                return;
            }
        ...
        if (heavyFrame && crowdHeavy)
        {
            g_characterRenderLodContext.allowShadow = false;
        }

        if (severeFrame || crowdExtreme)
        {
            g_characterRenderLodContext.allowSupplemental = false;
        }

        if (farBucket && severeFrame)
        {
            g_characterRenderLodContext.allowAttachments = false;
        }
        else if (crowdHeavy && heavyFrame)
        {
            g_characterRenderLodContext.allowAttachments = false;
        }

        if (farBucket && criticalFrame)
        {
            g_characterRenderLodContext.allowShadow = false;
            g_characterRenderLodContext.allowSupplemental = false;
            g_characterRenderLodContext.allowAttachments = false;
    ...
    inline bool ShouldRenderCharacterShadowPass()
    {
        return !g_characterRenderLodContext.active || g_characterRenderLodContext.allowShadow;
    }

    inline bool ShouldRenderCharacterSupplementalVisuals()
    {
        return !g_characterRenderLodContext.active || g_characterRenderLodContext.allowSupplemental;
    }

    inline bool ShouldRenderCharacterAttachments()
    {
        return !g_characterRenderLodContext.active || g_characterRenderLodContext.allowAttachments;
    }

    inline bool ShouldReuseCharacterAnimationPoseContext()
    {
        return g_characterRenderLodContext.active && g_characterRenderLodContext.reuseAnimationPose;
    }

    inline bool ShouldReuseCharacterAnimationPose(const int index,
        const CHARACTER* c,
        const CharacterDistanceBucket bucket,
        const bool isPriority)
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        if (!IsCharacterAdaptiveEnabled() ||
            c == nullptr ||
            isPriority)
        {
            return false;
        ...
        if (o->Type != MODEL_PLAYER ||
            ...
            !o->m_bAdaptivePoseReady ||
            o->m_iAdaptivePoseAction != o->CurrentAction)
        {
            return false;
        }

        const float currentFps = static_cast<float>(FPS);
        if (currentFps <= 0.0f || currentFps >= 50.0f)
        {
            return false;
        }
        ...
        if (bucket == CharacterDistanceBucket::Far)
        {
            period = (currentFps < 34.0f || g_characterVisibleRenderCount >= 18) ? 3u : 2u;
        }
        else if (bucket == CharacterDistanceBucket::Mid &&
            (currentFps < 38.0f || g_characterVisibleRenderCount >= 24))
        {
            period = 2u;
        }

        return period > 1u && !ShouldRunCharacterStride(index, c->Key, period, 0x4b1d2f3u);
#else
        (void)index;
        (void)c;
        (void)bucket;
    ...
    inline uint32_t ComputeCharacterMoveBudgetPeriod(const int liveCandidates, const float fps)
    {
        if (liveCandidates <= 0 || fps <= 0.0f)
        {
            return 1u;
        }

        int moveBudget = liveCandidates;
        if (fps < 16.0f)
        {
            moveBudget = (std::max)(14, (liveCandidates * 36) / 100);
        }
        else if (fps < 22.0f)
        {
            moveBudget = (std::max)(18, (liveCandidates * 48) / 100);
        }
        else if (fps < 28.0f)
        {
            moveBudget = (std::max)(22, (liveCandidates * 58) / 100);
        }
        else if (fps < 34.0f)
        {
            moveBudget = (std::max)(24, (liveCandidates * 68) / 100);
        }
        else if (fps < 42.0f)
        {
            moveBudget = (std::max)(28, (liveCandidates * 82) / 100);
        }

        moveBudget = (std::min)(moveBudget, liveCandidates);
        if (moveBudget <= 0 || moveBudget >= liveCandidates)
        {
            return 1u;
        }

        const int period = (liveCandidates + moveBudget - 1) / moveBudget;
        return static_cast<uint32_t>(std::clamp(period, 1, 5));
    }

    inline void RenderCharacterEntry(const int index,
        CHARACTER* c,
        OBJECT* o,
        const CharacterDistanceBucket bucket,
        const bool isPriority)
    {
        ConfigureCharacterRenderLodContext(c, bucket, isPriority);
        const bool reuseAnimationPose =
            ShouldReuseCharacterAnimationPose(index, c, bucket, isPriority);
        if (reuseAnimationPose && !g_characterRenderLodContext.active)
        {
            g_characterRenderLodContext.active = true;
            g_characterRenderLodContext.allowShadow = true;
            g_characterRenderLodContext.allowSupplemental = true;
            g_characterRenderLodContext.allowAttachments = true;
        }
        g_characterRenderLodContext.reuseAnimationPose = reuseAnimationPose;
        if (o->Type == MODEL_PLAYER)
        {
            ++g_characterPerfSnapshot.renderPlayers;
        }
        else
        {
            ++g_characterPerfSnapshot.renderMonsters;
        }
...
CharacterPerfSnapshot ConsumeCharacterPerfSnapshot()
{
    const CharacterPerfSnapshot snapshot = g_characterPerfSnapshot;
    g_characterPerfSnapshot = {};
    return snapshot;
}
...
void SetPlayerStop(CHARACTER *c)
{
	...
	if(c->Object.Type==MODEL_PLAYER)
	{
		...
		if ((c->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5) && !c->SafeZone && (c->CacheTimeAttack < GetTickCount()))
#else
		if ((c->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5) && !c->SafeZone)
#endif
		{
...
#if(CB_ATTACK_HIDEN_PET)
		else if ((c->Helper.Type == MODEL_HELPER + 4 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6) && !c->SafeZone && (c->CacheTimeAttack < GetTickCount()))
#else
...
void SetPlayerWalk(CHARACTER *c)
{
	...
	if(!c->SafeZone && c->Run < 40)
	{
		if (gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK || gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK_LORD 
			...
			|| gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 1
	...
	if(c->Object.Type==MODEL_PLAYER)
	{
...
#if(CB_ATTACK_HIDEN_PET)
		if ((c->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5) && !c->SafeZone && (c->CacheTimeAttack < GetTickCount())) //mang pet di chuyen
#else
		if ((c->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5) && !c->SafeZone) //mang pet di chuyen
#endif	
		{
        ...
        else if ( (/*c->Weapon[1].Type==MODEL_HELPER+5 ||*/  c->GetEquipedPetInfo(0)->m_dwPetType == PET_TYPE_DARK_SPIRIT) && c->SafeZone) //c->m_PetInfo->m_dwPetType == PET_TYPE_DARK_SPIRIT
        {
		//	g_ConsoleDebug->Write(MCD_ERROR, " Get DL %d ", c->GetEquipedPetInfo(0)->m_dwPetType);
...
//				    SetAction(&c->Object,PLAYER_FLY_RIDE_WEAPON);
    ...
    else if ( gCharacterManager.GetBaseClass(c->Class)==CLASS_DARK_LORD && (c->Helper.Type==MODEL_HELPER+4 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6 )&& !c->SafeZone )
    {
		PlayBuffer ( SOUND_RUN_DARK_HORSE_1+rand()%3, o );
    }
#if(CB_ATTACK_HIDEN_PET)
	else if ((c->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5) && !c->SafeZone && (c->CacheTimeAttack < GetTickCount())
...
float BCalculateAttackSpeed(int Type)
{
	//float SpeedReturn = Type == 1 ? CharacterAttribute->PrintPlayer.ViewPhysiSpeed : CharacterAttribute->PrintPlayer.ViewMagicSpeed;
	float SpeedReturn = Type == 1 ? CharacterAttribute->AttackSpeed : CharacterAttribute->MagicSpeed;

	
	switch (gCharacterManager.GetBaseClass(CharacterAttribute->Class))
	{
	case CLASS_WIZARD:
	case CLASS_SOULMASTER:
	case CLASS_GRANDMASTER:
	{
		if (CharacterAttribute->Skill[Hero->CurrentSkill] == 9 ||
			CharacterAttribute->Skill[Hero->CurrentSkill] == 8 ||
			CharacterAttribute->Skill[Hero->CurrentSkill] == 13 || CharacterAttribute->Skill[Hero->CurrentSkill] == 382 ||
			CharacterAttribute->Skill[Hero->CurrentSkill] == 38 || CharacterAttribute->Skill[Hero->CurrentSkill] == 387 ||
			CharacterAttribute->Skill[Hero->CurrentSkill] == 39 ||
			CharacterAttribute->Skill[Hero->CurrentSkill] == 385 ||
			CharacterAttribute->Skill[Hero->CurrentSkill] == 5 || CharacterAttribute->Skill[Hero->CurrentSkill] == 378)
		{
			if (SpeedReturn > 450 && SpeedReturn < 480)
			{
				SpeedReturn = 480;
			}
			else if (SpeedReturn > 600 && SpeedReturn < 690)
			{
				SpeedReturn = 600;
			}
			else if (SpeedReturn > 850 && SpeedReturn < 1105)
			{
				SpeedReturn = 850;
			}
			else if (SpeedReturn > 1350 && SpeedReturn < 2355)
			{
				SpeedReturn = 1350;
			}
			else if (SpeedReturn > 2355)
			{
				SpeedReturn = 2355;
			}
		}
		//===VOng Tron Lua
		else if (CharacterAttribute->Skill[Hero->CurrentSkill] == 14 || CharacterAttribute->Skill[Hero->CurrentSkill] == 381)
		{
			if (SpeedReturn > 3276)
			{
				SpeedReturn = 3276;
	...
	break;
		...
		case CLASS_HIGHELF:
		{
			if (CharacterAttribute->Skill[Hero->CurrentSkill] == 52 ||
				CharacterAttribute->Skill[Hero->CurrentSkill] == 51 ||
				CharacterAttribute->Skill[Hero->CurrentSkill] == 424 ||
				CharacterAttribute->Skill[Hero->CurrentSkill] == 416)
			{
				if (SpeedReturn > 400)
				{
					SpeedReturn = 400;
			...
			if (CharacterAttribute->Skill[Hero->CurrentSkill] == 24 ||
				CharacterAttribute->Skill[Hero->CurrentSkill] == 414)
			{
				if (SpeedReturn > 600)
				{
					SpeedReturn = 600;
		...
		{
			if (CharacterAttribute->Skill[Hero->CurrentSkill] == 55 ||
				CharacterAttribute->Skill[Hero->CurrentSkill] == 490)
			{
				if (SpeedReturn > 1368)
				{
					SpeedReturn = 1368;
				...
				CharacterAttribute->Skill[Hero->CurrentSkill] == 5 || CharacterAttribute->Skill[Hero->CurrentSkill] == 378)
			{
				...
				else if (SpeedReturn > 2850)
				{
					SpeedReturn = 2850;
				}
			}
			if (CharacterAttribute->Skill[Hero->CurrentSkill] == 14)
			{
				if (SpeedReturn > 3276)
				{
					SpeedReturn = 3276;
		...
		{
			if (CharacterAttribute->Skill[Hero->CurrentSkill] == 78 ||
				CharacterAttribute->Skill[Hero->CurrentSkill] == 518)
			{
				if (SpeedReturn > 249 && SpeedReturn < 264)
				{
					SpeedReturn = 249;
				}
				else if (SpeedReturn > 324 && SpeedReturn < 367)
				{
					SpeedReturn = 324;
				}
				else if (SpeedReturn > 449 && SpeedReturn < 575)
				{
					SpeedReturn = 449;
				}
				else if (SpeedReturn > 699 && SpeedReturn < 1200)
				{
					SpeedReturn = 699;
				}
				else if (SpeedReturn > 1449)
				{
					SpeedReturn = 1449;
				}
			}
		...
		{
			if (CharacterAttribute->Skill[Hero->CurrentSkill] == 261 ||
				CharacterAttribute->Skill[Hero->CurrentSkill] == 490 ||
				CharacterAttribute->Skill[Hero->CurrentSkill] == 265 ||
				CharacterAttribute->Skill[Hero->CurrentSkill] == 555)
			{
				if (SpeedReturn > 4476)
				{
					SpeedReturn = 4476;
				}
				if (SpeedReturn > 754 && SpeedReturn < 1087)
				{
					SpeedReturn = 754;
				}
			}
	...
	//gInterface.DrawMessage(1, "Class %d Skill %d Speed %f", gCharacterManager.GetBaseClass(CharacterAttribute->Class), CharacterAttribute->Skill[Hero->CurrentSkill], SpeedReturn);
	return SpeedReturn;
}
...
void SetAttackSpeed()
{
	...
	//===Fix Attack Speed
	CharacterAttribute->AttackSpeed = (WORD)CharacterAttribute->PrintPlayer.ViewPhysiSpeed;
	CharacterAttribute->MagicSpeed = (WORD)CharacterAttribute->PrintPlayer.ViewMagicSpeed;


	float AttackSpeed1 = (BCalculateAttackSpeed(1)) * 0.004f;    // 
	float MagicSpeed1 = (BCalculateAttackSpeed(0)) * 0.004f;    // 
	float MagicSpeed2 = (BCalculateAttackSpeed(0)) * 0.002f;    // 
	...
	float RageAttackSpeed = (BCalculateAttackSpeed(1)) * 0.002f;
	...
	Models[MODEL_PLAYER].Actions[PLAYER_RAGE_UNI_ATTACK].PlaySpeed = AttackSpeed1 + 0.25;
	Models[MODEL_PLAYER].Actions[PLAYER_RAGE_UNI_ATTACK_ONE_RIGHT].PlaySpeed = AttackSpeed1 + 0.25;
	Models[MODEL_PLAYER].Actions[PLAYER_SKILL_MUITENPHUONGHOANG].PlaySpeed = 0.74;
}

//void SetAttackSpeed()
//{
//	WORD mAttackSpeed1 = CharacterAttribute->AttackSpeed;
//	WORD mMagicSpeed  = CharacterAttribute->MagicSpeed;
//	//g_ConsoleDebug->Write(MCD_ERROR, "Skill %d", CharacterAttribute->Skill[Hero->CurrentSkill]);
//
//	//float AttackSpeed1 = (CalculateAttackSpeed(1)) * 0.002f;    // 
//	//float MagicSpeed1 = (CalculateAttackSpeed(0)) * 0.002f;    // 
//	//float MagicSpeed2 = (CalculateAttackSpeed(0)) * 0.002f;    // 
//
//	float AttackSpeed1 = (CalculateAttackSpeed(1)) * 0.001f;    // 
//	float MagicSpeed1 = (CalculateAttackSpeed(0)) * 0.001f;    // 
//	float MagicSpeed2 = (CalculateAttackSpeed(0)) * 0.002f;    // 
//
//	//MagicSpeed2 = CalculateAttackSpeed();
//	//=== Fix Speed Test
//	/*float AttackSpeed1 = (mAttackSpeed1) * 0.001f;    // 
//
//	if (gCharacterManager.GetBaseClass(CharacterAttribute->Class) == CLASS_KNIGHT)
//	{
//		AttackSpeed1 = (mAttackSpeed1) * 0.0022f;
//	}
//	float MagicSpeed1 = (mMagicSpeed) * 0.001f;    // 
//	float MagicSpeed2 = (mMagicSpeed) * 0.002f;    // 
//	//float RageAttackSpeed = (mAttackSpeed1) * 0.01f;
//	if (gCharacterManager.GetBaseClass(CharacterAttribute->Class) == CLASS_WIZARD)
//	{
//		MagicSpeed2 = (mMagicSpeed) * 0.002f;    // 
//	}
//	else if (gCharacterManager.GetBaseClass(CharacterAttribute->Class) == CLASS_SUMMONER)
//	{
//		MagicSpeed2 = (mMagicSpeed) * 0.02f;    //
//	}
//
//	// -> Fix speed attack ELF & MG
//	if (gCharacterManager.GetBaseClass(CharacterAttribute->Class) == CLASS_ELF)
//	{
//		AttackSpeed1 = (mAttackSpeed1) * 0.0028f;
//	}
//
//	if (gCharacterManager.GetBaseClass(CharacterAttribute->Class) == CLASS_DARK)
//	{
//		MagicSpeed1 = (mMagicSpeed) * 0.006f;    // 
//	}
//	//===
//	*/
// 	Models[MODEL_PLAYER].Actions[PLAYER_ATTACK_FIST].PlaySpeed = 0.6f + AttackSpeed1;
...
void SetPlayerShock(CHARACTER *c,int Hit)
{
	...
	if(c->Helper.Type==MODEL_HELPER+4 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6) return;
	...
	if(Hit > 0)
	{
		if(c->Object.Type==MODEL_PLAYER)
		{
			if(c->Helper.Type == MODEL_HELPER+37 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5)
			{
...
bool AttackStage(CHARACTER* c, OBJECT* o)
{
    ...
    g_iLimitAttackTime = g_iLimitAttackTimeSet;
    switch ( iSkill)
    {
	...
	case MASTER_SKILL_ADD_DEATH_STAB_MASTERED:
		{
			...
			if ( c->AttackTime>=12 )
			{
				c->AttackTime = g_iLimitAttackTimeSet;
			}
		}
		break;
	...
	if(c->JumpTime > 0)
	{
        ...
        else
        {
			...
			o->Position[1] += (((float)c->TargetY + 0.5f) * TERRAIN_SCALE - o->Position[1]) * Speed + FPS_ANIMATION_FACTOR;
     	    ...
     	    if(o->Type != MODEL_BALL) o->Position[2] = RequestTerrainHeight(o->Position[0],o->Position[1]);
...
#if(CB_ATTACK_HIDEN_PET)
			if ((c->Helper.Type >= MODEL_HELPER + 2 && c->Helper.Type <= MODEL_HELPER + 4) || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6 && !c->SafeZone && (c->CacheTimeAttack < GetTickCount()))
#else
			if ((c->Helper.Type >= MODEL_HELPER + 2 && c->Helper.Type <= MODEL_HELPER + 4) || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6 && !c->SafeZone)
#endif			
			{
...
// 				CreateEffect(MODEL_LIGHTNING_SHOCK, to->Position, to->Angle, to->Light, 2, to);
		...
		if(c->TargetCharacter == -1)
		{
			...
			if(
				...
				(AT_SKILL_MANY_ARROW_UP <= ( c->Skill) && ( c->Skill) <= AT_SKILL_MANY_ARROW_UP+4))
				Skill = 1;
			if ( ( o->Type==MODEL_PLAYER && 
			...
			case AT_SKILL_ALICE_THORNS:
				{
					...
					if(iSkillIndex == AT_SKILL_ALICE_SLEEP 
						|| iSkillIndex == MASTER_SKILL_ADD_SLEEP_IMPROVED
						|| iSkillIndex == AT_SKILL_ALICE_THORNS|| (AT_SKILL_ALICE_SLEEP_UP <= iSkillIndex && iSkillIndex <= AT_SKILL_ALICE_SLEEP_UP+4))
					{
...
void MoveCharacterVisual(CHARACTER *c,OBJECT *o)
{
	...
	if(o->Visible)
	{
		...
		if( !g_isCharacterBuff(o, eDeBuff_Harden) && !g_isCharacterBuff(o, eDeBuff_Stun) 
			...
			&& !g_isCharacterBuff(o, EFFECT_FITNESS_IMPROVED)
			&& !g_isCharacterBuff((o), EFFECT_GREATER_DEFENSE_SUCCESS_RATE_IMPROVED)
			&& !g_isCharacterBuff((o), EFFECT_GREATER_DEFENSE_SUCCESS_RATE_ENHANCED)
...
#if(CB_ATTACK_HIDEN_PET)
		if ((c->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5) && !c->SafeZone && !isholyitem && (c->CacheTimeAttack < GetTickCount()))
#else
		if ((c->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5) && !c->SafeZone && !isholyitem)
#endif		
		{
...
#if(CB_ATTACK_HIDEN_PET)
		else if ((c->Helper.Type == MODEL_HELPER + 4 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6) && !c->SafeZone && !isholyitem && (c->CacheTimeAttack < GetTickCount()))
#else
		else if ((c->Helper.Type == MODEL_HELPER + 4 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6) && !c->SafeZone && !isholyitem && (c->CacheTimeAttack < GetTickCount()))
#endif 
		{
...
void MoveCharacterClient(CHARACTER *cc)
{
	...
	if(co->Live)
	{
		...
		if (GetTickCount() > cc->CacheTimeAttack_WING && cc->WingCache > 0)
		{
			cc->Wing.Type = cc->WingCache;
			cc->WingCache = -1;			
		}
...
{
    const Uint64 moveTicksStart = static_cast<Uint64>(MU_MobilePerfNow());
    ++g_characterAdaptiveFrameId;
    const bool adaptiveEnabled = IsCharacterAdaptiveEnabled();
    const bool fullCharacterVisibility = IsAndroidFullCharacterVisibilityMode();
    const AdaptiveCharacterDistanceThresholds adaptiveThresholds = GetCharacterDistanceThresholds();
    const float currentFps = static_cast<float>(FPS);
    const bool heavyFrame = (currentFps > 0.f && currentFps < 32.f);
    const bool severeFrame = (currentFps > 0.f && currentFps < 24.f);
    const float nearAdaptiveMinDistanceSq =
        kCharacterNearAdaptiveMinDistance * kCharacterNearAdaptiveMinDistance;
    int liveCharacterCandidates = 0;

    ClearCharacterTerrainOccupancy();
	...
	for(int i=0;i<MAX_CHARACTERS_CLIENT;i++)		
	{
		...
		if(to->Live)
		{
            ++liveCharacterCandidates;
			if(tc->Dead==0 && to->Kind!=KIND_TRAP)
			{
      			const int index = TERRAIN_INDEX_REPEAT((tc->PositionX),(tc->PositionY));
				MarkCharacterTerrainOccupancy(index);
			}
    ...
    const uint32_t moveBudgetPeriod = (adaptiveEnabled && !fullCharacterVisibility)
        ? ComputeCharacterMoveBudgetPeriod(liveCharacterCandidates, currentFps)
        : 1u;
	...
	for(int i=0;i<MAX_CHARACTERS_CLIENT;i++)
	{
        ...
        ++g_characterPerfSnapshot.moveCandidates;
        const float distSq = GetCharacterDistanceSq2D(to);
        const CharacterDistanceBucket bucket = ClassifyCharacterDistance(distSq, adaptiveThresholds);
        TrackCharacterMoveBucket(bucket);
        const bool isPriority = IsPriorityCharacter(i, tc);

        bool deferMove = false;
        if (adaptiveEnabled && !fullCharacterVisibility && !isPriority)
        {
            uint32_t movePeriod = 1u;
            const int movePressure = g_characterPerfSnapshot.moveCandidates;
            if (bucket == CharacterDistanceBucket::Far)
            {
                movePeriod = (distSq > adaptiveThresholds.veryFarSq) ? 3u : 2u;
                if (movePressure > 80)
                {
                    ++movePeriod;
                }
            ...
            else if (bucket == CharacterDistanceBucket::Mid && movePressure > 180)
            {
                movePeriod = (movePressure > 260) ? 3u : 2u;
            }
            else if (bucket == CharacterDistanceBucket::Near && distSq > nearAdaptiveMinDistanceSq)
            {
                if ((movePressure > 220 && heavyFrame) || (movePressure > 280 && severeFrame))
                {
                    movePeriod = 2u;
                }

                if (movePressure > 340 && severeFrame)
                {
                    movePeriod = 3u;
                }
            }
            ...
            if (severeFrame)
            {
                if (bucket == CharacterDistanceBucket::Far)
                {
                    movePeriod = (std::max)(movePeriod, uint32_t{3u});
                }
                else if (bucket == CharacterDistanceBucket::Mid)
                {
                    movePeriod = (std::max)(movePeriod, uint32_t{2u});
                }
                else if (distSq > nearAdaptiveMinDistanceSq && movePressure > 80)
                {
                    movePeriod = (std::max)(movePeriod, uint32_t{2u});
                }
            }

            if (moveBudgetPeriod > 1u && distSq > nearAdaptiveMinDistanceSq)
            {
                movePeriod = (std::max)(movePeriod, moveBudgetPeriod);
                if (severeFrame && bucket != CharacterDistanceBucket::Near)
                {
                    movePeriod = (std::min)(uint32_t{5u}, movePeriod + 1u);
                }
            }

            if (bucket == CharacterDistanceBucket::Near)
            {
                movePeriod = (std::min)(movePeriod, uint32_t{2u});
            }
            else if (bucket == CharacterDistanceBucket::Mid)
            {
                movePeriod = (std::min)(movePeriod, uint32_t{4u});
            }
            else
            {
                movePeriod = (std::min)(movePeriod, uint32_t{5u});
            }

            if (movePeriod > 1u)
            {
                deferMove = !ShouldRunCharacterStride(i, tc->Key, movePeriod, 0xa511e9b3u);
            }
        }

        if (deferMove)
        {
            ++g_characterPerfSnapshot.moveDeferred;
            if (tc != Hero && tc->Movement)
            {
                MoveCharacterPosition(tc);
            }
            continue;
        }

        ++g_characterPerfSnapshot.moveUpdated;
		MoveCharacterClient(tc);
	}
    MoveBlurs();	
    g_characterPerfSnapshot.moveTicks += static_cast<unsigned long long>(MU_MobilePerfNow() - moveTicksStart);
}
...
{		
	...
	if (c->MonsterIndex >= 529 && c->MonsterIndex <= 539 || !g_pNewUISystem->GetUI_NewOptionWindow()->OnOffGrap[g_pNewUISystem->GetUI_NewOptionWindow()->eBMDImg] && SceneFlag == MAIN_SCENE && c != Hero)
	{
	...
	if(Link)
	{
		...
		else if (Type == MODEL_SWORD_35_WING) //Fix Speed BMD
		{
			Vector(x, y, z, Angle);
			AngleMatrix(Angle, Matrix);
			Matrix[0][3] = 15.f;
			...
			{
				f->CurrentAction = 1;
				f->AnimationFrame = 0.0f;
				f->PriorAnimationFrame = 0.0f;
			}
			else
			{
				f->PlaySpeed = 0.03;
...
#endif //PBG_ADD_NEWCHAR_MONK_ITEM
		else if(Type>=MODEL_WING+40 || gCustomWing.CheckCustomWingByModelType(Type - MODEL_ITEM, 1))
		{
    		Vector(0.f,90.f,0.f,Angle);
			AngleMatrix(Angle,Matrix);
			Matrix[0][3] = -47.f;
		...
		else if((Type>=MODEL_BOW+8 && Type<MODEL_BOW+15) 
			|| (Type>=MODEL_BOW+16 && Type<MODEL_BOW+17) 
			|| gCustomBow.CheckCustomCross(Type - MODEL_ITEM) //cCrossBowAddPlayerDimension
		...
		else
		{
			...
			if(Type == MODEL_BOW+20
				/*|| gCustomBow.CheckCustomBow(Type - MODEL_ITEM)*/)//cBowAddPlayerDimension
			{
    ...
    if( ( c->Skill)==AT_SKILL_PIERCING 
		|| (c->Skill)== MASTER_SKILL_ADD_PENETRATION_IMPROVED
...
*/
	...
	//case MODEL_SWORD_35_EFFECT:
	//{
	//	float v151 = (double)(rand() % 30) / 300.0;
	//	int v152[8] = { 0 };
	//	int v150[8] = { 0 };
	//	float v42,v43, v66;
	//	v152[0] = 3;
	...
	//	v152[6] = 9;
	//	v152[7] = 10;
	//	Vector(0.18000001, 0.44999999, 0.22f, Light);
	//	Vector(0.f, 0.f, 0.f, p);
	//	for (int i14 = 0; i14 < 8; ++i14)
	//	{
	//		b->TransformPosition(BoneTransform[3 * v152[i14]], p, Position, true);
	//		v42 = (float)(rand() % 360);
	//		v66 = v151 + 0.300000011920929;
	//		CreateSprite(32380, Position, v66, Light, o, v42,0);
	//	}
	//	v150[0] = 13;
	//	v150[1] = 14;
	//	v150[2] = 15;
	...
	//	{
	//		b->TransformPosition(BoneTransform[3 * v152[i15]], p, Position, true);
	//		v43 = (float)(rand() % 360);
	//		v66 = v151 + 0.300000011920929;
	//		CreateSprite(32380, Position, v66, Light, o, v43,0);
	//	}
	//}
	//break;
	case MODEL_MACE+8:
...
void RenderCharacter(CHARACTER *c,OBJECT *o,int Select)
{
    ...
    {
        const bool reuseAdaptivePose =
            ShouldReuseCharacterAnimationPoseContext() &&
            o->EnableBoneMatrix &&
            o->BoneTransform != NULL &&
            o->m_bAdaptivePoseReady;

        if (reuseAdaptivePose)
        {
            BMD* adaptiveModel = &Models[o->Type];
            adaptiveModel->BodyHeight = 0.f;
            adaptiveModel->ContrastEnable = o->ContrastEnable;
            BodyLight(o, adaptiveModel);
            adaptiveModel->BodyScale = o->Scale;
            adaptiveModel->CurrentAction = o->CurrentAction;
            VectorCopy(o->Position, adaptiveModel->BodyOrigin);
        }
        else
        {
	...
	{
        const Uint64 monsterObjectTicksStart = static_cast<Uint64>(MU_MobilePerfNow());
        const bool allowMonsterSupplementalPasses = ShouldRenderCharacterSupplementalVisuals();
        BeginAdaptiveObjectRenderPassBudget(allowMonsterSupplementalPasses);
        ...
        EndAdaptiveObjectRenderPassBudget();
        ++g_characterPerfSnapshot.renderMonsterObjectCalls;
        g_characterPerfSnapshot.renderMonsterObjectTicks += static_cast<unsigned long long>(MU_MobilePerfNow() - monsterObjectTicksStart);
	}
	...
	{
        if (ShouldRenderCharacterShadowPass() && o->Alpha>=0.3f )
		{
            const Uint64 shadowTicksStart = static_cast<Uint64>(MU_MobilePerfNow());
    ...
    {
    	for (std::vector<jCEffectMonster::xMonsterEffectBBBBB>::iterator it = gggJCEffectMonster.m_MonsterEffect.begin(); it != gggJCEffectMonster.m_MonsterEffect.end(); it++)
    	{
    		if (it->IndexMonster == c->MonsterIndex)
    		{
    			vec3_t	v3EffectLightColor, v3EffectPosition;
    			{
    				Vector(it->Color[0], it->Color[1], it->Color[2], v3EffectLightColor);
    				Vector(it->Join[1], it->Join[2], it->Join[0], v3EffectPosition);

    				if (it->TypeEffect == 1)
    				{
    					b->TransformPosition(o->BoneTransform[it->EfectValue], p, v3EffectPosition, true);
    					CreateParticle(it->IDEffect, o->Position, o->Angle, v3EffectLightColor, 6, it->Scale, o);
    				}
    				else if (it->TypeEffect == 2)
    				{
    					b->TransformPosition(o->BoneTransform[it->EfectValue], p, v3EffectPosition, true);
    					CreateSprite(it->IDEffect, o->Position, it->Scale, v3EffectLightColor, o);
    				}
    			}
    		}
    	}

    	int MonsterGolden = gCustomMonsterGlow.CheckCustomByMonsterGolden(c->MonsterIndex);
    	...
    	{	
    		...
    		else
    		{
    			...
    			BoneManager::GetBonePosition(o, "Rabbit_2", vPos);
    			Vector(0.4f, 0.4f, 0.9f, vLight);
    			CreateSprite( BITMAP_LIGHT, vPos, 2.0f, vLight, o );

    			BoneManager::GetBonePosition(o, "Rabbit_4", vPos);
    			Vector(0.4f, 0.4f, 0.9f, vLight);
	...
	else if (c->MonsterIndex >= 529 && c->MonsterIndex <= 539 || !g_pNewUISystem->GetUI_NewOptionWindow()->OnOffGrap[g_pNewUISystem->GetUI_NewOptionWindow()->eBMDImg] && SceneFlag == MAIN_SCENE && c != Hero)
	{
		c->HideShadow = true;
...
// 			vec3_t vLight;
	...
	else if(o->Kind == KIND_PLAYER && o->Type == MODEL_PLAYER && (o->SubType == MODEL_PANDA || o->SubType >= 10000))
	{
		RenderPartObject(&c->Object,o->SubType,NULL,c->Light,o->Alpha,0,0,0,false,false,Translate,Select);
	}
...
// 							Select,RENDER_DOPPELGANGER|RENDER_BRIGHT|RENDER_CHROME);
					...
					{
						if (!g_pNewUISystem->GetUI_NewOptionWindow()->OnOffGrap[g_pNewUISystem->GetUI_NewOptionWindow()->eBMDImg] && SceneFlag == MAIN_SCENE && c != Hero)
						{
								RenderPartObject(&c->Object, Type, p, c->Light, o->Alpha, p->Level << 3, p->Option1, p->ExtOption, false, false, Translate,
									Select, RENDER_DOPPELGANGER | RENDER_BRIGHT | RENDER_TEXTURE);
						}
        ...
        {
			if(ShouldRenderCharacterAttachments() &&
                c->GuildMarkIndex>=0 && o->Type==MODEL_PLAYER && o->Alpha != 0.0f
	...
	if ( bCloak )
	{
		if ( !c->Object.m_pCloth)
		{
			CUSTOMCLOAK_DATA* CapeModel = gCloak.GetInfoByItem(c->Wing.Type - MODEL_ITEM);
			if (CapeModel)
			{
					int numCloth = (CapeModel->Mode == 1) ? 3 : 1;;
					CPhysicsCloth* pCloth = new CPhysicsCloth[numCloth];
	
					pCloth[0].Create(o, 19, 0.0f, 15.0f, 5.0f, 10, 10, 180.0f, 170.0f, CapeModel->IndexCloak, CapeModel->IndexCloak, PCT_CURVED | PCT_SHORT_SHOULDER | PCT_HEAVY | PCT_MASK_ALPHA);
					...
					if (CapeModel->Mode == 1)
					{
						pCloth[1].Create(o, 19, 25.0f, 15.0f, 2.0f, 2, 5, 12.0f, 180.0f, CapeModel->IndexStrip, CapeModel->IndexStrip, PCT_FLAT | PCT_SHAPE_NORMAL | PCT_COTTON | PCT_ELASTIC_RAGE_L | PCT_MASK_ALPHA);
						pCloth[1].AddCollisionSphere(0.0f, -15.0f, -20.0f, 35.0f, 2);
						pCloth[1].AddCollisionSphere(0.0f, 0.0f, 0.0f, 45.0f, 17);

						pCloth[2].Create(o, 19, -25.0f, 15.0f, 2.0f, 2, 5, 12.0f, 180.0f, CapeModel->IndexStrip, CapeModel->IndexStrip, PCT_FLAT | PCT_SHAPE_NORMAL | PCT_COTTON | PCT_ELASTIC_RAGE_R | PCT_MASK_ALPHA);
		...
		if( !g_isCharacterBuff(o, eBuff_Cloaking) )
        {
            for ( int i=0; i<o->m_byNumCloth; i++ )
            {
                if ( gCharacterManager.GetBaseClass( c->Class )==CLASS_DARK_LORD )
                {
                    if ( i==2 && ((c->Wing.Type!=MODEL_HELPER+30 && c->Wing.Type!=MODEL_WING+40 && !gCustomWing.CheckCustomWingByModelType(c->Wing.Type - MODEL_ITEM, 1) && c->Wing.Type!=MODEL_WING+130) && ( CloakLight[0]==1.f && CloakLight[1]==1.f && CloakLight[2]==1.f ) ) )
					{
						continue;
					}
				...
				if(gCharacterManager.GetBaseClass( c->Class )==CLASS_RAGEFIGHTER)
				{
					if(i==0 && ((c->Wing.Type!=MODEL_WING+49
						&& c->Wing.Type!=MODEL_WING+50
						 && !gCustomWing.CheckCustomWingByModelType(c->Wing.Type - MODEL_ITEM, 1)
	...
	//=== Render Effect Vuot Phuong Hoang
	for (int i5 = 0; i5 < 2; ++i5)
	{
		if (g_CMonkSystem.EqualItemModelType(c->Weapon[i5].Type) == MODEL_SWORD + 35)
		{
	...
	if ( !Bind )
	{
		if( !g_isCharacterBuff(o, eBuff_Cloaking) )
		{
			...
			if (
				...
				g_isCharacterBuff(o, EFFECT_GREATER_CRITICAL_DAMAGE_MASTERED) ||
				g_isCharacterBuff(o, EFFECT_GREATER_CRITICAL_DAMAGE_EXTENDED)
				
				&& o->Kind==KIND_PLAYER && o->Type==MODEL_PLAYER && (c->LastCritDamageEffect < (WorldTime - CritDamageEffectInterval)))
            {
				c->LastCritDamageEffect = WorldTime;
               bool    renderSkillWave = (rand_fps_check(20)) ? true : false;
                short   weaponType = -1;                
                Vector ( 0.f, 0.f, 0.f, p );
                Vector ( 1.f, 0.6f, 0.3f, Light );
                if ( c->Weapon[0].Type!=MODEL_BOW+15 )//&& ( c->Weapon[0].Type<MODEL_SHIELD || c->Weapon[0].Type>=MODEL_SHIELD+MAX_ITEM_INDEX ) ) //CustomArrow Cung
                {
				    b->TransformPosition ( o->BoneTransform[c->Weapon[0].LinkBone], p, Position, true );
                ...
                if ( c->Weapon[1].Type!=MODEL_BOW+7 && ( c->Weapon[1].Type<MODEL_SHIELD || c->Weapon[1].Type>=MODEL_SHIELD+MAX_ITEM_INDEX ) )//CustomArrow No
                {
                    ...
                    //CreateEffect ( BITMAP_FLARE_FORCE, Position, o->Angle, o->Light, 1, o, weaponType, c->Weapon[1].LinkBone ); //DAT FIX DL
                    if ( renderSkillWave==false )
                    {
                        //CreateEffect ( MODEL_DARKLORD_SKILL, Position, o->Angle, Light, 1 );
                    }
		...
		for(int i=0;i<2;i++)
		{
            ...
            PART_t *w = &c->Weapon[i];
			if ( w->Type!=-1 && w->Type!=MODEL_BOW+7 && w->Type!=MODEL_BOW+15 && w->Type!=MODEL_HELPER+5)//CustomArrow Cung,No
			{
	...
	case MODEL_PLAYER:
		...
		else if (o->CurrentAction >= PLAYER_SKILL_SLEEP	&& o->CurrentAction <= PLAYER_SKILL_SLEEP_FENRIR)
		{
			...
			if(iSkillType == AT_SKILL_ALICE_SLEEP
				...
				|| (AT_SKILL_ALICE_SLEEP_UP <= iSkillType && iSkillType <= AT_SKILL_ALICE_SLEEP_UP+4))	// nukun
			{
			...
			// cra04, clud64
			if(
				iSkillType == AT_SKILL_ALICE_SLEEP ||
				iSkillType == MASTER_SKILL_ADD_SLEEP_IMPROVED
						|| (AT_SKILL_ALICE_SLEEP_UP <= iSkillType && iSkillType <= AT_SKILL_ALICE_SLEEP_UP+4)
				)
			{
		...
		if(
			g_isCharacterBuff((&c->Object), eBuff_SwellOfMagicPower)||
			g_isCharacterBuff((&c->Object), EFFECT_MAGIC_CIRCLE_IMPROVED)||
			g_isCharacterBuff((&c->Object), EFFECT_MAGIC_CIRCLE_ENHANCED)
			)
		{
        ...
        if ( gMapManager.InChaosCastle()==false )
        {
				...
				|| c->BodyPart[BODYPART_BOOTS].Type == MODEL_BOOTS+43)
			{
                ...
                if ( EquipmentLevelSet>9 )
                {
					if (rand_fps_check(20))//(o->CurrentAction<PLAYER_WALK_MALE || o->CurrentAction>PLAYER_RUN_RIDE_WEAPON) && (rand()%6)==0)
                    {
                        VectorCopy ( o->Light, Light );
                        Vector ( 1.f, 1.f, 1.f, o->Light );
	...
	if (ShouldRenderCharacterAttachments() && gCharacterManager.GetBaseClass(c->Class) == CLASS_SUMMONER)
	{
		PART_t *w = &c->Weapon[1];
		g_SummonSystem.MoveEquipEffect(c, w->Type, w->Level, w->Option1);
	}
    g_characterPerfSnapshot.renderAttachmentTicks += static_cast<unsigned long long>(MU_MobilePerfNow() - attachmentTicksStart);
}

void RenderCharactersClient()
{
    const Uint64 renderTicksStart = static_cast<Uint64>(MU_MobilePerfNow());
    constexpr int kMinFarRenderBudget = 20;
    constexpr int kMinCrowdRenderBudget = 12;
    constexpr int kMaxFarRenderBudget = 96;
    constexpr int kVeryFarQueuePressure = 72;
    constexpr int kFarQueuePressure = 128;
    constexpr int kMidQueuePressure = 176;
    constexpr int kHardCullPressure = 220;
    int farRenderIndices[MAX_CHARACTERS_CLIENT];
    int farRenderCount = 0;
    const bool adaptiveEnabled = IsCharacterAdaptiveEnabled();
    const bool fullCharacterVisibility = IsAndroidFullCharacterVisibilityMode();
    const AdaptiveCharacterDistanceThresholds adaptiveThresholds = GetCharacterDistanceThresholds();
    g_characterVisibleRenderCount = 0;
    g_characterVisibleMonsterRenderCount = 0;

    for (int i = 0; i < MAX_CHARACTERS_CLIENT; ++i)
    {
        CHARACTER* visibleCharacter = &CharactersClient[i];
        OBJECT* visibleObject = &visibleCharacter->Object;
        if (!visibleObject->Live || !visibleObject->Visible)
        {
            continue;
        }

        ++g_characterVisibleRenderCount;
        if (visibleObject->Type != MODEL_PLAYER)
        {
	...
	for ( int i=0; i<MAX_CHARACTERS_CLIENT; ++i )
	{
		...
		if (!g_pNewUISystem->GetUI_NewOptionWindow()->OnOffGrap[g_pNewUISystem->GetUI_NewOptionWindow()->eBMDPlayer] && c != Hero && o->Type == MODEL_PLAYER && SceneFlag == MAIN_SCENE)
		{
			continue;
		}
		...
		if ( o->Visible )
		{
            const int renderPressure = g_characterPerfSnapshot.renderCandidates;
            ++g_characterPerfSnapshot.renderCandidates;
            const float distSq = GetCharacterDistanceSq2D(o);
            const CharacterDistanceBucket bucket = ClassifyCharacterDistance(distSq, adaptiveThresholds);
            TrackCharacterRenderBucket(bucket);
            const bool isPriority = IsPriorityCharacter(i, c);

            if (adaptiveEnabled && !fullCharacterVisibility && !isPriority)
            {
                if (ShouldCullMonsterByImportance(c, bucket, distSq, isPriority))
                {
                    ++g_characterPerfSnapshot.renderDistanceCulled;
                    continue;
                }

                const bool queueAsFar =
                    (bucket == CharacterDistanceBucket::Far &&
                        ((distSq > adaptiveThresholds.veryFarSq && renderPressure > kVeryFarQueuePressure) ||
                            renderPressure > kFarQueuePressure)) ||
                    (bucket == CharacterDistanceBucket::Mid && renderPressure > kMidQueuePressure);
                if (queueAsFar)
                {
                    if (distSq > adaptiveThresholds.hardCullSq && renderPressure > kHardCullPressure)
                    {
                        const uint32_t cullPeriod = (renderPressure > 300) ? 2u : 3u;
                        if (!ShouldRunCharacterStride(i, c->Key, cullPeriod, 0x2f6e2b1du))
                        {
                            ++g_characterPerfSnapshot.renderDistanceCulled;
                            continue;
                        }
                    ...
                    if (farRenderCount < MAX_CHARACTERS_CLIENT)
                    {
                        farRenderIndices[farRenderCount++] = i;
                    }
                    else
                    {
                        ++g_characterPerfSnapshot.renderDeferred;
                    }
                    continue;
                }
            }
    ...
    if (farRenderCount > 0)
    {
        int farRenderBudget = farRenderCount;
        if (adaptiveEnabled && !fullCharacterVisibility)
        {
            const int renderPressure = g_characterPerfSnapshot.renderCandidates;
            farRenderBudget = (std::min)(kMaxFarRenderBudget, (std::max)(kMinFarRenderBudget, 24 + farRenderCount / 6));
            if (renderPressure > 120)
            {
                farRenderBudget = (std::max)(kMinCrowdRenderBudget, farRenderBudget - (renderPressure - 120) / 6);
            }
            if (farRenderCount > 220)
            {
                farRenderBudget = (std::max)(kMinCrowdRenderBudget, farRenderBudget - 8);
            }
        }

        const int start = g_characterFarRenderCursor % farRenderCount;
        int renderedFar = 0;
        int deferredFar = 0;
        for (int n = 0; n < farRenderCount; ++n)
        {
            const int idx = farRenderIndices[(start + n) % farRenderCount];
            ...
            if (renderedFar >= farRenderBudget)
            {
                ++deferredFar;
                continue;
            }
            ...
            const float distSq = GetCharacterDistanceSq2D(o);
            uint32_t renderPeriod = 1u;
            if (distSq > adaptiveThresholds.veryFarSq && farRenderCount > 96)
            {
                renderPeriod = 2u;
            }
            if (farRenderCount > 160)
            {
                ++renderPeriod;
            }
            if (farRenderCount > 240)
            {
                ++renderPeriod;
            }
            if (distSq > adaptiveThresholds.hardCullSq && farRenderCount > 180)
            {
                ++renderPeriod;
            }
            if (adaptiveEnabled && !ShouldRunCharacterStride(idx, c->Key, renderPeriod, 0x94d049bdu))
            {
                ++deferredFar;
                continue;
            }
        ...
        g_characterFarRenderCursor = (start + (std::max)(renderedFar, 1)) % farRenderCount;
    }
...
void ChangeCharacterExt(int Key,BYTE *Equipment, CHARACTER * pCharacter, OBJECT * pHelper)
{
		...
		case 0:				//ÀÛÀº³¯°³
			{
				...
				//===WingMakePreviewCharSet
				int IndexCustomWing = (Equipment[16] >> 1) - 1;
				if (gCustomWing.CheckCustomWing(IndexCustomWing))
				{
					c->Wing.Type = gCustomWing.GetCustomWingItem(IndexCustomWing)+MODEL_ITEM;
				}
			}
			break;
	...
	if (SceneFlag == 4)
	{
		if (SelectCharSet != nullptr)
		{
			...
			if (sNewCharSet0 > 0 && sNewCharSet0 < 112)
			{
				...
				if (pHelper == NULL)
					CreateBug(c->Helper.Type, o->Position, o);
				else
	...
	if (lpCharSet != nullptr)
	{
		//gInterface.DrawMessage(1, "NEW_PET_STRUCT Key %d", c->Key);
		...
		if (NewCharSet1 > 0 && NewCharSet1 < 94)
		{
			...
			//gInterface.DrawMessage(1, "NEW_PET_STRUCT Type %d", c->Helper.Type);
		}
...
void Setting_Monster(CHARACTER *c,int Type,int PositionX,int PositionY)
{
	...
	if(c != NULL)
	{
		...
		NPCNAME_DATA* NpcName = gNPCName.GetNPCName(Type, gMapManager.WorldActive, PositionX, PositionY);
		...
		|| Type == 546
		|| Type == 547
#ifdef ASG_ADD_TIME_LIMIT_QUEST_NPC //Add Custom Monter
			|| Type == 566
			|| Type == 567
			|| Type == 568
#endif
...
CHARACTER *CreateMonster(int Type,int PositionX,int PositionY,int Key)
{
	...
	if (BossClass)
	{
		...
		c->BodyPart[BODYPART_HELM].ExtOption = 1;
		c->BodyPart[BODYPART_ARMOR].Type = MODEL_ARMOR + BossClass->SetID;
		c->BodyPart[BODYPART_ARMOR].Level = 7;
		c->BodyPart[BODYPART_ARMOR].Option1 = 1;
		c->BodyPart[BODYPART_ARMOR].ExtOption = 1;
		c->BodyPart[BODYPART_PANTS].Type = MODEL_PANTS + BossClass->SetID;
		c->BodyPart[BODYPART_PANTS].Level = 7;
		c->BodyPart[BODYPART_PANTS].Option1 = 1;
		c->BodyPart[BODYPART_PANTS].ExtOption = 1;
		c->BodyPart[BODYPART_GLOVES].Type = MODEL_GLOVES + BossClass->SetID;
		c->BodyPart[BODYPART_GLOVES].Level = 7;
		c->BodyPart[BODYPART_GLOVES].Option1 = 1;
		c->BodyPart[BODYPART_GLOVES].ExtOption = 1;
		c->BodyPart[BODYPART_BOOTS].Type = MODEL_BOOTS + BossClass->SetID;
		c->BodyPart[BODYPART_BOOTS].Level = 7;
		c->BodyPart[BODYPART_BOOTS].Option1 = 1;
		c->BodyPart[BODYPART_BOOTS].ExtOption = 1;

		c->Wing.Type = MODEL_ITEM + BossClass->IndexWing;
		c->Wing.Level = 7;
		c->Wing.Option1 = 1;
		c->Wing.ExtOption = 1;

		c->Weapon[0].Type = MODEL_ITEM + BossClass->IndexVK1;
		c->Weapon[0].Level = 9;
		c->Weapon[0].Option1 = 1;
		c->Weapon[0].ExtOption = 1;
		c->Weapon[1].Type = MODEL_ITEM + BossClass->IndexVK2;
		c->Weapon[1].Level = 9;
		c->Weapon[1].Option1 = 1;
		c->Weapon[1].ExtOption = 1;


		c->Object.Scale = BossClass->Size;
		//SetCharacterScale(c);
		if (c != NULL)
		{
			OBJECT* o = &c->Object;
	...
	if (lpInfo != nullptr)
	{
		if (lpInfo->Type == 0) //==NPC
		{
			...
			{
				gLoadData.AccessModel(ModelID, lpInfo->Dir, lpInfo->BMDFile);
				gLoadData.OpenTexture(ModelID, lpInfo->Folder);

				for (int i = 0; i < b->NumActions; i++)
					b->Actions[i].PlaySpeed = 0.25f;
				//SetTexture(BITMAP_NPC);

				if (b->NumMeshs > 0) gLoadData.OpenTexture(ModelID, lpInfo->Folder);
			}
NexCustomMonter:
...
bool RenderCharacterBackItem(CHARACTER *c, OBJECT* o, bool bTranslate)
{
	...
	if(o->Type == MODEL_PLAYER)
	{
		...
		for(int i=0; i<2; ++i)
		{
			...
			if (!g_pNewUISystem->GetUI_NewOptionWindow()->OnOffGrap[g_pNewUISystem->GetUI_NewOptionWindow()->eBMDWeapons]) continue;
			...
			if(iType == MODEL_BOW+7 || iType == MODEL_BOW+15)//CustomArrow Cung,No
			{
				bBack = true;
			}
			else
...
// 			}
			...
			if(bBack && iType != -1)
			{
				...
				if (o->CurrentAction == PLAYER_FLY || o->CurrentAction == PLAYER_FLY_CROSSBOW) // animation vỗ cánh
				{
					w->PlaySpeed = 1.f;
				}
				else
        ...
        if ( gMapManager.InBloodCastle() && c->EtcPart != 0 )
        {
			...
			if (iType != 0) RenderLinkObject(0.f,0.f,15.f, c, w, iType, iLevel, iOption1, true, bTranslate);
        }
        ...
        if(gMapManager.InChaosCastle() == false)
        {
			...
			if(w->Type != -1)
			{
				if (!g_pNewUISystem->GetUI_NewOptionWindow()->OnOffGrap[g_pNewUISystem->GetUI_NewOptionWindow()->eBMDWings]) return bBindBack;
				w->LinkBone = 47;
				if (o->CurrentAction == PLAYER_FLY || o->CurrentAction == PLAYER_FLY_CROSSBOW)
				{
				...
				//== Get CUstom Wing Cloak
				if (gCustomWing.CheckCustomWingByModelType(w->Type - MODEL_ITEM, 1))
				{
					w->LinkBone = 19;
