...
void CreateEffect(int Type, vec3_t Position, vec3_t Angle, vec3_t Light, int SubType, OBJECT * Owner, short PKKey, WORD SkillIndex, WORD Skill, WORD SkillSerialNum, float Scale, short int sTargetIndex)
{
	...
	if (ShouldThrottleAdaptiveEffectSpawn(ADAPTIVE_EFFECT_MODEL, Type, Position, SubType, Scale, Owner))
	{
		return;
	}
#endif
	...
	for (int icntEffect = 0; icntEffect < MAX_EFFECTS; icntEffect++)
	{
		...
		//== Fix Effect
		if ((icntEffect >= MAX_EFFECTS - 2) && o->Live)
		{
			o->Live = false;
		}
		if (!o->Live)
		{
			...
			case MODEL_CUNDUN_SKILL:
				...
				case 2:
					o->LifeTime = 40;
					o->PKKey = 0;
					if (rand_fps_check(2)) CreateEffect(MODEL_CUNDUN_GHOST, o->Position, o->Angle, o->Light, 0, o);
					break;
				};
				break;
...
void MoveEffect(OBJECT * o, int iIndex)
{
	...
	case MODEL_SKILL_WHEEL1:
		...
		break;
	case MODEL_SKILL_WHEEL2: // xoay kiem effect
	{
		switch (o->SubType)
		{
		...
		if (rand() % 1000 == 0) CreateSprite(BITMAP_LIGHT, Position, 2.f, Light, o);
		...
		if (o->SubType == 0)
		{
			if ((int)o->LifeTime == 30)
			{
				for (int i = 0; i < 5; ++i)
				{
					...
					if (rand_fps_check(2)) CreateEffect(MODEL_CUNDUN_PHOENIX, o->Position, Angle, o->Light, 0);
				}
			}
		}
		else if (o->SubType == 1)
...
void RenderWheelWeapon(OBJECT * o)
{
	...
	if (rand_fps_check(1)) RenderPartObject(o, Type, NULL, Light, Alpha, o->Owner->WeaponLevel << 3, 0, 0, true, true, true);
