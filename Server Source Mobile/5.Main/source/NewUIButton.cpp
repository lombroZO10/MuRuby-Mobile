...
bool SEASON3B::CNewUIButton::Render( bool RendOption )
{
	...
	if( m_TooltipText.size() != 0 )
	{
		if( CheckMouseIn(m_Pos.x, m_Pos.y, m_Size.x, m_Size.y) )
		{
			...
			x = ( _iTempWidth > DisplayWin) ? (x - ( _iTempWidth - DisplayWin)) : x;
...
#else // KJH_ADD_INGAMESHOP_UI_SYSTEM
void CNewUIRadioGroupButton::ChangeRadioButtonInfo( bool iswidth, int x, int y, int sx, int sy, int iDistance )
{
	int i = 0;
	...
	for( RadioButtonList::iterator iter = m_RadioList.begin(); iter != m_RadioList.end(); )
	{
		...
		if( button )
		{
			if( iswidth ) button->SetPos(x+((sx+iDistance)*i), y);
			else button->SetPos(x, y+((sy+iDistance)*i));
			button->SetSize( sx, sy );
		}
