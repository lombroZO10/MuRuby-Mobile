...
int DisplayWinCDepthBox = 0;
int DisplayWin = 0;
int DisplayHeight = 0;
int DisplayWinMid = 0;
int DisplayWinExt =0;
int DisplayHeightExt =0;
int DisplayWinReal = 0;
int g_iWidthEx = 5;

extern int g_iChatInputType;
namespace
{
int ComputeSectionCount(int totalSize, int sectionLimit)
{
	if (totalSize <= 0 || sectionLimit <= 0)
	{
		return 0;
	}
	return (totalSize + sectionLimit - 1) / sectionLimit;
}
int ComputeSectionSpan(int totalSize, int sectionLimit, int sectionIndex, int sectionCount)
{
	if (totalSize <= 0 || sectionLimit <= 0 || sectionCount <= 0)
	{
		return 0;
	}
	...
	return totalSize - (sectionLimit * (sectionCount - 1));
}
...
{
	//iPos_x = iPos_x + DisplayWinCDepthBox;
	m_iPos_x = iPos_x;
	m_iPos_y = iPos_y;
}
...
BOOL CUITextListBox<T>::DoMouseAction()
{
	...
	textInputRect.x = static_cast<int>(m_iPos_x * g_fScreenRate_x);
	textInputRect.y = static_cast<int>(m_iPos_y * g_fScreenRate_y);
	textInputRect.w = static_cast<int>(m_iWidth * g_fScreenRate_x);
	textInputRect.h = static_cast<int>(m_iHeight * g_fScreenRate_y);
...
CUISocketListBox::CUISocketListBox()
{
	...
	SetPosition(275+DisplayWinCDepthBox, 360);
	SetSize(160, 65);
}
...
void CUIRenderText::SetShadowText(int Type)
{
	if (m_pRenderText)
		m_pRenderText->SetShadowText(Type);
}
...
#if defined(__ANDROID__) || defined(MU_IOS)
	if (m_hEditWnd != NULL && GetFocus() == m_hEditWnd)
	{
		SDL_Rect textInputRect = {};
		textInputRect.x = static_cast<int>(m_iPos_x * g_fScreenRate_x);
		textInputRect.y = static_cast<int>(m_iPos_y * g_fScreenRate_y);
	...
	case WM_CHAR:
		...
		case VK_RETURN:
			...
			if (pTextInputBox->IsLocked() == TRUE || pTextInputBox->UseMultiline() == TRUE) 
				break;
#if defined(__ANDROID__) || defined(MU_IOS)
			if (pTextInputBox->CheckOption(UIOPTION_ENTERASTAB)
				&& pTextInputBox->GetTabTarget() != NULL
				&& pTextInputBox->GetTabTarget()->GetState() == UISTATE_NORMAL)
			{
...
char* CUITextInputBox::ShowText()
{
	char* pszText = NULL;
	GetWindowText(m_hEditWnd, pszText, 255);
	return pszText;
}
...
void CUITextInputBox::GiveFocus(BOOL SelectText)
{
	...
	g_dwKeyFocusUIID = GetUIID();
#if defined(__ANDROID__) || defined(MU_IOS)
	wchar_t currentText[MAX_TEXT_LENGTH + 1] = { 0 };
	const int currentLength = GetWindowTextW(m_hEditWnd, currentText, MAX_TEXT_LENGTH + 1);
	const BOOL shouldSelectAll = (SelectText == TRUE && currentLength <= 0) ? TRUE : FALSE;
...
void CUITextInputBox::Render()
{
    ...
    if (CheckOption(UIOPTION_PAINTBACK))
    {
        ...
        RenderColor((float)m_iPos_x, (float)m_iPos_y, (float)m_iWidth, (float)m_iHeight);
        EndRenderColor();
    }

    char rawText[MAX_TEXT_LENGTH + 1] = {0,};
    GetText(rawText, MAX_TEXT_LENGTH);

    char displayText[MAX_TEXT_LENGTH + 1] = {0,};
    if (IsPassword() == FALSE)
    {
        strncpy(displayText, rawText, MAX_TEXT_LENGTH);
    ...
    g_pRenderText->RenderText(m_iPos_x + 2, m_iPos_y + 1, displayText);

    BOOL bIsCaretTime = (HaveFocus() && m_iCaretBlinkTemp % 24 < 12);
    if (bIsCaretTime)
    {
        const LRESULT selection = SendMessageW(m_hEditWnd, EM_GETSEL, 0, 0);
        int selectionStart = (int)LOWORD(selection);
        int selectionEnd = (int)HIWORD(selection);
        int caretIndex = (selectionStart > selectionEnd) ? selectionStart : selectionEnd;
        int displayLength = (int)strlen(displayText);

        if (caretIndex < 0)
            caretIndex = 0;
        else if (caretIndex > displayLength)
            caretIndex = displayLength;

        SIZE caretPrefixSize = {0, 0};
        if (caretIndex > 0)
        {
            g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), displayText, caretIndex, &caretPrefixSize);
        }

        SIZE sampleTextSize = {0, 0};
        g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), "Q", 1, &sampleTextSize);

        float invRateX = (g_fScreenRate_x > 0.f) ? (1.f / g_fScreenRate_x) : 1.f;
        float invRateY = (g_fScreenRate_y > 0.f) ? (1.f / g_fScreenRate_y) : 1.f;
        int caretX = m_iPos_x + 2 + (int)(caretPrefixSize.cx * invRateX + 0.5f);
        int caretY = m_iPos_y + 1;
        int caretHeight = (int)(sampleTextSize.cy * invRateY + 0.5f);
        if (caretHeight <= 0)
        {
	...
	if (m_bUseMultiLine == FALSE)
	{
		...
		int iNumberOfSections = ComputeSectionCount(RealTextLine.cx, LIMIT_WIDTH);
		for(int i=0; i<iNumberOfSections; i++)
		{
			SIZE RealSectionLine = { ComputeSectionSpan(RealTextLine.cx, LIMIT_WIDTH, i, iNumberOfSections), RealTextLine.cy };
	...
	else
	{
		...
		RealTextLine.cx = RealWndSize.cx; 
		RealTextLine.cy = RealWndSize.cy;
		int iNumberOfXSections = ComputeSectionCount(RealTextLine.cx, LIMIT_WIDTH);
		int iNumberOfYSections = ComputeSectionCount(RealTextLine.cy, LIMIT_HEIGHT);
		int iPitch = ((RealWndSize.cx*24+31)&~31)>>3;
		for(int y=0; y<iNumberOfYSections; y++)
		{
			SIZE RealSectionLine = { LIMIT_WIDTH, ComputeSectionSpan(RealTextLine.cy, LIMIT_HEIGHT, y, iNumberOfYSections) };
			for(int x=0; x<iNumberOfXSections; x++)
			{
				RealSectionLine.cx = ComputeSectionSpan(RealTextLine.cx, LIMIT_WIDTH, x, iNumberOfXSections);
...
{
	if(iIndex < 0 || iIndex > MAX_MY_INVENTORY_EX_INDEX || cComType == COMGEM::NOCOM) return;
...
CUIInGameShopListBox::CUIInGameShopListBox()
{
	...
	SetPosition(490+ DisplayWinExt, 360);
}
