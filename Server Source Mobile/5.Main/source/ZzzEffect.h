...
enum AdaptiveEffectKind
{
	ADAPTIVE_EFFECT_PARTICLE = 0,
	ADAPTIVE_EFFECT_SPRITE = 1,
	ADAPTIVE_EFFECT_JOINT = 2,
	ADAPTIVE_EFFECT_MODEL = 3,
};
...
void CreatePoint(vec3_t Position, QWORD Value,vec3_t Color,float scale=15.f, bool bMove=true, bool bRepeatedly = false);
...
bool ShouldEmitStableAmbientFrame(const OBJECT *o,int period,int phase=0);
