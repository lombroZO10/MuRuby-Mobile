...
// Android zoom override: when > 0, UpdateCameraDistance() uses this value
// instead of the g_shCameraLevel-based table.  Set by virtual ZOOM +/- buttons.
...
static float GetAndroidStableSceneViewFar()
{
...
#if defined(__ANDROID__) || defined(MU_IOS)
    // Scale CameraViewFar to match zoom level.
    // Zoom OUT (farther camera) sees more terrain → needs larger view far.
    // Zoom IN  (closer camera)  has wider angle  → also needs larger view far.
