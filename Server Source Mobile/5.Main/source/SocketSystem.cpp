...
BOOL CSocketItemMgr::IsSocketItem(int iItemType)
{
	...
	case ITEM_HELM + 77: case ITEM_ARMOR + 77: case ITEM_PANTS + 77: case ITEM_GLOVES + 77: case ITEM_BOOTS + 77:
		return TRUE;
	default:
		return FALSE;
	}
...
int CSocketItemMgr::AttachToolTipForSocketItem(const ITEM * pItem, int iTextNum)
{
	...
	//gInterface.DrawMessage(1, "Item %d Socket %d Number %d <> %d (%d)", pItem->Type, pItem->SocketCount, GetNum, iTextNum, pItem->SocketSeedSetOption);
	return iTextNum;
}
	...
	for (int i = 0; i < MAX_EQUIPMENT; ++i)
	{
		...
		for (int j = 0; j < pItem->SocketCount; ++j)
		{
			if (pItem->SocketSeedID[j] != SOCKET_EMPTY)
			{
				...
				if (pInfo->m_iOptionCategory >= 1 && pInfo->m_iOptionCategory <= 6)
