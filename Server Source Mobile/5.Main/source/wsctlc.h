...
public:
	...
	__forceinline int  sSend(char *buf, int len)
	{
		...
		{
#if(ENCRYPT_STATE==1)
			if (gProtect.CheckSocketPort(m_socket) != 0)
			{
				gProtect.EncryptData((BYTE*)buf + nDx, len - nDx);
				g_ConsoleDebug->Write(MCD_ERROR, "[Send Packet ] EncryptData 3 __forceinline int  sSend");
			}
#endif
