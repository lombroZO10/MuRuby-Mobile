...
namespace
{
	bool ParseIPv4Octets(const char* address,unsigned int& a,unsigned int& b,unsigned int& c,unsigned int& d)
	{
		...
		return (sscanf(address,"%u.%u.%u.%u",&a,&b,&c,&d) == 4);
	}

	bool IsPrivateIPv4(const char* address)
	{
		unsigned int a = 0;
		unsigned int b = 0;
		unsigned int c = 0;
	...
	const char* ResolveServerConnectAddress(const char* address,char* output,size_t outputSize)
	{
		...
		if(IsPrivateIPv4(address) != 0 && szServerIpAddress != 0 && szServerIpAddress[0] != 0 && IsPrivateIPv4(szServerIpAddress) == 0)
		{
			...
			strcpy_s(output,outputSize,szServerIpAddress);
		}
...
{
	char ResolvedIpAddr[64];
	char* ConnectIpAddr = (char*)ResolveServerConnectAddress(IpAddr,ResolvedIpAddr,sizeof(ResolvedIpAddr));
	...
	g_ErrorReport.Write("[Connect to Server 33] ip address = %s, port = %d\r\n", ConnectIpAddr, Port);

	g_ConsoleDebug->Write(MCD_NORMAL, "[Connect to Server] ip address = %s, port = %d", ConnectIpAddr, Port);

	SocketClient.Create(g_hWnd, TRUE);
	if( SocketClient.Connect(ConnectIpAddr,Port,WM_ASYNCSELECTMSG) == FALSE )
	{
...
void ReceiveTradeInventory( BYTE *ReceiveBuffer )
{
	...
	for(int i=0;i<Data->Value;i++)
	{
		...
		else
		{
			...
			{
				g_ConsoleDebug->Write(MCD_ERROR, " [Item Ware] %d", Data2->Index);
...
{
	if(SceneFlag == LOG_IN_SCENE && CurrentProtocolState == REQUEST_JOIN_SERVER)
	{
	...
	else
	{
		...
		else if (Data->ChatText[0] == '$') //Chat Gens
		{
			for (int i = 0; i < MAX_CHAT_SIZE - 1; i++)
				Text[i] = Data->ChatText[i + 1];
...
{	
	...
	case 7:
		if(Type==0x1FFF)
		{
			...
			if (gCustomWing.CheckCustomWingByModelType(Type - MODEL_ITEM, 1))
			{
				DeleteCloth(c, o);
			}
			c->Wing.Type = -1;
	...
	case 8:
		...
		else
		{
			...
			//==CustomPet
			if (gCustomPet2.CheckCustomPetByItem(Type))
			{
				CreateBug(Type+1171, o->Position, o);
			}
		}
		break;
	}
...
void ReceiveCreatePlayerViewport(BYTE *ReceiveBuffer,int Size)
{
	...
	for(int i=0;i<Data->Value;i++)
	{
        ...
        if(FindText(Temp, "webzen") == false)
		{
			...
			g_ConsoleDebug->Write(MCD_ERROR, "(RCPV)ID : %s | sX : %d | sY : %d | tX : %d | tY : %d", c->ID, c->PositionX, c->PositionY, c->TargetX, c->TargetY);
			...
			if (c)
			{
				...
				if (Type != 0x1FFF)
				{
					...
					int CacheSlot = c->m_PetInfo[PET_TYPE_END].m_dwPetType;
					...
					//char Text[100];
					//sprintf(Text, "ReceiveCreatePlayerViewport %d %s Muun %d %d", Key, c->ID, Type, c->m_PetInfo[PET_TYPE_END].m_dwPetType);
					//CreateNotice(Text, 1);
				}
				else
				{
...
void ReceiveCreateTransformViewport( BYTE *ReceiveBuffer )
{
	...
	for(int i=0;i<Data->Value;i++)
	{
		...
		//char Text[100];
		//sprintf(Text, "1ReceiveCreateTransformViewport %d Muun", Key );
		//CreateNotice(Text, 1);
        ...
        if(FindText(Temp, "webzen") == false)
		{
			...
			if (pCha)
			{
				...
				if (Type != 0x1FFF)
				{
					//DeleteBug(&pCha->Object);
					//*(WORD*)(pCha + 592) = Type + 1171;
					pCha->m_PetInfo[PET_TYPE_END].m_dwPetType = Type + 1171;
					CreateEffectEquipping(Type, pCha, &pCha->Object);
					//char Text[100];
					//sprintf(Text, "ReceiveCreateTransformViewport %d Muun %d", Key, Type);
					//CreateNotice(Text, 1);
				}
				else
				{
					pCha->m_PetInfo[PET_TYPE_END].m_dwPetType = 0xFFFF;
					//DeleteBug(&pCha->Object);
				}
			}
#endif	
...
void ReceiveDamage( BYTE *ReceiveBuffer )
{
	...
	CharacterAttribute->PrintPlayer.ViewCurHP = Data->ViewCurHP;
	CharacterAttribute->PrintPlayer.ViewCurSD = Data->ViewCurSD;
	CharacterAttribute->PrintPlayer.ViewDamageHP = Data->ViewDamageHP;
	CharacterAttribute->PrintPlayer.ViewDamageSD = Data->ViewDamageSD;
...
void ReceiveAttackDamage( BYTE *ReceiveBuffer )
{
	...
	else
	{
		...
		//====
		CUSTOM_DMG_COLOR* GetColorDmg = nullptr;

		if (bComboEnable)
		{
			GetColorDmg = gCustomDmgColor.GetInfoTypeIndex(8);
		}
		else if (bDoubleEnable)
		{
			GetColorDmg = gCustomDmgColor.GetInfoTypeIndex(7);
		}
		else if (DamageType >= 0 && DamageType <= 15)
		{
			GetColorDmg = gCustomDmgColor.GetInfoTypeIndex(DamageType);
		}
		if (GetColorDmg)
		{
			Vector(RGBTOGLFLOAT(GetColorDmg->Red), RGBTOGLFLOAT(GetColorDmg->Green), RGBTOGLFLOAT(GetColorDmg->Blue), Light);
		}
		//====
#ifdef PBG_ADD_NEWCHAR_MONK_SKILL
...
void ReceiveAction(BYTE *ReceiveBuffer,int Size)
{
	...
	case AT_RAGEBUFF_1:
		{
			g_ConsoleDebug->Write(MCD_NORMAL, "Recv Action  %d", Data->Action);
	...
	case AT_RAGEBUFF_2:
		{
			g_ConsoleDebug->Write(MCD_NORMAL, "Recv Action2  %d", Data->Action);
	...
	g_ConsoleDebug->Write(MCD_NORMAL, "0x18 [ReceiveAction(%d)] %d", Data->Angle, Data->Action);
	g_ConsoleDebug->Write(MCD_NORMAL, "0x18 [ReceiveAction(%d)] %d", Data->Angle, Data->Action);
}
...
//#endif
	...
	if(
		...
		(AT_SKILL_DEF_POWER_UP <= MagicNumber && MagicNumber <= AT_SKILL_DEF_POWER_UP+4) || (AT_SKILL_ATT_POWER_UP <= MagicNumber && MagicNumber <= AT_SKILL_ATT_POWER_UP+4))
		{
			if(Success == false)
			{
		...
		switch(MagicNumber)
		{
			...
			case MASTER_SKILL_ADD_FORCE_WAVE_IMPROVED:
				...
				if ( (sc->Helper.Type>=MODEL_HELPER+2 && sc->Helper.Type<=MODEL_HELPER+4) || gCustomPet2.GetInfoPetType(sc->Helper.Type - 1171) == 6 && !sc->SafeZone )
				{
					SetAction ( so, PLAYER_ATTACK_RIDE_STRIKE );
				}
				else if(sc->Helper.Type == MODEL_HELPER+37 || gCustomPet2.GetInfoPetType(sc->Helper.Type - 1171) == 5 && !sc->SafeZone)
				{
			...
			case AT_SKILL_BRAND_OF_SKILL:
				...
				if ( (sc->Helper.Type==MODEL_HELPER+4 || gCustomPet2.GetInfoPetType(sc->Helper.Type - 1171) == 6) && !sc->SafeZone )
				{
			...
			case MASTER_SKILL_ADD_GREATER_LIFE_MASTERED:
				...
				if(AT_SKILL_LIFE_UP == MagicNumber) g_CharacterRegisterBuff(to, eBuff_HpRecovery); //eBuff_Life
			...
			case MASTER_SKILL_ADD_FIRE_SCREAM_ENHANCED:
				...
				{
					if( sc->Helper.Type == MODEL_HELPER+37 || gCustomPet2.GetInfoPetType(sc->Helper.Type - 1171) == 5)
					{
						SetAction( so, PLAYER_FENRIR_ATTACK_DARKLORD_STRIKE );
					}
					else if((sc->Helper.Type>=MODEL_HELPER+2) && (sc->Helper.Type<=MODEL_HELPER+4) || gCustomPet2.GetInfoPetType(sc->Helper.Type - 1171) == 6)
					{
						SetAction( so, PLAYER_ATTACK_RIDE_STRIKE );
					}
					else
...
//#endif
	...
	//int     Damage = ((int)(Data->DamageH)<<8) + Data->DamageL;
	QWORD     Damage = Data->ViewDamageHP;
	int     Success = (Key>>15);
	Key &= 0x7FFF;
...
BOOL ReceiveEquipmentItem(BYTE *ReceiveBuffer, BOOL bEncrypted)
{
	...
	if (Data->SubCode != 255)
	{
		...
		else if (STORAGE_TYPE::LUCKYITEM_ == storageType || STORAGE_TYPE::LUCKYITEM__ == storageType)
		{
	...
	if (g_pMyInventory->SlotAutoMove != -1)
	{
		g_pMixInventory->DeleteItem(g_pMyInventory->SlotAutoMove);
		g_pMyInventory->SlotAutoMove = -1;
	}
...
void ReceiveLevelUp( BYTE *ReceiveBuffer )
{
	...
	__int64 iExp = CharacterAttribute->NextExperince - CharacterAttribute->Experience;
...
void ReceiveGuildIDViewport( BYTE *ReceiveBuffer )
{
	...
	for( int i=0 ; i<Data->Value ; ++i )
	{
		...
		else
		{
			...
			g_ConsoleDebug->Write(MCD_ERROR, "SendRequestGuildInfo"); //Test Cache Guild
		}
...
void ReceiveGuildInfo( BYTE *ReceiveBuffer )
{
	...
	g_ConsoleDebug->Write(MCD_ERROR, "ReceiveGuildInfo");
}
...
void Receive_Master_LevelUp( BYTE *ReceiveBuffer )
{
	...
	unicode::t_char szText[256] = {NULL, };
	__int64 iExp = Master_Level_Data.lNext_MasterLevel_Experince - Master_Level_Data.lMasterLevel_Experince;
...
void ReceiveSetPriceResult(BYTE* ReceiveBuffer)
{
	...
	if (Header->byResult != 0x01 && g_IsPurchaseShop == PSHOPWNDTYPE_SALE)
	{
		...
		g_ConsoleDebug->Write(MCD_ERROR, "@ [Fault] ReceiveSetPriceResult (result : %d) %d\n", Header->byResult, MAX_MY_INVENTORY_EX_INDEX + g_pMyShopInventory->GetTargetIndex());
	}
}
...
void ReceivePersonalShopItemList(BYTE* ReceiveBuffer)
{
	...
	if (Header->byResult == 0x01)
	{
		...
		for (int i = 0; i < Header->byCount; i++, pShopItem++)
		{
			...
			{
				g_pPurchaseShopInventory->InsertItem(pShopItem->byPos- MAX_MY_INVENTORY_EX_INDEX, pShopItem->byItemInfo);
...
void ReceivePurchaseItem(BYTE* ReceiveBuffer)
{
	...
	else
	{
		...
		{
#if(CB_CUSTOMOFFTRADE)
			if (gCB_OffTrade) gCB_OffTrade->ShowMessNotice();
#else
		...
		SEASON3B::CNewUIInventoryCtrl::BackupPickedItem();
		g_ConsoleDebug->Write(MCD_RECEIVE, "@ [Fault] ReceivePurchaseItem (result : %d)\n", Header->byResult);
	}
}
...
void ReceiveChangeMapServerResult ( BYTE* ReceiveBuffer )
{
    ...
    g_ErrorReport.Write("[AndroidLogin] ReceiveChangeMapServerResult result=0x%02X logIn=%d reconnectStatus=%u reconnectProgress=%u\r\n",
        ...
        LogIn,
#if(UseReconnect)
        (unsigned int)g_pReconnect->s_Data.ReconnectStatus,
        (unsigned int)g_pReconnect->s_Data.ReconnectProgress
#else
        0,
        0
#endif
    ...
    bool reconnectJoinPending =
        (g_pReconnect->s_Data.ReconnectStatus == CReconnect::RECONNECT_STATUS_RECONNECT &&
        (g_pReconnect->s_Data.ReconnectProgress == CReconnect::RECONNECT_PROGRESS_JOINED ||
         g_pReconnect->s_Data.ReconnectProgress == CReconnect::RECONNECT_PROGRESS_CHAR_LIST));
#else
    bool reconnectJoinPending = false;
#endif

    if (Data->Value == 1 && LogIn != 0 && reconnectJoinPending == false)
    {
        g_csMapServer.SendJoinCurrentHero();
    }
...
void ReceiveCheckSumRequest( BYTE *ReceiveBuffer)
{
	...
	if (Data->NumberCheckSum != CB_NUMBERCHECKSUM)
	{
		ExitProcess(0);
	}
#endif
...
void ProtocolCompiler( CWsctlc *pSocketClient, int iTranslation, int iParam)
{
	...
	const int maxMessages = (g_MaxMessagePerCycle > 0) ? g_MaxMessagePerCycle : -1;
#endif
...
//#endif
	...
	g_ConsoleDebug->Write(MCD_NORMAL, "ReceiveStraightAttack AttackNumber %d TargetIndex %d", AttackNumber, TargetIndex);
...
void Receive_Master_SkillList(BYTE* ReceiveBuffer)
{
	...
	g_pMasterSkillTreeInterface->SetMasterType(gCharacterManager.GetCharacterClass(CharacterAttribute->Class));
	//g_pMasterSkillTreeInterface->SetMasterSkillTreeInfo(37, 5, 511.000000, 1511.000000);
	...
	for (int n = 0; n < lpMsg->count; n++)
	{
		...
		g_ConsoleDebug->Write(MCD_RECEIVE, "0x53 [Receive_Master_SkillList]");
	}
}
...
BOOL TranslateProtocol( int HeadCode, BYTE *ReceiveBuffer, int Size, BOOL bEncrypted)
{
	...
	{
		LPPHEADER_DEFAULT_SUBCODE Data = (LPPHEADER_DEFAULT_SUBCODE)ReceiveBuffer;
		g_ConsoleDebug->Write(MCD_RECEIVE, "[%x][TranslateProtocol] HeadCode (%x) Sub (%x)", ReceiveBuffer[0], HeadCode, Data->SubCode);
	}
	else
	{
		LPPHEADER_DEFAULT_SUBCODE_WORD Data = (LPPHEADER_DEFAULT_SUBCODE_WORD)ReceiveBuffer;
	...
	case 0xF3:     			
		{
			...
			case 0x04: //receive revival GCCharacterRegenRecv
			...
			case 0x52:
				...
				break;
			case 0x53:
				Receive_Master_SkillList(ReceiveBuffer);
				break;
