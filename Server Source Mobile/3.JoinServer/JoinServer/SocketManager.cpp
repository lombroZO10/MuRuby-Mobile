...
bool CSocketManager::DataRecv(int index,IO_MAIN_BUFFER* lpIoBuffer) // OK
{
	...
	while(true)
	{
		...
		else
		{
			...
			{
				strcpy_s(ip,gServerManager[index].m_IpAddr);
			}

			LogAdd(LOG_RED,"[SECURITY][JoinServer] Protocol header error (Index: %d, Ip: %s, Header: %x)",index,ip,lpMsg[count]);
			return 0;
		}
		...
		if(size < 3 || size > MAX_MAIN_PACKET_SIZE)
		{
			...
			LogAdd(LOG_RED,"[SECURITY][JoinServer] Protocol size error (Index: %d, Ip: %s, Header: %x, Size: %d, Head: %x)",index,ip,header,size,head);
			return 0;
		}
