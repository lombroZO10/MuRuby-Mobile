...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) // OK
{
	...
	{
		WORD ConnectServerPortTCP = GetPrivateProfileInt("ConnectServerInfo","ConnectServerPortTCP",63000,".\\ConnectServer.ini");

		WORD ConnectServerPortUDP = GetPrivateProfileInt("ConnectServerInfo","ConnectServerPortUDP",63001,".\\ConnectServer.ini");
		...
		if(gSocketManager.Start(ConnectServerPortTCP) != 0)
		{
			...
			{
				gServerList.Load("ServerList.ini");
	...
	{
		LogAdd(LOG_RED,"WSAStartup() failed with : %d",WSAGetLastError());
	}
