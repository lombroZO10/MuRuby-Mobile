...
bool CSocketManager::CreateListenSocket() // OK
{
	...
	{
		LogAdd(LOG_RED,"[Premium] bind() failed Port with error: %d",WSAGetLastError());
		return 0;
	}
...
bool CSocketManager::DataRecv(int index,IO_MAIN_BUFFER* lpIoBuffer) // OK
{
	...
	while(true)
	{
		...
		else
		{
			...
			{
				strcpy_s(ip,gClientManager[index].m_IpAddr);
			}

			LogAdd(LOG_RED,"[SECURITY][ConnectServer] Protocol header error (Index: %d, Ip: %s, Header: %x)",index,ip,lpMsg[count]);
			return 0;
		}
		...
		if(size < 3 || size > MAX_MAIN_PACKET_SIZE)
		{
			...
			LogAdd(LOG_RED,"[SECURITY][ConnectServer] Protocol size error (Index: %d, Ip: %s, Header: %x, Size: %d, Head: %x)",index,ip,header,size,head);
			return 0;
		}
